...
import { durationSeconds, makeRunRecord, mergeRunIntoReport } from "./ci-report-publisher.mjs";

test("durationSeconds returns whole seconds and rejects incomplete ranges", () => {
  assert.equal(durationSeconds("2026-09-24T10:00:00Z", "2026-09-24T10:00:07.500Z"), 8);
  assert.equal(durationSeconds("2026-09-24T10:00:00Z", null), null);
  assert.equal(durationSeconds("bad", "also bad"), null);
  assert.equal(durationSeconds("2026-09-24T10:00:10Z", "2026-09-24T10:00:00Z"), null);
});
...
test("run records link PR, SHA, status, duration, and per-job runner identity", () => {
  ...
  ], [{ number: 527, html_url: "https://github.com/duikindiesee/citylife/pull/527" }]);

  assert.equal(record.runId, 42);
  assert.equal(record.sha, sha);
  assert.equal(record.commitUrl, `https://github.com/duikindiesee/citylife/commit/${sha}`);
  assert.equal(record.pullRequestNumbers, "#527");
  assert.equal(record.pullRequestUrl, "https://github.com/duikindiesee/citylife/pull/527");
  ...
  assert.deepEqual(record.jobs[0].runnerLabels, ["self-hosted", "Windows", "X64"]);
...
test("REST-shaped PR metadata without html_url still gets a same-repository link", () => {
  ...
  }, [], [{ number: 527, url: "https://api.github.com/repos/duikindiesee/citylife/pulls/527" }]);
  assert.equal(record.pullRequestUrl, "https://github.com/duikindiesee/citylife/pull/527");
});

test("report upsert is idempotent, newest first, and bounded", () => {
  const original = { schemaVersion: 1, runs: [{ runId: 1, conclusion: "failure" }] };
  const updated = mergeRunIntoReport(original, { runId: 1, conclusion: "success" }, 3, "2026-09-24T00:00:00Z");
  assert.equal(updated.runs.length, 1);
  assert.equal(updated.runs[0].conclusion, "success");

  const bounded = mergeRunIntoReport(updated, { runId: 4 }, 3, "2026-09-24T00:00:01Z");
  const latest = mergeRunIntoReport(bounded, { runId: 3 }, 3, "2026-09-24T00:00:02Z");
  const capped = mergeRunIntoReport(latest, { runId: 2 }, 3, "2026-09-24T00:00:03Z");
  assert.deepEqual(capped.runs.map((run) => run.runId), [4, 3, 2]);
...
test("PR URLs outside the repository are omitted", () => {
  const record = makeRunRecord({
    ...
    { number: 2, html_url: "https://github.com/duikindiesee/citylife/pull/2" },
  ]);
  assert.equal(record.pullRequestNumbers, "#2");
  assert.equal(record.pullRequestUrl, "https://github.com/duikindiesee/citylife/pull/2");
});
