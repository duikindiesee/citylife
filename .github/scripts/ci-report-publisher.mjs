...
export function makeRunRecord(
  ...
  const normalizedPullRequests = pullRequests
    .filter(
      (pr) => Number.isSafeInteger(Number(pr.number)) && Number(pr.number) > 0,
    )
    .map((pr) => {
  ...
  const normalizedJobs = jobs.map((job) => ({
    ...
    runnerLabels: Array.isArray(job.labels)
      ? job.labels.map((label) => safeText(label, 100)).filter(Boolean)
      : [],
  ...
  const runnerNames = [
    ...new Set(
      normalizedJobs
        .map((job) => job.runnerName || job.runnerGroup)
        .filter(Boolean),
    ),
  ];
...
export function mergeRunIntoReport(
  existing,
  incoming,
  limit = MAX_RUNS,
  ...
  const runs = [...byId.values()]
    .filter(
      (run) => Number.isSafeInteger(Number(run.runId)) && Number(run.runId) > 0,
    )
...
function createGitHubClient({
  ...
  async function request(path, options = {}) {
    ...
    if (!response.ok)
      throw new ApiError(
        response.status,
        body?.message ?? `GitHub API ${response.status}`,
      );
    return body;
  }
  return { request };
...
async function resolvePullRequests(client, run, repository) {
  ...
  return (Array.isArray(associated) ? associated : []).filter(
    (pr) => pr.base?.repo?.full_name === repository,
  );
}
...
async function readReport(client, branchSha) {
  ...
  try {
    const file = await client.request(
      `/contents/${REPORT_PATH}?ref=${REPORT_BRANCH}`,
    );
...
async function publishReport(client, record, attempts = 8) {
  ...
  throw new Error(
    "Could not publish CI report after concurrent update retries",
  );
}

async function main() {
  const repository =
    process.env.CI_REPORT_REPOSITORY || process.env.GITHUB_REPOSITORY;
  const token = process.env.GITHUB_TOKEN;
  const eventPath = process.env.GITHUB_EVENT_PATH;
  if (!repository || !token || !eventPath)
    throw new Error("Missing repository, GITHUB_TOKEN, or event payload path");
  ...
  if (run.repository?.full_name !== repository)
    throw new Error("Refusing to report a run from another repository");
  ...
  if (!record.sha)
    throw new Error("Workflow run has no valid 40-character commit SHA");
  const commitSha = await publishReport(client, record);
  console.log(
    `Published run ${record.runId} for ${repository} (${record.sha}) as ${commitSha}`,
  );
}
