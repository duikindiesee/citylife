...
describe("door access — the driveway lands ON the compiled door", () => {
  it("blueprintDoorCell matches the compiler: same column the micro-grid door is carved on", () => {
    ...
    const compiled = compileBlueprint(script, {
      w: p.houseZone.w,
      d: p.houseZone.d,
      seed: p.houseSeed,
    });
  ...
  it("retarget to the street door keeps the straight drive: verge -> gate -> door, one column", () => {
    ...
    expect(p.fence.some((f) => f.x === p.gate.x && f.y === p.gate.y)).toBe(
      false,
    );
  });
  ...
  it("a side (east) door opens a second fence gap beside the real door", () => {
    ...
    expect(p.fence.some((f) => f.x === door.x + 1 && f.y === door.y)).toBe(
      false,
    );
    expect(p.fence.some((f) => f.x === p.gate.x && f.y === p.gate.y)).toBe(
      false,
  ...
  it("a rear door gets a doorstep pad onto the garden", () => {
    ...
    expect({ x: p.doorX, y: p.doorY }).toEqual(door);
    expect(
      p.driveway.some((c) => c.x === door.x && c.y === door.y + p.side),
    ).toBe(true);
  });
  ...
  it("archetype blueprints all author street-facing doors, so every house stays reachable", () => {
    for (const p of parcels(42)) {
      const dir = parseBlueprint(
        defaultBlueprint(p.houseSeed, streetDoorDir(p)),
      ).doorDir;
...
describe("door access S2 — the side-strip L-walkway", () => {
  function stripParcel(): Parcel {
    ...
    expect(
      p.houseZone.x,
      "expected an estate-tier parcel with a side strip",
    ).toBeGreaterThanOrEqual(x0 + 2);
    return p;
  }
  ...
  function inParcelRect(p: Parcel, c: { x: number; y: number }): boolean {
    const xHalf = (p.w - 1) / 2;
    const y0 =
      (p.side === 1 ? p.gate.y : p.gate.y - p.h + 1) - (p.side === 1 ? 2 : 0);
    const y1 =
      (p.side === 1 ? p.gate.y + p.h - 1 : p.gate.y) + (p.side === 1 ? 0 : 2);
  ...
  it("an east door gets a connected walkway ending ON the door, fence whole, all inside the parcel", () => {
    ...
    for (const c of p.driveway)
      expect(inParcelRect(p, c), `cell ${c.x},${c.y} escapes the parcel`).toBe(
        true,
      );
    // The side fence stays WHOLE — the walkway is internal; only the front gate gap exists.
    expect(p.fence.some((f) => f.x === door.x + 2 && f.y === door.y)).toBe(
      true,
    );
    expect(p.fence.some((f) => f.x === p.gate.x && f.y === p.gate.y)).toBe(
      false,
...
describe("door access S2 — founder restore guard", () => {
  it("a founder plot only restores its own design; ordinary lots restore anything", () => {
    expect(
      canRestoreBlueprint({ reservedFor: "citizen_joe" }, "citizen_joe"),
    ).toBe(true);
    expect(
      canRestoreBlueprint({ reservedFor: "citizen_joe" }, "citizen_someone"),
    ).toBe(false);
    expect(
      canRestoreBlueprint({ reservedFor: undefined }, "citizen_anyone"),
    ).toBe(true);
  });
});
