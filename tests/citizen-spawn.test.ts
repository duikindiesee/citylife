...
describe('splitName', () => {
  it('splits a two-word name into first + last', () => {
    expect(splitName('Dax Brackenhollow')).toEqual({ firstName: 'Dax', lastName: 'Brackenhollow' })
  })
...
describe('spawnCitizenSubUser', () => {
  it('fails closed when no one is signed in', async () => {
    ...
    const r = await spawnCitizenSubUser({ firstName: 'Dax', lastName: 'Brackenhollow' })
  ...
  it('posts the citizen with the player Bearer when signed in', async () => {
    ...
    vi.stubGlobal('fetch', async (_url: string, init: RequestInit) => {
      ...
      return { ok: true, status: 200, json: async () => ({ userId: 42, profileId: 7, status: 'PROVISIONING' }) }
    })
    const r = await spawnCitizenSubUser({ firstName: 'Dax', lastName: 'Brackenhollow', age: 41, profession: 'Botanist' })
    expect(r).toEqual({ ok: true, citizenUserId: 42, profileId: 7, status: 'PROVISIONING' })
    expect(sentAuth).toBe('Bearer player.jwt.tok')
    expect(sentBody).toMatchObject({ firstName: 'Dax', profession: 'Botanist', age: 41 })
  })
