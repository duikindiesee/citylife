...
import {
  ...
  libraryStep,
  wasteStep,
  wasteDesirabilityFactor,
  wasteStatus,
  securityStatus,
  dietVarietyStatus,
  varietyCovered,
  varietyDesirabilityFactor,
  varietyEvolutionFactor,
  labourStatus,
  planterStatus,
  planterBlooming,
  planterLiveabilityBoost,
  planterDesirabilityFactor,
  stallStatus,
  galleryStatus,
  galleryAppeal,
  galleryStep,
...
describe("Spec 009 — First Aid Clinic: health keeps the workers productive", () => {
  ...
  it("a colony with clinic coverage out-produces an unhealthy one (sick workers dig slower)", () => {
    const run = (withClinic: boolean) => {
      ...
      s.materials = 10;
      s.buildings.push(
        mk("mine", s.terrain.landing.x + 3, s.terrain.landing.y, {
          jobs: 6,
          materialsGen: 5,
        }),
      );
...
describe("Spec 011 — Civic Pulse Survey Office + the liveability map", () => {
  const mk = (
    kind: "habitat" | "water" | "depot" | "clinic" | "theatre" | "survey",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
...
describe("Spec 012 — Skybridge Exchange trades surplus for treasury", () => {
  ...
  it("a staffed exchange ships surplus components above the reserve and earns treasury", () => {
    ...
    expect(s.components).toBeGreaterThanOrEqual(
      COLONY.build.tradeComponentReserve,
    ); // never traded below the reserve
...
describe("Spec 015 — the full-service top tier: grand homes demand the whole stack", () => {
  ...
  const svc = (
    kind: "water" | "depot" | "clinic" | "theatre" | "market",
    x: number,
    y: number,
  ): ColonyBuilding => ({
    id: x * 1000 + y + 7,
...
describe("Spec 016 — the Kookerverse Courier broadcasts the colony news", () => {
  const mk = (
    kind: "mast" | "water" | "habitat" | "theatre",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  it("a congested colony mines less than the same colony with enough Transit Depots", () => {
    const run = (depots: number) => {
      ...
      for (let i = 0; i < depots; i++)
        s.buildings.push(mk("transit", 60 + i, 60));
      const m0 = s.materials;
      for (let i = 0; i < 80; i++) stepBuild(s, sim.rng, 10);
      return s.materials - m0;
    };
...
describe("Spec 023 — Storehouse Platforms: finite storage, the surplus goes overboard", () => {
  ...
  it("storage caps grow with each Storehouse Platform", () => {
    ...
    s.buildings.push(
      mk("storehouse", 50, 50, { jobs: COLONY.build.storehouseWorkers }),
    );
    const one = storageCaps(s);
    expect(one.materials).toBe(
      COLONY.build.storeBaseMaterials + COLONY.build.storePerMaterials,
    );
    expect(one.components).toBe(
      COLONY.build.storeBaseComponents + COLONY.build.storePerComponents,
    );
  });
...
describe("Spec 024 — Emergency Bellhouse: incidents strike, get answered, or hit a consequence", () => {
  ...
  it("a building with an active incident produces nothing", () => {
    ...
    s.totalJobs = 6;
    s.materials = 5;
    const mine = mk("mine", s.terrain.landing.x + 3, s.terrain.landing.y, {
      jobs: 6,
      materialsGen: 5,
    });
  ...
  it("a staffed Emergency Bellhouse resolves an incident and the building recovers", () => {
    ...
    s.buildings.push(
      mine,
      mk("bellhouse", s.terrain.landing.x + 4, s.terrain.landing.y, {
...
describe("Spec 025 — The Levy Office: the council sets a fiscal rate with a real tradeoff", () => {
  ...
  it("the levy is inert until a Levy Office is built and staffed", () => {
    ...
    s.buildings.push(
      mk("levy", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 2 }),
    );
    s.totalJobs = 2;
    s.colonists = 0;
  ...
  it("with a staffed office, a higher levy earns more treasury per day", () => {
    expect(incomeAfterOneDay("high", true)).toBeGreaterThan(
      incomeAfterOneDay("normal", true),
    );
    expect(incomeAfterOneDay("normal", true)).toBeGreaterThan(
      incomeAfterOneDay("low", true),
    );
  });

  it("with no Levy Office the rate is inert — income is identical at every setting", () => {
    expect(incomeAfterOneDay("high", false)).toBe(
      incomeAfterOneDay("low", false),
    );
    expect(incomeAfterOneDay("normal", false)).toBe(
      incomeAfterOneDay("low", false),
    );
  });
  ...
  it("a gentle levy draws more settlers than a hard one", () => {
    const immigrationUnder = (rate: "low" | "high") => {
      ...
      s.buildings.push(
        mk("habitat", s.terrain.landing.x + 4, s.terrain.landing.y, {
          residents: 9,
        }),
      ); // room to grow
      s.colonists = 4;
...
describe("Spec 026 — The Fever Watch: an outbreak that spreads, sickens, and is contained", () => {
  const mk = (
    kind: "habitat" | "mine" | "clinic" | "feverwatch",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  it("the Fever Watch only contains while a post is built and staffed", () => {
    ...
    s.buildings.push(
      mk("feverwatch", s.terrain.landing.x + 3, s.terrain.landing.y, {
        jobs: 2,
      }),
    );
    s.totalJobs = 2;
  ...
  it("sustained bad conditions (crowded + smoggy + unhealthy) grow an outbreak from nothing", () => {
    ...
    s.buildings.push(
      mk("mine", L.x + 2, L.y + 1, { jobs: 6, materialsGen: 5 }),
    ); // smog over the homes, no scrubber
...
describe("Spec 027 — The Housewares Market: manufactured goods reach the home", () => {
  const mk = (
    kind: "habitat" | "water" | "depot" | "clinic" | "theatre" | "market",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
...
describe("Spec 028 — The Ward Post: idleness and hardship breed unrest", () => {
  ...
  it("a Ward Post keeps order only while a post is built and staffed", () => {
    ...
    s.buildings.push(
      mk("ward", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 2 }),
    );
    s.totalJobs = 2;
    s.colonists = 0;
...
describe("Spec 029 — The Pay Office: the colony pays the hands that hold it up", () => {
  ...
  it("wages are paid only while a Pay Office is built and staffed", () => {
    ...
    s.buildings.push(
      mk("payoffice", s.terrain.landing.x + 3, s.terrain.landing.y, {
        jobs: 2,
      }),
    );
    s.totalJobs = 2;
  ...
  it("payroll is zero without an office and scales with the wage rate with one", () => {
    ...
    s.wageRate = "generous";
    expect(payrollPerDay(s)).toBeGreaterThan(
      10 * COLONY.build.wagePerWorkerPerDay,
    ); // generous costs more
    s.wageRate = "low";
    expect(payrollPerDay(s)).toBeLessThan(
      10 * COLONY.build.wagePerWorkerPerDay,
    ); // low costs less
  });

  const treasuryAfterDay = (
  ...
  it("a generous wage drains more treasury per day than a low one", () => {
    expect(treasuryAfterDay("generous", true)).toBeLessThan(
      treasuryAfterDay("low", true),
    ); // bigger payroll → less left
  });

  it("without a Pay Office the wage rate is inert — the treasury is identical", () => {
    expect(treasuryAfterDay("generous", false)).toBe(
      treasuryAfterDay("low", false),
    ); // no office → no payroll
  });
  ...
  it("a generous wage draws more settlers than a low one", () => {
    ...
    expect(immigrationUnder("generous")).toBeGreaterThan(
      immigrationUnder("low"),
    ); // better pay draws people
  });
});
...
describe("Spec 030 — The Civic Feast: the council buys one honest cheer", () => {
  ...
  it("a feast can only be hosted from a built, staffed Feast Deck", () => {
    ...
    s.buildings.push(
      mk("feast", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 2 }),
    );
    s.totalJobs = 2;
    s.colonists = 0;
...
describe("Spec 031 — The Skyflax Line: a second resource and chain", () => {
  ...
  it("a Flax Skimmer Dock gathers skyflax fibre while staffed", () => {
    ...
    s.totalJobs = 6;
    s.buildings.push(
      mk("skimmer", s.terrain.landing.x + 3, s.terrain.landing.y, {
        jobs: 6,
        fibreGen: 5,
      }),
    );
  ...
  it("a Weavery weaves fibre into linen and halts when fibre runs out", () => {
    const run = (fibre: number) => {
      ...
      s.totalJobs = 6;
      s.buildings.push(
        mk("weavery", s.terrain.landing.x + 3, s.terrain.landing.y, {
          jobs: 6,
        }),
      );
      s.fibre = fibre;
  ...
  it("clinics draw linen as bandage cloth, more during a fever outbreak", () => {
    const drained = (outbreak: number) => {
      ...
      s.totalJobs = 6;
      s.buildings.push(
        mk("clinic", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 2 }),
      );
      s.linen = 100;
      s.outbreak = outbreak;
      for (let i = 0; i < 100; i++) stepBuild(s, sim.rng, 10);
...
describe("Spec 032 — The Kookerverse Liaison Office: standing and Civic Requests", () => {
  ...
  it("the Kookerverse only deals with a built, staffed Liaison Office", () => {
    ...
    s.buildings.push(
      mk("liaison", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 2 }),
    );
    s.totalJobs = 2;
    s.colonists = 0;
  ...
  it("high standing draws more settlers than low standing", () => {
    const run = (standing: number) => {
      ...
      const s = sim.state;
      s.buildings.push(
        mk("liaison", s.terrain.landing.x + 3, s.terrain.landing.y, {
          jobs: 2,
        }),
      );
      s.buildings.push(
...
describe("Spec 033 — The Horizon Spire: a monument the colony builds toward", () => {
  ...
  it("a finished Spire draws more settlers and calms unrest", () => {
    const immigration = (complete: boolean) => {
      ...
      if (complete) s.spireStage = 4;
      s.buildings.push(
        mk("habitat", s.terrain.landing.x + 3, s.terrain.landing.y, {
          residents: 9,
        }),
      );
      s.colonists = 4;
...
describe("Spec 034 — The Stormwatch Shelter: weathering the Cloudsea Fronts", () => {
  ...
  it("the Stormwatch braces only while a shelter is built and staffed", () => {
    ...
    s.buildings.push(
      mk("stormwatch", s.terrain.landing.x + 3, s.terrain.landing.y, {
        jobs: 2,
      }),
    );
    s.totalJobs = 2;
  ...
  it("a staffed Stormwatch braces the colony and cuts the damage sharply", () => {
    const lost = (braced: boolean) => {
      ...
      if (braced)
        s.buildings.push(
          mk("stormwatch", s.terrain.landing.x + 5, s.terrain.landing.y, {
            jobs: 2,
          }),
        );
      s.frontTimer = 5;
...
describe("Spec 022 — Maintenance Sheds: working buildings wear and need upkeep", () => {
  ...
  it("a fresh building under the healthy threshold pays no penalty", () => {
    ...
    expect(maintenanceStatus(s).worstWear).toBeLessThan(
      COLONY.build.wearHealthyThreshold,
    );
  ...
  it("an uncovered mine wears down and produces less than one kept under a Maintenance Shed", () => {
    const run = (shed: boolean) => {
      ...
      if (shed)
        s.buildings.push(
          mk("maintshed", mx + 1, my, { jobs: COLONY.build.maintShedWorkers }),
        );
  ...
  it("a Maintenance Shed repairs a worn building back below the healthy threshold", () => {
    ...
    for (let i = 0; i < 200; i++) stepBuild(s, sim.rng, 10);
    expect(s.buildings[0]!.wear!).toBeLessThan(
      COLONY.build.wearHealthyThreshold,
    ); // repaired back to healthy
...
describe("Spec 035 — The Founders Hall: the Living Roster of the people who built Landing One", () => {
  const mk = (
    kind: "hall" | "liaison" | "habitat" | "mine",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  it("the Living Roster is seated only while a Founders Hall is built and staffed", () => {
    ...
    s.buildings.push(
      mk("hall", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 3 }),
    );
    s.totalJobs = 3;
    s.colonists = 0;
  ...
  it("a seated Roster makes each fulfilled Civic Request earn a little more standing", () => {
    const gain = (withHall: boolean) => {
      ...
      if (withHall)
        s.buildings.push(
          mk("hall", s.terrain.landing.x + 4, s.terrain.landing.y, { jobs: 3 }),
        );
      s.standing = 0.5;
      s.components = 100;
  ...
  it("a seated Roster eases unrest a touch (vs an identical colony with no hall)", () => {
    const run = (withHall: boolean) => {
      ...
      s.buildings.push(
        mk("mine", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 6 }),
      );
      if (withHall)
        s.buildings.push(
          mk("hall", s.terrain.landing.x + 4, s.terrain.landing.y, { jobs: 3 }),
  ...
  it("a seated Roster draws settlers a little faster (vs an identical colony with no hall)", () => {
    const run = (withHall: boolean) => {
      ...
      ); // ample headroom — never saturates
      s.buildings.push(
        mk("mine", s.terrain.landing.x + 5, s.terrain.landing.y, { jobs: 6 }),
      ); // employ everyone → no idle-unrest confound
      if (withHall)
        s.buildings.push(
          mk("hall", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 3 }),
        );
  ...
  it("the Courier reports the founders by name once the Roster is seated", () => {
    ...
    s.colonists = 20;
    s.totalJobs = 4;
    expect(colonyHeadlines(s).some((h) => h.includes("Living Roster"))).toBe(
      false,
    ); // no hall → founders stay signatures
    s.buildings.push(
      mk("hall", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 3 }),
    );
    const lines = colonyHeadlines(s);
    ...
    expect(
      lines.some((h) =>
        FOUNDERS.some((f) => h.includes(f.name) && h.includes(f.role)),
      ),
    ).toBe(true); // a founder named with their post
  });
...
describe("Spec 036 — The Skybridge Import Office: buying what the colony cannot make in time", () => {
  ...
  it("the Import Office only buys while built and staffed", () => {
    ...
    s.buildings.push(
      mk("import", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 3 }),
    );
    s.totalJobs = 3;
    s.colonists = 0;
  ...
  it("imports cost a premium over the Exchange sell price", () => {
    expect(COLONY.build.importPrice.components).toBeGreaterThan(
      COLONY.build.tradeComponentPrice,
    );
    expect(COLONY.build.importPrice.food).toBeGreaterThan(
      COLONY.build.tradeFoodPrice,
    );
    expect(COLONY.build.importPrice.reels).toBeGreaterThan(
      COLONY.build.reelPrice,
    );
  });
  ...
  it("a staffed Import Office spends treasury to land the order good — at the premium", () => {
    ...
    expect(t0 - s.treasury).toBeGreaterThan(
      s.components * COLONY.build.tradeComponentPrice,
    ); // paid MORE than the Exchange would have paid to sell them (the premium holds)
  });
...
describe("Spec 037 — The Mooring Shrine: faith, solace, and the first reason to call it home", () => {
  ...
  it("Solace reaches homes only from a built, staffed shrine — and dims without linen", () => {
    const sim = new ColonySim(7);
    const s = sim.state;
    const lx = s.terrain.landing.x,
      ly = s.terrain.landing.y;
...
describe("Spec 039 — Treasury Arrears: giving an empty treasury teeth", () => {
  ...
  it("a Comptroller Office is the debt desk only when built and staffed", () => {
    ...
    s.buildings.push(
      mk("comptroller", s.terrain.landing.x + 3, s.terrain.landing.y, {
        jobs: 3,
      }),
    );
    s.totalJobs = 3;
...
describe("Spec 038 — The Roster Office: making a labour shortage a choice", () => {
  ...
  it("labour priority bites only with a built, staffed Roster Office", () => {
    ...
    s.buildings.push(
      mk("roster", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 3 }),
    );
    s.totalJobs = 3;
    s.colonists = 0;
  ...
  it("under a shortage, Essentials-first fills Food before Industry — and Industry-first reverses it", () => {
    ...
    expect(sectorStaffing(s, "food")).toBeGreaterThan(
      sectorStaffing(s, "industry"),
    );
    ...
    expect(sectorStaffing(s, "industry")).toBeGreaterThan(
      sectorStaffing(s, "food"),
    );
  ...
  it("priority redistributes labour but conserves the total (essentials)", () => {
    ...
    const placed =
      sectorStaffing(s, "food") * 6 +
      sectorStaffing(s, "industry") * 6 +
      sectorStaffing(s, "civic") * 3;
...
describe("Spec 041 — Departure Pressure: the colony can lose people, not just gain them", () => {
  ...
  const poweredHomes = (residents: number, colonists: number) => {
    ...
    const s = sim.state;
    s.buildings.push(
      mk("habitat", s.terrain.landing.x, s.terrain.landing.y, { residents }),
    );
    s.colonists = colonists;
    s.totalJobs = 0;
  ...
  it("distress is zero with no homes, and rises when homes go unserved", () => {
    ...
    s.buildings.push(
      mk("habitat", s.terrain.landing.x, s.terrain.landing.y, { residents: 6 }),
    );
...
describe("Spec 042 — The Little Schoolroom: the colony learns its letters", () => {
  const mk = (
    kind: "school" | "habitat" | "academy" | "mine",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
...
describe("Spec 040 — The Census Hall: a colony-wide Prosperity Rank", () => {
  ...
  it("the colony reads its Prosperity only with a built, staffed Census Hall", () => {
    ...
    s.buildings.push(
      mk("census", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 3 }),
    );
    s.totalJobs = 3;
    s.colonists = 0;
  ...
  it("Prosperity is a weighted blend of the colony signals", () => {
    ...
    const expected =
      w.prospEmploymentWeight + w.prospStandingWeight + w.prospSolvencyWeight;
  ...
  it("with no Census Hall, Prosperity is dark and the colony is unchanged (inert)", () => {
    ...
    const s = sim.state;
    s.buildings.push(
      mk("habitat", s.terrain.landing.x, s.terrain.landing.y, { residents: 8 }),
    );
    s.colonists = 12;
    s.totalJobs = 10;
    s.standing = 0.9;
...
describe("Spec 044 — Skybound Folios: the colony signature finished export", () => {
  ...
  it("a staffed Folio House binds reels + linen 1:1 into folios", () => {
    ...
    s.totalJobs = 4;
    s.buildings.push(
      mk("folio", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 4 }),
    );
    s.reels = 50;
    s.linen = 50;
    s.folios = 0;
  ...
  it("the Exchange sells folios at a premium above reels", () => {
    ...
    s.totalJobs = 4;
    s.buildings.push(
      mk("exchange", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 2 }),
    );
    s.folios = 50;
    s.reels = 0;
    s.components = 0;
...
describe("Spec 045 — The Wind-Shear Turbine Mast: power that scales", () => {
  ...
  it("a staffed Turbine Mast adds power, scaled by staffing; none without a mast", () => {
    ...
    s.buildings.push(
      mk("turbine", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 4 }),
    );
    s.totalJobs = 4;
  ...
  it("staffed Turbine Masts raise the supply and ease a brownout", () => {
    ...
    for (let i = 0; i < 10; i++)
      s.buildings.push(
        mk("turbine", s.terrain.landing.x + i, s.terrain.landing.y + 5, {
          jobs: 4,
        }),
      );
...
describe("Spec 046 — Stored Water: the sky can deny the tanks", () => {
  ...
  it("water is full coverage with no cistern, and scales with the tank once one stands", () => {
    ...
    s.buildings.push(
      mk("cistern", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 3 }),
    );
    s.water = 0; // bone dry
...
describe("Spec 047 — The Tool Crib: bare hands do not mine forever", () => {
  ...
  it("tools are full output with no crib, and scale with the rack once a crib stands", () => {
    ...
    s.buildings.push(
      mk("toolcrib", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 2 }),
    );
  ...
  it("a dry rack weakens tooled output toward the floor; a full rack leaves it unchanged", () => {
    const runMine = (mode: "nocrib" | "full" | "dry") => {
      ...
      if (mode !== "nocrib")
        s.buildings.push(mk("toolcrib", lx + 2, ly, { jobs: 2 }));
      ...
      s.materials = 0;
      if (mode === "full") {
        s.tools = toolStockCap(s);
        s.components = 9999;
      } // crib stays fed → rack stays full
      if (mode === "dry") {
        s.tools = 0;
        s.components = 0;
      } // crib starved → rack stays at 0
      for (let i = 0; i < 6; i++) stepBuild(s, sim.rng, 60);
      return s.materials;
    };
    const base = runMine("nocrib"),
      full = runMine("full"),
      dry = runMine("dry");
...
describe("Spec 048 — The Seed Loft: food should not grow from bare deck-plating", () => {
  const mk = (
    kind: "seedloft" | "greenhouse" | "cistern",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  it("food is full yield with no loft, and scales with the bin once a loft stands", () => {
    ...
    s.buildings.push(
      mk("seedloft", s.terrain.landing.x + 3, s.terrain.landing.y, { jobs: 2 }),
    );
  ...
  it("a dry bin cuts skyfarm yield toward the floor; a full bin leaves it unchanged", () => {
    const runFarms = (mode: "noloft" | "full" | "dry") => {
      ...
      for (let g = 0; g < 20; g++)
        s.buildings.push(
          mk("greenhouse", lx + (g % 10), ly + 4 + Math.floor(g / 10), {
            jobs: COLONY.build.greenhouseWorkers,
          }),
        ); // 20 skyfarms, none near a Water Hub (uniform boost = 1)
      if (mode !== "noloft")
        s.buildings.push(mk("seedloft", lx, ly, { jobs: 2 }));
...
describe("Spec 049 — Settler Confidence: word travels faster than skyships", () => {
  const mk = (
    kind: "habitat" | "water" | "comptroller",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  it("a fed, watered, solvent, orderly colony sits at the plateau (immigration unchanged)", () => {
    ...
    s.food = 100; // fed
    s.colonists = 8;
    expect(settlerConfidence(s)).toBeGreaterThanOrEqual(
      COLONY.build.confPlateau,
    );
...
describe("Spec 050 — Household Births: a colony that can grow its own", () => {
  ...
  const water = (x: number, y: number): ColonyBuilding => ({
    id: x * 1000 + y + 9,
    x,
    y,
    artifact: {
  ...
  it("children mature into colonists on a vacancy, and the pool holds when housing is full", () => {
    ...
    const sim2 = new ColonySim(7);
    const s2 = sim2.state;
    const lx2 = s2.terrain.landing.x,
      ly2 = s2.terrain.landing.y;
...
describe("Spec 051 — The Survey Camp: the colony can claim new ground", () => {
  ...
  it("the effective radius grows one ring per claim, capped at maxClaims", () => {
    ...
    expect(effectiveBuildRadius(s)).toBe(
      COLONY.build.maxBlockRadius + COLONY.build.maxClaims,
    ); // capped at the island edge
...
describe("Spec 052 — The Vein Ledger: ore runs out, so the colony must spread its diggings", () => {
  const mine = (
    ...
    y: number,
    vein?: number,
    gen = COLONY.build.mineOutputPerDay,
  ): ColonyBuilding => {
    const b: ColonyBuilding = {
      id: x * 1000 + y,
      x,
  ...
  it("output fades by bands as the vein runs down", () => {
    ...
    const at = (frac: number) => {
      m.vein = frac * COLONY.build.veinLifeDays;
      return veinFactor(m);
    };
  ...
  it("a staffed, producing mine spends its vein; an incident-stalled one holds", () => {
    ...
    const s = sim.state;
    const m = mine(
      s.terrain.landing.x + 4,
      s.terrain.landing.y,
      COLONY.build.veinLifeDays,
    );
    s.buildings.push(m);
    s.colonists = 8;
    s.totalJobs = 3;
    ...
    const s2 = sim2.state;
    const m2 = mine(
      s2.terrain.landing.x + 4,
      s2.terrain.landing.y,
      COLONY.build.veinLifeDays,
    );
  ...
  it("a fresh-vein mine out-digs a thin-vein one over the same run", () => {
    const produced = (veinFrac: number) => {
      ...
      s.buildings.push(
        mine(
          s.terrain.landing.x + 4,
          s.terrain.landing.y,
          veinFrac * COLONY.build.veinLifeDays,
        ),
      );
      s.colonists = 8;
      s.totalJobs = 3;
    ...
    const status = veinStatus({
      buildings: [
        mine(0, 0, COLONY.build.veinLifeDays),
        mine(1, 1, 0.3 * COLONY.build.veinLifeDays),
      ],
    } as any);
...
describe("Spec 054 — Mild Seasons: the almanac makes the year turn", () => {
  const mk = (
    kind: "calendar" | "greenhouse" | "water",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  it("with a Calendar Office, the season factor follows the month bands", () => {
    ...
    const s = sim.state;
    s.buildings.push(
      mk("calendar", s.terrain.landing.x + 4, s.terrain.landing.y, { jobs: 1 }),
    );
    const at = (day: number) => {
      s.clock.day = day;
      return seasonFactor(s);
    };
...
describe("Spec 055 — The Long Ledger: a life has a span, and the colony renews", () => {
  ...
  it("passings are hard-capped by renewal, by a small fraction, and never below the founding crew", () => {
    ...
    const col0 = s.colonists;
    ledgerStep(s);
    expect(s.lastPassings).toBeLessThanOrEqual(
      COLONY.build.renewalCapFraction * 6 + 1e-9,
    ); // <= half the year's renewal
    expect(s.lastPassings).toBeLessThanOrEqual(
      COLONY.build.maxPassFraction * col0 + 1e-9,
    ); // <= the small ceiling
...
describe("Spec 056 — Rimfish: a second food from the cloudsea rim", () => {
  ...
  it("a staffed Cloudsea Net Dock nets rimfish over several days, capped at storage", () => {
    ...
    const s = sim.state;
    s.buildings.push(
      mk("netdock", s.terrain.landing.x + 4, s.terrain.landing.y, {
...
describe("Spec 057 — Seasonal Solar Angling: the sun keeps the calendar too", () => {
  ...
  it("wind-shear turbines are unaffected by the season (wind has no season)", () => {
    ...
    s.buildings.push(
      mk("turbine", s.terrain.landing.x + 5, s.terrain.landing.y, {
...
describe("Spec 058 — Household Waste: a colony that does not handle its filth sickens", () => {
  ...
  it("a small colony over a short run stays under the harmless line (inert — no effect)", () => {
    ...
    const lx = s.terrain.landing.x,
      ly = s.terrain.landing.y;
    for (let h = 0; h < 3; h++)
      s.buildings.push(mk("habitat", lx + h, ly, { residents: 4 }));
    s.colonists = 8;
    for (let i = 0; i < 30; i++) stepBuild(s, sim.rng, 60);
  ...
  it("an occupied colony with no Sanitation Post slowly accumulates waste", () => {
    ...
    for (let h = 0; h < 20; h++)
      s.buildings.push(
        mk("habitat", lx + (h % 10), ly + Math.floor(h / 10), { residents: 4 }),
      );
    s.colonists = 8;
    const w0 = s.waste;
    for (let i = 0; i < 200; i++) stepBuild(s, sim.rng, 60);
  ...
  it("waste harms in bands: desirability slips above 0.25, fever breeds above 0.50 — nothing below", () => {
    ...
    const feverGain = (waste: number) => {
      ...
      const s2 = sub.state;
      s2.buildings.push(
        mk("habitat", s2.terrain.landing.x, s2.terrain.landing.y, {
          residents: 4,
        }),
      );
      s2.waste = waste;
  ...
  it("a staffed Sanitation Post clears the waste meter over time", () => {
    ...
    s.buildings.push(
      mk("sanitation", lx, ly + 5, { jobs: COLONY.build.sanitationWorkers }),
    );
    s.colonists = 8;
    s.totalJobs = 4; // staffed
    s.waste = 0.5;
...
describe("Spec 059 — The Watch Nook: a rich colony keeps honest lamps burning", () => {
  ...
  it("a poor or small colony loses nothing to theft (inert below the floors)", () => {
    ...
    s.power.batteryWh = s.power.batteryCapWh;
    s.colonists = 30;
    s.treasury = 400; // rich enough in people but the coffers are below the floor
    expect(securityStatus(s).active).toBe(false);
    s.colonists = 10;
    s.treasury = 1000; // rich coffers but too few people to tempt theft
    expect(securityStatus(s).active).toBe(false);
  });
  ...
  it("a staffed Watch Nook cuts theft, and two stop it entirely", () => {
    const stolen = (nooks: number) => {
      ...
      s.power.batteryWh = s.power.batteryCapWh;
      s.unrest = 0;
      for (let n = 0; n < nooks; n++)
        s.buildings.push(mk("watchnook", lx + n, ly, { jobs: 2 }));
      const t0 = s.treasury;
      for (let i = 0; i < 300; i++) stepBuild(s, sim.rng, 60);
...
describe("Spec 060 — The Variety Ration Counter: two foods finally beat one", () => {
  ...
  it("the bonus scales with coverage and never dips below the no-counter baseline", () => {
    ...
    expect(varietyDesirabilityFactor(s)).toBeCloseTo(
      1 + COLONY.build.varietyDesirabilityBonus * 0.5,
      5,
    ); // half coverage → half the lift
    expect(dietVarietyStatus(s).bonus).toBeCloseTo(
      COLONY.build.varietyDesirabilityBonus * 0.5,
      5,
    );
...
describe("Spec 061 — Rimfish Drying Racks: bank the second food for a lean season", () => {
  const mk = (
    kind: "dryrack" | "rationvar" | "storehouse",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  it("drying loses weight — dried banked never exceeds the loss-adjusted fresh consumed", () => {
    ...
    s.power.batteryWh = s.power.batteryCapWh;
    for (let n = 0; n < 3; n++)
      s.buildings.push(mk("storehouse", lx + 2 + n, ly, {})); // raise the caps so nothing clamps
    s.buildings.push(mk("dryrack", lx, ly, { jobs: 2 }));
    s.rimfish = 300;
    s.driedFish = 0;
    ...
    for (let i = 0; i < 30; i++) {
      s.food = 500;
      stepBuild(s, sim.rng, 60);
    } // do NOT top rimfish — let it draw down
    const rimfishConsumed = rim0 - (s.rimfish ?? 0);
    const driedProduced = s.driedFish ?? 0;
    const ratio =
      COLONY.build.dryRackOutputPerDay / COLONY.build.dryRackRimfishPerDay;
    expect(COLONY.build.dryRackOutputPerDay).toBeLessThan(
      COLONY.build.dryRackRimfishPerDay,
    ); // the loss is real by design (8 per 12)
  ...
  it("the dried reserve keeps fish (and a varied diet) on the table when the fresh catch runs out", () => {
    ...
    s.power.batteryWh = s.power.batteryCapWh;
    for (let n = 0; n < 2; n++)
      s.buildings.push(mk("storehouse", lx + 2 + n, ly, {})); // headroom so the reserve lasts
    s.buildings.push(mk("dryrack", lx, ly, { jobs: 2 }));
...
describe("Spec 062 — The Labour Registry Desk: idle hands finally show in the books", () => {
  ...
  it("inert without a desk — heavy unemployment does not drag Prosperity", () => {
    ...
    s.totalJobs = 30;
    s.treasury = 5000;
    s.standing = 1; // 70% idle, but no Registry
  ...
  it("chronic idleness drags the Prosperity Rank a step, then two", () => {
    ...
    s.totalJobs = 50;
    s.treasury = 5000;
    s.standing = 1; // 50% unemployment (above both lines)
...
describe("Spec 063 — The Planter Square: a deliberate patch of beauty", () => {
  ...
  it("a Bloom lifts a served home, the near ring more than the far ring", () => {
    ...
    const lx = s.terrain.landing.x,
      ly = s.terrain.landing.y;
    const planter = mk("planter", lx, ly, { jobs: 1 });
    planter.tend = COLONY.build.planterBloomCap; // Bloom it directly
    const near = mk("habitat", lx + 2, ly);
    const far = mk("habitat", lx + 6, ly);
    const control = mk("habitat", lx + 40, ly);
    ...
    const lNear = homeLiveability(s, near),
      lFar = homeLiveability(s, far),
      lControl = homeLiveability(s, control);
  ...
  it("a home gathers at most the cap from a wall of Planters, and never drops below baseline untended", () => {
    ...
    for (let i = 0; i < 5; i++) {
      const p = mk("planter", lx + 1 + i, ly + 1, { jobs: 1 });
      p.tend = COLONY.build.planterBloomCap;
      s.buildings.push(p);
    }
    expect(planterLiveabilityBoost(s, home, 1)).toBeCloseTo(
      COLONY.build.planterMaxBonus * COLONY.build.planterLiveabilityPerPoint,
      6,
    ); // capped at +12 points
...
describe("Spec 064 — The Market Stall: local custom returns a little coin", () => {
  ...
  it("a staffed stall sells surplus wares to paid colonists for coin", () => {
    ...
    s.treasury = 1000;
    s.linen = 100;
    s.folios = 5; // folios below the reserve → it sells linen
    const t0 = s.treasury,
      l0 = s.linen;
    for (let i = 0; i < 2; i++) {
      s.colonists = 40;
    ...
    expect(s.treasury - t0).toBe(
      (l0 - s.linen) * COLONY.build.stallCoinPerSale,
    ); // every coin matches a ware sold at the set price
  });
  ...
  it("the stall never sells linen or folios below the reserve", () => {
    ...
    s.totalJobs = 8;
    s.treasury = 1000;
    s.linen = COLONY.build.stallReserve;
    s.folios = COLONY.build.stallReserve; // exactly at the reserve → nothing to sell
    const t0 = s.treasury;
    for (let i = 0; i < 10; i++) {
      s.colonists = 40;
      s.totalJobs = 8;
  ...
  it("wages in deep arrears close the stalls; solvency reopens them", () => {
    ...
    const lx = s.terrain.landing.x,
      ly = s.terrain.landing.y;
    s.buildings.push(
      mk("stall", lx, ly, { jobs: 2 }),
      mk("comptroller", lx + 1, ly, { jobs: 2 }),
    ); // a Comptroller lets the treasury run negative
    s.colonists = 40;
    s.totalJobs = 8;
    s.linen = 100;
    for (let i = 0; i < 3; i++) {
      s.colonists = 40;
      s.totalJobs = 8;
      s.treasury = -COLONY.build.debtCeiling;
      stepBuild(s, sim.rng, 24 * 60);
    } // deep arrears strain
  ...
  it("the stall only ever adds coin — the treasury never falls from selling", () => {
    ...
    s.treasury = 1000;
    s.linen = 100;
    s.folios = 100; // food left at 0 so no other treasury effect stirs
    for (let i = 0; i < 10; i++) {
      s.colonists = 40;
      s.totalJobs = 8;
      const before = s.treasury;
      stepBuild(s, sim.rng, 24 * 60);
      expect(s.treasury).toBeGreaterThanOrEqual(before);
    }
...
describe("Spec 065 — Deck Fires and the Fire-Watch Post: a blaze that eats the floor", () => {
  const mk = (
    kind: "firewatch" | "workshop" | "habitat",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
    ...
    artifact: Object.assign(
      {
        ...
        height: 1,
        residents: 0,
        jobs: kind === "firewatch" ? 3 : kind === "workshop" ? 10 : 0,
        powerLoad: 0,
        powerGen: 0,
        buildTimeMin: 1,
        cost: 0,
  ...
  const forceBrownout = (s: ColonySim["state"]) => {
    s.power.loadW = 100000;
    s.power.batteryWh = 0;
    s.powerGen = 0;
    s.power.solarW = 0;
  ...
  it("inert without a Post — stressed buildings never carry fire risk or ignite", () => {
    ...
    const lx = s.terrain.landing.x,
      ly = s.terrain.landing.y;
    const ws = mk("workshop", lx, ly);
    ws.wear = 1; // worn + hot work, but no Fire-Watch anywhere
    s.buildings.push(ws);
    s.colonists = 20;
    s.totalJobs = 8;
    forceBrownout(s);
  ...
  it("a staffed, watered Post puts a Spark out", () => {
    ...
    s.buildings.push(mk("firewatch", lx, ly, { jobs: 3 }));
    const ws = mk("workshop", lx + 2, ly);
    ws.fire = 1; // a fresh Spark
    s.buildings.push(ws);
    s.colonists = 20;
    s.totalJobs = 8;
    s.water = 200;
    s.powerGen = 100;
    s.power.batteryWh = s.power.batteryCapWh; // staffed + watered
    for (let i = 0; i < 3; i++) {
      s.colonists = 20;
      s.totalJobs = 8;
      s.water = 200;
  ...
  it("an unfought fire spreads to a deck-neighbour, then destroys the building", () => {
    ...
    const a = mk("workshop", lx, ly);
    a.fire = COLONY.build.fireSpreadAt - 30; // a Blaze about to spread
  ...
  it("a dry Post cannot suppress, and destruction never drives a stockpile below zero", () => {
    ...
    s.buildings.push(mk("firewatch", lx, ly, { jobs: 3 }));
    const ws = mk("workshop", lx + 2, ly);
    ws.fire = 100; // a young fire
    s.buildings.push(ws);
    s.colonists = 20;
    s.totalJobs = 8;
    s.water = 0;
    s.materials = 0;
    s.components = 0; // dry tanks, empty stores
...
describe("Spec 066 — The Greywater Reclaimer: get some of our own water back", () => {
  const mk = (
    kind: "reclaimer" | "cistern" | "habitat",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  const setup = (withReclaimer: boolean) => {
    ...
    const lx = s.terrain.landing.x,
      ly = s.terrain.landing.y;
    for (let i = 0; i < 8; i++)
      s.buildings.push(mk("cistern", lx + i, ly + 4, { jobs: 2 }));
  ...
  it("inert without a Reclaimer; a staffed, powered one returns water to the tanks", () => {
    ...
    for (const c of [without, withR]) {
      c.s.colonists = 20;
      c.s.totalJobs = 6;
      stepBuild(c.s, c.sim.rng, 24 * 60);
    }
  ...
  it("a brownout halves the return", () => {
    ...
    s.power.loadW = 100000;
    s.power.batteryWh = 0;
    s.power.solarW = 1; // force a brownout (keep a trickle of solar so it is not power-dead)
    expect(inBrownout(s)).toBe(true);
...
describe("Spec 067 — The Highsun Lantern Supper: a year the people look forward to", () => {
  ...
  it("a well-stocked colony throws a full supper and earns Lantern Cheer", () => {
    ...
    const saved = s.festivalCheer;
    s.festivalCheer = 0;
    const confWithout = settlerConfidence(s);
    s.festivalCheer = saved;
  ...
  it("a thin supper helps less, and a failed one is not held at all", () => {
    ...
    expect(festivalStatus(partial.s).bonus).toBe(
      COLONY.build.festPartialCheerBonus,
    ); // the modest cheer
    ...
    fail.s.colonists = 60;
    fail.s.materials = COLONY.build.festMaterialsPerTable;
  ...
  it("the Lantern Cheer decays back to baseline", () => {
    ...
    for (let i = 0; i < COLONY.build.festFullCheerDays + 2; i++)
      stepBuild(s, sim.rng, 24 * 60); // run out the 30-day window
...
describe("Spec 068 — The Fungus Cellar: a third food the dark decks can grow", () => {
  ...
  it("a staffed, watered Cellar grows duskcap, even in a brownout", () => {
    ...
    const lx = s.terrain.landing.x,
      ly = s.terrain.landing.y;
    s.buildings.push(
      mk("cellar", lx, ly, { jobs: 3, duskcapGen: COLONY.build.duskcapPerDay }),
    );
    s.colonists = 20;
    s.totalJobs = 6;
    s.powerGen = 100;
  ...
  it("duskcap spares the other foods — the homes eat it for the protein course", () => {
    const make = (dusk: number) => {
      ...
      s.duskcap = dusk;
      s.powerGen = 100;
      s.power.batteryWh = s.power.batteryCapWh; // food below the 160 cap so no clamp confound
      stepBuild(s, sim.rng, 24 * 60); // one day's meals (no fish on hand)
      return s;
    };
    const withDusk = make(100),
      withoutDusk = make(0);
...
describe("Spec 069 — The Steam Bathhouse: hygiene the colony can wash in", () => {
  const mk = (
    kind: "bathhouse" | "habitat" | "mine" | "clinic",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  it("a staffed, watered Bathhouse raises the colony hygiene", () => {
    ...
    const lx = s.terrain.landing.x,
      ly = s.terrain.landing.y;
    s.buildings.push(
      mk("bathhouse", lx, ly, { jobs: COLONY.build.bathWorkers }),
    );
    s.colonists = 20;
    s.totalJobs = COLONY.build.bathWorkers;
    s.water = 100;
    s.powerGen = 100;
    s.power.batteryWh = s.power.batteryCapWh;
  ...
  it("a clean colony slows how fast the fever takes hold", () => {
    const run = (withBath: boolean) => {
      ...
      s.buildings.push(
        mk("mine", L.x + 2, L.y + 1, { jobs: 6, materialsGen: 5 }),
      ); // smog over the homes, no scrubber, no clinic
      let jobs = 6;
      if (withBath) {
        s.buildings.push(
          mk("bathhouse", L.x + 4, L.y, { jobs: COLONY.build.bathWorkers }),
        );
      ...
      s.powerGen = 100;
      s.power.batteryWh = s.power.batteryCapWh; // no brownout
      for (let i = 0; i < 400; i++) {
        s.colonists = 20;
        s.totalJobs = jobs;
        s.water = 200;
  ...
  it("hygiene is clamped to [0,1] and the fever it damps never goes negative", () => {
    ...
    for (let i = 0; i < 5; i++)
      s.buildings.push(
        mk("bathhouse", lx + i, ly, { jobs: COLONY.build.bathWorkers }),
      ); // five baths cover 250 >> 20 colonists
    s.colonists = 20;
    s.totalJobs = 5 * COLONY.build.bathWorkers;
    s.water = 1000;
    s.powerGen = 100;
    s.power.batteryWh = s.power.batteryCapWh;
...
describe("Spec 070 — The Clean-Home Standing: a washed colony draws settlers and lifts its homes", () => {
  ...
  it("the clean-home draw pulls more settlers in a day than a grimy colony", () => {
    const draw = (withBath: boolean) => {
      ...
      const L = s.terrain.landing;
      for (let h = 0; h < 20; h++) {
        const b = mk("habitat", L.x + (h % 10), L.y + Math.floor(h / 10), {
          residents: 4,
        });
        b.tier = 1;
        s.buildings.push(b);
      } // plenty of vacant housing in both
      if (withBath)
        s.buildings.push(
          mk("bathhouse", L.x, L.y + 4, { jobs: COLONY.build.bathWorkers }),
        );
      s.colonists = 10;
      s.totalJobs = withBath ? COLONY.build.bathWorkers : 0;
      s.food = 500;
      s.water = 100;
      s.powerGen = 100;
      s.power.batteryWh = s.power.batteryCapWh;
  ...
  it("both factors are bounded by their small ceilings however high hygiene climbs", () => {
    ...
    s.power.batteryWh = s.power.batteryCapWh;
    expect(hygieneLevel(s)).toBeCloseTo(1, 5);
    expect(hygieneDesirabilityFactor(s)).toBeCloseTo(
      1 + COLONY.build.hygieneDesirabilityGain,
      5,
    ); // exactly the ceiling, not beyond
    expect(hygieneEvolutionFactor(s)).toBeCloseTo(
      1 - COLONY.build.hygieneEvolutionGain,
      5,
    );
  });
});
...
describe("Spec 071 — The Folio Library: the colony keeps some of its own books", () => {
  ...
  it("a staffed, folio-stocked Library cultures the homes (a culture source on its own)", () => {
    const cultured = (kind: "none" | "library" | "theatre") => {
      ...
      if (kind === "library")
        s.buildings.push(
          mk("library", L.x, L.y, { jobs: COLONY.build.libraryWorkers }),
        );
      if (kind === "theatre")
        s.buildings.push(mk("theatre", L.x, L.y, { jobs: 2 }));
      s.colonists = 20;
      s.totalJobs = COLONY.build.libraryWorkers;
      s.folios = 10;
      s.reels = 10;
  ...
  it("reel-free resilience — a stocked Library keeps culture when the theatres go dark", () => {
    ...
    s.colonists = 20;
    s.totalJobs = COLONY.build.libraryWorkers + 2;
    s.reels = 0;
    s.folios = 10; // theatres dark (no reels), shelves stocked
  ...
  it("a Library with no folio supply drains to bare over time, then lends nothing (no error)", () => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      s.totalJobs = COLONY.build.libraryWorkers;
      stepBuild(s, sim.rng, 24 * 60);
    } // a month, no Folio House to refill
...
describe("Spec 072 — The Skydeck Gallery: the colony renown becomes coin", () => {
  const mk = (
    kind: "habitat" | "water" | "depot" | "clinic" | "theatre" | "gallery",
    x: number,
    y: number,
    extra: Record<string, number> = {},
  ): ColonyBuilding => ({
  ...
  it("inert without a Gallery — no visitor coin, treasury untouched", () => {
    ...
    s.food = 200;
    s.treasury = 1000;
    expect(galleryStatus(s)).toEqual({
      galleries: 0,
      open: false,
      coinPerDay: 0,
    });
  ...
  it("a staffed Gallery in a liveable colony earns visitor coin", () => {
    ...
    services(s.buildings, L.x, L.y);
    s.buildings.push(
      mk("gallery", L.x + 1, L.y, { jobs: COLONY.build.galleryWorkers }),
    );
    s.colonists = 20;
    s.totalJobs = 6 + COLONY.build.galleryWorkers;
    s.food = 200;
    s.treasury = 1000;
  ...
  it("the take scales with appeal — a liveable colony out-earns a drab one", () => {
    const earn = (serviced: boolean) => {
      ...
      s.colonists = 20;
      s.totalJobs = COLONY.build.galleryWorkers + (serviced ? 6 : 0);
      s.food = 200;
      s.treasury = 1000;
      const t0 = s.treasury;
      galleryStep(s, 24 * 60);
  ...
  it("an unstaffed Gallery earns nothing, and the appeal is bounded", () => {
    ...
    s.colonists = 20;
    s.spireStage = COLONY.build.spireStageCount;
    expect(galleryAppeal(s)).toBeLessThanOrEqual(
      COLONY.build.galleryAppealCeiling,
    );
...
describe("Spec 073 — Porter Sheds: the economy you can see move", () => {
  ...
  it("a staffed Porter Shed is working and puts carts on the roads", () => {
    ...
    const L = s.terrain.landing;
    s.buildings.push(
      mk("porter", L.x, L.y, { jobs: COLONY.build.porterWorkers }),
    );
    s.colonists = 20;
    s.totalJobs = COLONY.build.porterWorkers;
  ...
  it("an unstaffed Porter Shed is idle — no carts run", () => {
    ...
    s.colonists = 0;
    s.totalJobs = COLONY.build.porterWorkers; // nobody to push the carts
    const st = porterStatus(s);
  ...
  it("the carts scale with the number of staffed sheds", () => {
    ...
    s.buildings.push(
      mk("porter", L.x + 3, L.y, { jobs: COLONY.build.porterWorkers }),
    );
    s.colonists = 20;
    s.totalJobs = COLONY.build.porterWorkers * 2;
  ...
  it("a Porter Shed changes no good — it is pure visual life (materials + food unchanged vs a colony without one)", () => {
    const run = (withShed: boolean) => {
      ...
      s.colonists = 40;
      s.totalJobs = 6 + (withShed ? COLONY.build.porterWorkers : 0); // fully staffed either way, so the mine digs the same
      s.materials = 50;
      s.food = 200;
      s.powerGen = 100;
      s.power.batteryWh = s.power.batteryCapWh;
      for (let i = 0; i < 20; i++) {
        s.colonists = 40;
        s.totalJobs = 6 + (withShed ? COLONY.build.porterWorkers : 0);
        stepBuild(s, sim.rng, 60);
      }
...
describe("Spec 074 — the Avatar Foundry: the gate on the citizen-as-bot mint", () => {
  ...
  it("a staffed Foundry can mint up to its capacity", () => {
    ...
    const L = s.terrain.landing;
    s.buildings.push(
      mk("avatar", L.x, L.y, { jobs: COLONY.build.avatarWorkers }),
    );
    s.colonists = 30;
    s.totalJobs = COLONY.build.avatarWorkers; // civic sector fully staffed
  ...
  it("an unstaffed Foundry mints nobody", () => {
    ...
    s.colonists = 0;
    s.totalJobs = COLONY.build.avatarWorkers; // nobody to keep the link desks lit
    const st = avatarStatus(s);
  ...
  it("capacity scales with the number of staffed Foundries", () => {
    ...
    s.buildings.push(
      mk("avatar", L.x + 3, L.y, { jobs: COLONY.build.avatarWorkers }),
    );
    s.colonists = 40;
    s.totalJobs = COLONY.build.avatarWorkers * 2;
