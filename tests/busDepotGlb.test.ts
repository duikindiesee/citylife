...
interface GltfJson {
  nodes: GltfNode[];
  meshes: Array<{ name?: string; primitives: Array<{ attributes: { POSITION: number }; indices?: number; material?: number; mode?: number }> }>;
...
function parseGltfJson(raw: string): GltfJson {
  ...
  for (let index = start; index < raw.length; index += 1) {
    ...
    else if (char === "}" && --depth === 0) return JSON.parse(raw.slice(start, index + 1));
  }
...
const REQUIRED_NODES = [
  "Bus_Depot",
  "Depot_Apron",
  ...Array.from({ length: 10 }, (_, index) => `Depot_Bay_${String(index).padStart(2, "0")}`),
  "Depot_Office",
  "Depot_Shelter",
  "Depot_Sign",
];

const depotGlbBytes = readFileSync(new URL("../public/assets/citylife/props/bus-depot.glb", import.meta.url));
...
describe("bus-depot.glb", () => {
  ...
  it("parses and exposes exactly the 15 contract nodes without cameras, lights or textures", () => {
    expect(json.nodes.map((node) => node.name).sort()).toEqual([...REQUIRED_NODES].sort());
    ...
    expect(root.children?.sort((a, b) => a - b)).toEqual(Array.from({ length: 14 }, (_, index) => index));
  ...
  it("keeps the committed generator deterministic and export-only", () => {
    expect(generatorSource).not.toMatch(/\brandom\b|Math\.random|Date\.now|time\.time/);
  ...
  it("pins the Y-up 48 by 28 metre pad, +X gate furniture and exact open 4 by 13 metre bays", () => {
    ...
    for (let index = 0; index < 10; index += 1) {
      const bay = nodeBounds(json, `Depot_Bay_${String(index).padStart(2, "0")}`);
    ...
    for (const name of ["Depot_Office", "Depot_Shelter", "Depot_Sign"]) {
      ...
      expect(gateFurniture.min[0], `${name} gate-side minimum X`).toBeGreaterThan(16);
      expect(gateFurniture.max[0], `${name} within +X gate edge`).toBeLessThanOrEqual(24);
    }
  });

  it("uses the emissive sign material slot and stays under 25k triangles", () => {
    const signMesh = nodeMesh(json, "Depot_Sign");
    const signMaterials = signMesh.primitives.map((primitive) => json.materials?.[primitive.material ?? -1]);
    const signMaterial = signMaterials.find((material) => material?.name === "sign");
    expect(signMaterial).toBeDefined();
    expect(signMaterial?.emissiveFactor).toEqual([0.10000000149011612, 0.75, 1]);
    expect(signMaterial?.extensions?.KHR_materials_emissive_strength?.emissiveStrength).toBe(4);
    ...
    const triangles = json.meshes.reduce((sum, mesh) => sum + mesh.primitives.reduce((meshSum, primitive) => {
      ...
      return meshSum + (primitive.mode === undefined || primitive.mode === 4 ? count / 3 : 0);
    }, 0), 0);
