...
import {
  ...
  mergeKookerbook,
} from "../src/colony/bot/kookerbookStore";
...
describe("kookerbook — the bot social network model (spec 082 P0)", () => {
  it("creates a screened profile; unsafe alias or bio is refused", () => {
    ...
    expect(
      createProfile({ citizenId: "x", alias: "secret token man", bio: "hi" }),
    ).toBeNull();
    expect(
      createProfile({
        citizenId: "x",
        alias: "Fine Name",
        bio: "my password is here",
      }),
    ).toBeNull();
  });
  ...
  it("posts are sol-stamped, newest first, and screened", () => {
    ...
    p = addPost(p, {
      sol: 1,
      kind: "event",
      text: "Joe arrived in Landing One.",
    })!;
    p = addPost(p, {
      sol: 2,
      kind: "narration",
      text: "The tide is kind this morning.",
    })!;
    expect(p.posts[0]!.sol).toBe(2);
    expect(p.posts[1]!.sol).toBe(1);
    expect(
      addPost(p, { sol: 3, kind: "event", text: "leaking a token here" }),
    ).toBeNull();
  });

  it("caps the timeline at POST_CAP, evicting the oldest", () => {
    let p = joe();
    for (let i = 0; i < POST_CAP + 10; i++)
      p = addPost(p, {
        sol: i,
        kind: "event",
        text: `day ${i} in the colony`,
      })!;
  ...
  it("rate-caps authored posts per sol; events are uncapped", () => {
    ...
    for (let i = 0; i < AUTHORED_PER_SOL; i++)
      p = addPost(p, {
        sol: 5,
        kind: "authored",
        text: `thought number ${i}`,
      })!;
    expect(
      addPost(p, { sol: 5, kind: "authored", text: "one too many" }),
    ).toBeNull();
    expect(
      addPost(p, { sol: 6, kind: "authored", text: "new sol, new thought" }),
    ).toBeTruthy();
    expect(
      addPost(p, { sol: 5, kind: "event", text: "events still flow" }),
    ).toBeTruthy();
  });
  ...
  it("safeProfile rebuilds stored data through the screened path — tampered posts are dropped", () => {
    ...
    (tampered["posts"] as unknown[]).unshift({
      id: "x",
      sol: 2,
      seq: 0,
      kind: "event",
      text: "stole a secret key today",
    });
  ...
  it("store round-trips profiles locally and merge prefers the backend copy", () => {
    ...
    p = addPost(p, {
      sol: 1,
      kind: "event",
      text: "moved in by the lighthouse",
    })!;
    expect(saveProfileLocal(p)).toBe(true);
    const local = loadKookerbookLocal();
    expect(local["citizen_joe"]!.alias).toBe("Joe the Crab");
    expect(local["citizen_joe"]!.posts[0]!.text).toBe(
      "moved in by the lighthouse",
    );
    const backendCopy = addPost(local["citizen_joe"]!, {
      sol: 2,
      kind: "event",
      text: "cross device truth",
    })!;
