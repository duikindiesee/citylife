...
import { generateSafePersonality, MockBotAdapter, type BotAdapter } from '../src/colony/bots'
...
class ProfaneAdapter implements BotAdapter {
  ...
  async generate(): Promise<string> { return 'x' }
  async generatePersonality(): Promise<string> { return 'You are a fucking legend.' }
}
...
describe('generateSafePersonality — enforces PG on input AND output', () => {
  it('returns a clean personality for a clean prompt (mock)', async () => {
    const r = await generateSafePersonality(new MockBotAdapter(), 'a gentle gardener who sings to the plants')
  ...
  it('rejects an unsafe PROMPT before calling the model', async () => {
    let called = false
    const adapter: BotAdapter = { source: 't', async generate() { return '' }, async generatePersonality() { called = true; return 'clean' } }
  ...
  it('rejects an unsafe model OUTPUT and never returns it', async () => {
    const r = await generateSafePersonality(new ProfaneAdapter(), 'a kind baker')
    expect(r.ok).toBe(false)
  ...
  it('falls back gracefully when the adapter cannot generate', async () => {
    const noGen: BotAdapter = { source: 'nogen', async generate() { return '' } }
