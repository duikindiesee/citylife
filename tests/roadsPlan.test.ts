...
describe("road planning — least-cost routed block frames", () => {
  ...
  it("keeps roads off blocked-steep ground when a route exists (contour, not fall line)", () => {
    ...
    const bad = sim.state.roads.filter(
      (r) => t.buildable[t.idx(r.x, r.y)] === 0,
    );
    ...
    expect(bad.length).toBeLessThanOrEqual(
      Math.ceil(sim.state.roads.length * 0.1),
    );
  });

  it("is deterministic — the same seed lays the identical network", () => {
    const a = new ColonySim(1234).state.roads
      .map((r) => `${r.x},${r.y}`)
      .join("|");
    const b = new ColonySim(1234).state.roads
      .map((r) => `${r.x},${r.y}`)
      .join("|");
    expect(a).toBe(b);
  });
  ...
  it("PARCEL LAND IS SACRED — reserveParcelLand purges roads under parcels and blocks future laying", () => {
    ...
    for (let dx = -1; dx <= 1; dx++)
      for (let dy = -1; dy <= 1; dy++)
        cells.push({ x: roadCell.x + dx, y: roadCell.y + dy });
    ...
    for (const r of sim.state.roads) {
      ...
      expect(inside, `road ${r.x},${r.y} still inside reserved land`).toBe(
        false,
      );
    }
    for (const c of cells)
      expect(sim.state.occupied.has(`${c.x},${c.y}`)).toBe(true);
  });
  ...
  it("after the runtime-style homestead reservation, NO colony road sits under any parcel", () => {
    ...
    for (const r of sim.state.roads) {
      expect(
        reserved.has(`${r.x},${r.y}`),
        `road ${r.x},${r.y} under a homestead`,
      ).toBe(false);
    }
  });
