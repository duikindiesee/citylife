...
describe("spec 083 P2 — the Builder Desk contract", () => {
  ...
  it("a generous budget agrees, a starved budget walks — same seeds, the lever decides", () => {
    ...
    const generous = negotiate({
      clientSeed: seed,
      builderSeed: VIW_SEED,
      dream,
      budget: priceBrief(dream) * 3,
    });
    const starved = negotiate({
      clientSeed: seed,
      builderSeed: VIW_SEED,
      dream,
      budget: 10,
  ...
  it("the seeded purse strikes a deal for the vast majority — the city gets built", () => {
    ...
    for (let seed = 1; seed <= 60; seed++) {
      ...
      const session = negotiate({
        ...
        builderSeed: VIW_SEED,
        dream,
        budget: seededBudget(seed, dream),
      });
      if (session.state === "agreed") {
        agreed++;
        expect(
          validateBlueprint(briefToBlueprint(session.agreedBrief!, seed)).ok,
          `seed ${seed}`,
        ).toBe(true);
      }
    }
