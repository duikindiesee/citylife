...
import {
  ...
  briefToBlueprint,
  type Brief,
} from "../src/colony/builder/negotiation";
import {
  parseBlueprint,
  validateBlueprint,
...
function seeds(n: number): number[] {
  ...
  const out: number[] = [];
  for (let i = 0; i < n; i++)
    out.push((((i + 1) * 73856093) ^ ((i * 7 + 3) * 19349663)) >>> 0);
  return out;
}
...
describe("builder negotiation — the pure deterministic engine (spec 083 P1)", () => {
  ...
  it("priceBrief is pure and monotonic in bedrooms and floor area", () => {
    ...
    expect(priceBrief({ ...base, outdoor: "pool" })).toBeGreaterThan(
      priceBrief({ ...base, outdoor: "patio" }),
    );
    expect(priceBrief({ ...base, outdoor: "patio" })).toBeGreaterThan(
      priceBrief(base),
    );
  });
  ...
  it("a stingy budget trims to an agreement or walks within the round caps", () => {
    for (const s of seeds(30)) {
      ...
      expect(
        session.rounds.filter((r) => r.who === "viw").length,
      ).toBeLessThanOrEqual(3);
      expect(
        session.rounds.filter((r) => r.who === "client").length,
      ).toBeLessThanOrEqual(3);
      session.rounds.forEach((r, i) =>
        expect(r.who).toBe(i % 2 === 0 ? "viw" : "client"),
      );
      ...
      if (session.state === "agreed") {
        ...
        expect(priceBrief(session.agreedBrief!)).toBeLessThanOrEqual(
          priceBrief(dream),
        );
    ...
    for (const s of seeds(40)) {
      ...
      for (const stretch of [0.85, 0.9, 0.95, 1.0]) {
        ...
        if (session.state === "agreed" && session.agreedPrice! > budget) {
          middles++;
          const lastQuote = [...session.rounds]
            .reverse()
            .find((r) => r.who === "viw")!;
  ...
  it("briefToBlueprint validates, holds the bedroom count and lands the door for 60 dreamed briefs", () => {
    for (const s of seeds(60)) {
      for (const dir of ["n", "s"] as const) {
        ...
        expect(v.ok, `seed ${s} door ${dir}: ${v.errors.join("; ")}`).toBe(
          true,
        );
        const p = parseBlueprint(script);
        expect(p.doorDir).toBe(brief.doorDir);
        expect(p.wallH).toBe(brief.storeys);
        expect(p.rooms.filter((r) => r.kind === "bedroom").length).toBe(
          brief.bedrooms,
        );
        const outdoor = p.rooms.filter(
          (r) => r.kind === "patio" || r.kind === "pool",
        );
  ...
  it("every agreed brief a negotiation can produce also compiles to a valid blueprint", () => {
    ...
    for (const s of seeds(50)) {
      ...
      for (const budget of [120, 250, 400, 10000]) {
        ...
        expect(
          parseBlueprint(script).rooms.filter((r) => r.kind === "bedroom")
            .length,
        ).toBe(session.agreedBrief!.bedrooms);
      }
    }
