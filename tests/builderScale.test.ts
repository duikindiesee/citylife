...
describe("spec 084 S4 — caps + the enforced voxel budget", () => {
  ...
  it("a zone whose floor slab alone busts the budget refuses to compile", () => {
    const small =
      "house{w:6 d:5 wallH:1 door:s} room{kind:living x:0 y:0 w:6 d:5 win:1}";
    expect(() => compileBlueprint(small, { w: 50, d: 50, seed: 1 })).toThrow(
      /voxel budget/,
    );
  });
  ...
  it("validateBlueprint rejects beyond the caps: w/d 24, wallH 3, 16 rooms", () => {
    expect(
      validateBlueprint(
        "house{w:25 d:5 wallH:1 door:s} room{kind:living x:0 y:0 w:25 d:5 win:1}",
      ).ok,
    ).toBe(false);
    expect(
      validateBlueprint(
        "house{w:6 d:5 wallH:4 door:s} room{kind:living x:0 y:0 w:6 d:5 win:1}",
      ).ok,
    ).toBe(false);
    const seventeen = Array.from(
      { length: 17 },
      () => "room{kind:living x:0 y:0 w:2 d:2 win:1}",
    ).join(" ");
    expect(
      validateBlueprint(`house{w:6 d:5 wallH:1 door:n} ${seventeen}`).ok,
    ).toBe(false);
    // at the caps exactly is fine
    expect(
      validateBlueprint(
        "house{w:24 d:24 wallH:3 door:s} room{kind:living x:0 y:0 w:24 d:24 win:1}",
      ).ok,
    ).toBe(true);
  });

  it("addRoom spawns 3x3 on estate footprints and stays 2x2 on cottages", () => {
    const cottage = parseBlueprint(
      "house{w:6 d:5 wallH:1 door:s} room{kind:living x:0 y:0 w:6 d:5 win:1}",
    );
    const estate = parseBlueprint(
      "house{w:14 d:10 wallH:1 door:s} room{kind:living x:0 y:0 w:14 d:10 win:1}",
    );
  ...
  it("defaultBlueprint authors finer footprints for estate zones, classic ones otherwise — all valid", () => {
    for (let seed = 1; seed <= 30; seed++) {
      for (const dir of ["n", "s"] as const) {
        ...
        expect(
          validateBlueprint(defaultBlueprint(seed, dir, 19)).ok,
          `seed ${seed} ${dir}`,
        ).toBe(true);
      }
    }
