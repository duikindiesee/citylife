...
describe("Low Power Radio state", () => {
  ...
  it("anyConfigured tells whether at least one channel has a playlist set", () => {
    ...
    const blank = {
      ...createRadio(),
      channels: DEFAULT_CHANNELS.map((c) => ({ ...c, ref: "" })),
    };
    expect(anyConfigured(blank)).toBe(false);
    const wired = {
      ...blank,
      channels: [
        { ...DEFAULT_CHANNELS[0]!, ref: "PLABC" },
        ...blank.channels.slice(1),
      ],
    };
...
describe("parseYouTubeRef — tolerant of any URL shape the user pastes", () => {
  it("bare playlist id", () => {
    expect(parseYouTubeRef("PLLEPa-vYw7we1ESoo7mtd7hG76aDZCKAX")).toEqual({
      listId: "PLLEPa-vYw7we1ESoo7mtd7hG76aDZCKAX",
    });
  });
  ...
  it("watch URL with v + list params", () => {
    expect(
      parseYouTubeRef(
        "https://www.youtube.com/watch?v=uHWbaklm0bE&list=PLLEPa-vYw7we1ESoo7mtd7hG76aDZCKAX",
      ),
    ).toEqual({
      listId: "PLLEPa-vYw7we1ESoo7mtd7hG76aDZCKAX",
      videoId: "uHWbaklm0bE",
  ...
  it("the format users paste from YouTube (VID&list=PL...) — videoId is the bare 11-char prefix", () => {
    expect(
      parseYouTubeRef("uHWbaklm0bE&list=PLLEPa-vYw7we1ESoo7mtd7hG76aDZCKAX"),
    ).toEqual({
      listId: "PLLEPa-vYw7we1ESoo7mtd7hG76aDZCKAX",
      videoId: "uHWbaklm0bE",
    });
...
describe("channelEmbedUrl — with playlist + starting video", () => {
  it("user's pasted format yields a playlist embed that starts on the chosen video", () => {
    const ch: RadioChannel = {
      ...
      vibe: "",
      kind: "youtube-playlist",
      ref: "uHWbaklm0bE&list=PLLEPa-vYw7we1ESoo7mtd7hG76aDZCKAX",
    };
