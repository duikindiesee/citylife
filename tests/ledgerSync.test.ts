...
function harness(opts?: {
  failAt?: (n: number) => { ok: boolean; status: number } | null;
  token?: string | null;
}) {
  const sent: Sent[] = [];
  ...
  const deps: LedgerSyncDeps = {
    ...
    getToken: async () =>
      opts && "token" in opts ? opts.token! : "h.eyJ1c2VySWQiOjgyfQ.s",
    getUserId: () => "82",
    storage: memStorage(),
...
const DEPOSIT: LedgerMove = {
  kind: "deposit",
  citizenId: "citizen_dax",
  amount: 700,
};
const PURCHASE: LedgerMove = {
  kind: "purchase",
  citizenId: "citizen_dax",
  lotId: "lot_3",
  amount: 196,
};
const COMMISSION: LedgerMove = {
  kind: "commission",
  fromCitizenId: "citizen_dax",
  toCitizenId: "citizen_viw",
  lotId: "lot_3",
  amount: 472,
};
...
describe("ledgerSync pure request builders", () => {
  ...
  it("purchaseBody balances (CREDIT citizen == DEBIT land office) and is LAND_PURCHASE", () => {
    const b = purchaseBody(
      "82",
      "citizen_dax",
      196,
      "citylife:purchase:citizen_dax:lot_3",
    );
    ...
    expect(b.entries).toEqual([
      ...
      {
        ownerId: "city_treasury",
        walletType: "TREASURY",
        entryType: "DEBIT",
        amount: 196,
      },
    ]);
    const debit = b.entries
      .filter((e) => e.entryType === "DEBIT")
      .reduce((s, e) => s + e.amount, 0);
    const credit = b.entries
  ...
  it("commissionBody pays the builder and is BUILD_FEE", () => {
    const b = commissionBody(
      "82",
      "citizen_dax",
      "citizen_viw",
      472,
      "citylife:build:citizen_dax:lot_3",
    );
  ...
  it("a different citizen or lot yields a distinct ref (no false dedup after a reload)", () => {
    expect(
      moveRef({ kind: "deposit", citizenId: "citizen_mara", amount: 700 }),
    ).toBe("citylife:deposit:citizen_mara");
    expect(
      moveRef({
        kind: "purchase",
    ...
    ).toBe("citylife:purchase:citizen_mara:lot_4");
  });
});
...
describe("LedgerSync drain — happy path", () => {
  it("posts a deposit to /wallets and a purchase/commission to /transactions, with player auth headers", async () => {
    ...
    expect(sync.status()).toMatchObject({
      ...
      synced: 3,
      lastError: null,
      lastSyncedRef: "citylife:build:citizen_dax:lot_3",
    });
  });
});
...
describe("LedgerSync stop-on-failure preserves order", () => {
  it("syncs up to the failure, keeps the rest queued in order, resumes on the next drain", async () => {
    ...
    let failNext = true;
    const { deps, sent } = harness({
      failAt: (n) => (n === 2 && failNext ? { ok: false, status: 503 } : null),
    });
    const sync = new LedgerSync(deps);
    sync.notice(DEPOSIT);
    sync.notice(PURCHASE);
    ...
    expect(sent.slice(2).map((s) => s.body.transactionType)).toEqual([
      "LAND_PURCHASE",
      "BUILD_FEE",
    ]);
    expect(sync.status()).toMatchObject({
  ...
  it("restores a queued move across instances and drains it; restored synced refs block re-posting", async () => {
    ...
    const deps1: LedgerSyncDeps = {
      transport: async (path, body, headers) => {
        ...
        return block && (body as any)?.transactionType === "LAND_PURCHASE"
          ? { ok: false, status: 503 }
          : { ok: true, status: 200 };
      },
      getToken: async () => "tok",
