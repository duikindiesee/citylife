...
import {
  ...
  PSA_POSTER,
  paintPoster,
} from "../src/colony/commerce/adCanvas";
...
function blockedFor(rt: ColonyRuntime): Set<string> {
  ...
  for (const p of rt.commercialDistrict?.parcels ?? []) {
    for (let y = p.y; y < p.y + p.h; y++)
      for (let x = p.x; x < p.x + p.w; x++) b.add(`${x},${y}`);
  }
  return b;
}
...
describe("081 ad boards — billboard survey", () => {
    ...
    for (const seed of SEEDS) {
      ...
      for (let i = 0; i < sites.length; i++) {
        for (let j = i + 1; j < sites.length; j++) {
          const md =
            Math.abs(sites[i]!.x - sites[j]!.x) +
            Math.abs(sites[i]!.y - sites[j]!.y);
  ...
  it("places at least one board for the live dev seed", () => {
    const rt = rtFor(4242);
    const sites = surveyBillboards(
      rt.commercialDistrict!,
  ...
  it("is deterministic — a second boot of the same seed yields identical sites", () => {
    ...
    const sa = surveyBillboards(
      a.commercialDistrict!,
      a.sim.state.terrain,
      blockedFor(a),
    );
    const sb = surveyBillboards(
      fresh.commercialDistrict!,
      fresh.sim.state.terrain,
      blockedFor(fresh),
    );
  ...
  it("rotation shifts which shop each board shows, keeping placement fixed", () => {
    ...
    expect(s1.map((s) => `${s.x},${s.y},${s.faceX}`)).toEqual(
      s0.map((s) => `${s.x},${s.y},${s.faceX}`),
    );
    if (d.parcels.length > 1 && s0.length > 0)
      expect(s1[0]!.shopId).not.toBe(s0[0]!.shopId);
  });
});
