...
describe("house builder — the blueprint compiler (spec 077 P1)", () => {
  ...
  it("carries multi-storey — wallH 1..3 raises the wall band by one full storey each", () => {
    const one = compileBlueprint(
      "house{w:5 d:5 wallH:1 door:s} room{kind:living x:0 y:0 w:5 d:5 win:1}",
      { w: 7, d: 6, seed: 9 },
    );
    const three = compileBlueprint(
      "house{w:5 d:5 wallH:3 door:s} room{kind:living x:0 y:0 w:5 d:5 win:1}",
      { w: 7, d: 6, seed: 9 },
    );
    ...
    const wallsOne = one.blocks.filter(
      (b) => b.kind === "brick" || b.kind === "brickAlt",
    ).length;
    const wallsThree = three.blocks.filter(
      (b) => b.kind === "brick" || b.kind === "brickAlt",
    ).length;
  ...
  it("caps wallH at 3 storeys so a runaway wallH cannot blow the grid", () => {
    const huge = compileBlueprint(
      "house{w:5 d:5 wallH:99 door:s} room{kind:living x:0 y:0 w:5 d:5 win:1}",
      { w: 7, d: 6, seed: 2 },
    );
  ...
  it("stays under the voxel budget for the worst real case (a 3-storey home on a BIG plot)", () => {
    ...
    const script =
      "house{w:6 d:5 wallH:3 door:s} room{kind:living x:0 y:0 w:4 d:3 win:1} room{kind:bedroom x:4 y:0 w:2 d:3 win:1} room{kind:patio x:0 y:3 w:6 d:2 win:0}";
  ...
  it("is DETERMINISTIC — identical inputs yield byte-identical blocks across two calls", () => {
    ...
    const ser = (blocks: typeof a.blocks) =>
      blocks.map((x) => `${x.x},${x.y},${x.z},${x.kind}`).join("|");
  ...
  it("CARVE semantics — a pool dropped onto a bedroom takes the cells: no full-height brick, no roof above", () => {
    ...
    const inPool = (b: { x: number; y: number }) =>
      b.x >= 4 * n && b.x < 6 * n && b.y >= 4 * n && b.y < 6 * n;
