...
import {
  CitizenGatewayAdapter,
  hasReachablePod,
  type FetchLike,
  type FetchLikeResponse,
} from "../src/colony/bot/citizenClient";
...
describe("CitizenGatewayAdapter — spec 074 pod client", () => {
  it("posts to the OpenAI-compatible endpoint on the pod gateway with the bearer token", async () => {
    ...
    const fetchImpl: FetchLike = async (url, init) => {
      ...
      seenInit = init;
      return jsonResponse({
        choices: [{ message: { content: "I am Pim, glad to be here." } }],
      });
    };
    const a = new CitizenGatewayAdapter(
      "http://bot-pim.citylife-citizens.svc.cluster.local:18789/gateway",
      "tok",
    ...
    expect(seenUrl).toBe(
      "http://bot-pim.citylife-citizens.svc.cluster.local:18789/gateway/v1/chat/completions",
    );
    ...
    expect(
      sent.messages.some((m: { role: string }) => m.role === "assistant"),
    ).toBe(true);
  });
  ...
  it("throws when the network is unreachable, so the runtime can fall back", async () => {
    ...
    const a = new CitizenGatewayAdapter(
      "http://unreachable:18789",
      undefined,
      "hermes-openai-gpt-5.5",
      fetchImpl,
    );
    await expect(a.generate("sys", history, "newcomer")).rejects.toThrow(
      /unreachable/,
    );
  });
  ...
  it("hasReachablePod gates on a real http(s) url", () => {
    expect(
      hasReachablePod(
        "http://bot-a.citylife-citizens.svc.cluster.local:18789/gateway",
      ),
    ).toBe(true);
