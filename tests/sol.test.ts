...
import { solCount, secondsToNextSol, resolveFoundingMs, MS_PER_SOL } from '../src/colony/sol'
...
describe('sols — 1 real day = 1 sol', () => {
  ...
  it('resolveFoundingMs persists the founding instant on first boot and reuses it after', () => {
    ...
    const store = { getItem: (k: string) => (k in mem ? mem[k]! : null), setItem: (k: string, v: string) => { mem[k] = v } }
