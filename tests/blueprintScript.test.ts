...
describe("blueprint DSL — parse, serialise, validate", () => {
  ...
  it("throws on malformed input", () => {
    ...
    expect(() => parseBlueprint("house{w:6 d:5 wallH:2 door:z}")).toThrow(
      /door must be one of/,
    );
    expect(() => parseBlueprint("house{w:6 d:5 door:s}")).toThrow(
      /missing wallH/,
    );
    expect(() => parseBlueprint("house{w:six d:5 wallH:2 door:s}")).toThrow(
      /must be an integer/,
    );
    expect(() =>
      parseBlueprint(
        "house{w:6 d:5 wallH:2 door:s} room{kind:dungeon x:0 y:0 w:2 d:2 win:0}",
      ),
    ).toThrow(/room kind must be one of/);
  });
  ...
  it("validate catches a room that escapes the house bounds (un-enclosed)", () => {
    ...
    const bad =
      "house{w:4 d:4 wallH:2 door:s} room{kind:living x:0 y:0 w:5 d:4 win:0}";
  ...
  it("validate catches an unreachable door (no room on the door edge)", () => {
    ...
    const bad =
      "house{w:5 d:6 wallH:2 door:s} room{kind:living x:0 y:0 w:5 d:2 win:0}";
    const r = validateBlueprint(bad);
    expect(r.ok).toBe(false);
  ...
  it("validate rejects a roomless house and non-positive dimensions", () => {
    ...
    expect(
      validateBlueprint(
        "house{w:0 d:5 wallH:2 door:s} room{kind:living x:0 y:0 w:0 d:0 win:0}",
      ).ok,
    ).toBe(false);
  });
