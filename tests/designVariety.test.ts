...
describe("design variety — every citizen designs their own home (spec 077 P5)", () => {
  ...
  it("24 seeds compile to 24 DISTINCT houses (no two block sets identical)", () => {
    ...
    for (const s of seeds(24)) {
      const h = compileBlueprint(defaultBlueprint(s, "s"), {
        w: ZONE.w,
        d: ZONE.d,
        seed: s,
      });
      const sig = h.blocks
        .map((b) => `${b.x},${b.y},${b.z},${b.kind}`)
        .join("|");
      expect(sigs.has(sig), `seed ${s} duplicates an earlier house`).toBe(
        false,
      );
      sigs.add(sig);
    }
  ...
  it("the street sample is visibly DIVERSE — multiple footprints, storey counts and room mixes", () => {
    ...
    for (const s of seeds(24)) {
      ...
      const kinds = [...bp.matchAll(/kind:(\w+)/g)]
        .map((m) => m[1])
        .sort()
        .join(",");
      mixes.add(kinds);
    ...
    expect(
      footprints.size,
      `footprints seen: ${[...footprints].join(" ")}`,
    ).toBeGreaterThanOrEqual(3);
    expect(storeys.size).toBe(2); // both 1- and 2-storey homes appear
    expect(mixes.size, `room mixes seen: ${mixes.size}`).toBeGreaterThanOrEqual(
      4,
    ); // several archetypes appear
  });
  ...
  it("south-door designs keep the yard at the back — no pool or patio touches the street edge", () => {
    for (const s of seeds(40)) {
      ...
      for (const m of bp.matchAll(
        /room\{kind:(pool|patio) x:\d+ y:(\d+) w:\d+ d:(\d+)/g,
      )) {
        const yEnd = Number(m[2]) + Number(m[3]);
        expect(yEnd, `seed ${s}: ${m[1]} reaches the street edge`).toBeLessThan(
          d,
        );
      }
    }
