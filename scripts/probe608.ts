...
for (const p of n.parcels) {
  console.log(
    ...
    p.w,
    "zone",
    `${p.houseZone.w}x${p.houseZone.d}`,
    "at",
    p.x,
    p.y,
    "distToWater",
    t.distToWater[t.idx(Math.round(p.x), Math.round(p.y))],
  );
}
...
for (const days of [8, 16, 24]) {
  ...
  for (const b of sim.state.buildings)
    kinds.set(b.artifact.kind, (kinds.get(b.artifact.kind) ?? 0) + 1);
  console.log(
    `day ${days}: buildings`,
  ...
  console.log(
    "  roads",
    sim.state.roads.length,
    "cars",
    sim.state.cars.length,
    "colonists",
    Math.round(sim.state.colonists),
  );
}
