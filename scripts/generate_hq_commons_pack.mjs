...
const warmWood = material("Commons_Warm_Wood", 0x8a6a48, {
  roughness: 0.68,
  metalness: 0.04,
});
const darkWood = material("Commons_Dark_Wood", 0x5c4632, {
  roughness: 0.74,
  metalness: 0.03,
});
const blackIron = material("Commons_Black_Iron", 0x424b4b, {
  roughness: 0.36,
  metalness: 0.82,
});
const oldBrass = material("Commons_Old_Brass", 0x927747, {
  roughness: 0.43,
  metalness: 0.9,
});
const obsidian = material("Commons_Obsidian_Panel", 0x27383c, {
  roughness: 0.34,
  metalness: 0.62,
});
...
const fabricSage = material("Commons_Fabric_Sage", 0x6d7a6f, {
  roughness: 0.9,
  metalness: 0.02,
});
const fabricRust = material("Commons_Fabric_Rust", 0x7d5a44, {
  roughness: 0.9,
  metalness: 0.02,
});
...
const rugWeave = material("Commons_Rug_Weave", 0x4a4136, {
  roughness: 0.96,
  metalness: 0.0,
});
...
mesh(
  "Foos_Body",
  new THREE.BoxGeometry(1.3, 0.28, 0.8),
  warmWood,
  foosball,
  [0, 0.66, 0],
);
...
mesh(
  "Arcade_Screen",
  new THREE.BoxGeometry(0.56, 0.5, 0.05),
  screenGlow,
  arcade,
  [0, 1.25, 0.325],
);
mesh(
  "Arcade_Deck",
  new THREE.BoxGeometry(0.66, 0.08, 0.3),
  warmWood,
  arcade,
  [0, 0.86, 0.25],
);
mesh(
  "Arcade_Marquee",
  new THREE.BoxGeometry(0.66, 0.1, 0.12),
  oldBrass,
  arcade,
  [0, 1.75, 0.24],
);
...
mesh(
  "Couch_Back",
  new THREE.BoxGeometry(2.2, 0.45, 0.22),
  fabricSage,
  couch,
  [0, 0.625, -0.365],
);
...
for (let i = 0; i < 3; i += 1) {
  mesh(
    `Couch_Cushion_${i}`,
    new THREE.BoxGeometry(0.58, 0.1, 0.7),
    fabricRust,
    couch,
...
mesh(
  "Fleet_Panel",
  new THREE.BoxGeometry(3.6, 1.9, 0.08),
  obsidian,
  fleet,
  [0, 0.95, 0.04],
);
mesh(
  "Fleet_Screen",
  new THREE.BoxGeometry(3.44, 1.6, 0.03),
  screenGlow,
  fleet,
  [0, 0.95, 0.095],
);
mesh(
  "Fleet_Header",
  new THREE.BoxGeometry(3.6, 0.1, 0.14),
  oldBrass,
  fleet,
  [0, 1.95, 0.07],
);
...
mesh(
  "Snack_Back",
  new THREE.BoxGeometry(1.2, 1.6, 0.03),
  darkWood,
  snack,
  [0, 0.8, 0.015],
);
...
for (let i = 0; i < 3; i += 1) {
  mesh(
    `Snack_Board_${i}`,
    new THREE.BoxGeometry(1.1, 0.035, 0.36),
    warmWood,
    snack,
    [0, 0.3 + i * 0.55, 0.2],
  );
...
mesh(
  "Rug_Field",
  new THREE.BoxGeometry(3.0, 0.02, 2.0),
  rugWeave,
  rug,
  [0, 0.01, 0],
);
mesh(
  "Rug_Border",
  new THREE.BoxGeometry(2.7, 0.005, 1.7),
  fabricRust,
  rug,
  [0, 0.0225, 0],
);
