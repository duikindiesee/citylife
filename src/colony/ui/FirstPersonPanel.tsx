...
export function FirstPersonPanel({ runtime, fp }: {
  ...
  return (
    <div style={{
      position: 'fixed', bottom: 90, right: 12, width: 290,
      background: 'rgba(8,14,24,0.92)', border: '1px solid #1e3a5a',
      borderRadius: 10, padding: '10px 12px', color: '#c8dff0',
      fontFamily: 'monospace', fontSize: 13, zIndex: 50,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button
          style={{ padding: '2px 8px', fontSize: 11, background: 'rgba(255,255,255,0.07)', border: '1px solid #2a4a6a', borderRadius: 5, color: '#7ab0d0', cursor: 'pointer' }}
      ...
      {/* Scene card */}
      {v && (
        <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: 6, padding: '6px 8px', fontSize: 12, lineHeight: 1.6 }}>
          <div>
          ...
          <div>
            <span style={{ color: '#6ea8d0' }}>Time</span>{' '}
            Day {v.clock.day} · {String(v.clock.hour).padStart(2, '0')}:{String(v.clock.minute).padStart(2, '0')}
            {' '}{v.clock.isDay ? '☀' : '🌙'}
          </div>
          {v.nearestCivic.length > 0 && (
            <div>
              <span style={{ color: '#6ea8d0' }}>Near</span>{' '}
              {v.nearestCivic.slice(0, 2).map((b) => `${b.kind} (${Math.round(b.distance)})`).join(', ')}
            </div>
          )}
          {v.neighbours.length > 0 && (
            <div>
              <span style={{ color: '#6ea8d0' }}>Neighbours</span>{' '}
              {v.neighbours.slice(0, 2).map((n) => n.displayName.split(' ')[0]).join(', ')}
            </div>
          )}
      ...
      {/* Bot narration */}
      <div style={{
        minHeight: 38, background: 'rgba(160,212,240,0.06)', borderRadius: 6,
        padding: '6px 8px', fontSize: 12, fontStyle: 'italic', color: '#a0d4f0',
        lineHeight: 1.5,
      }}>
        {fp.narrating
          ...
          : fp.narration
            ? fp.narration
            : <span style={{ opacity: 0.4 }}>step somewhere to hear what {fp.citizenName?.split(' ')[0]} notices</span>
        }
      </div>

      {/* Compass rose */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3, alignSelf: 'center', width: 120 }}>
        {DIR.map(({ label, emoji, dx, dy }) => (
          <button
            ...
            style={{
              padding: 0, height: 36, fontSize: label === '·' ? 18 : 16,
              background: label === '·' ? 'rgba(160,212,240,0.12)' : 'rgba(255,255,255,0.05)',
              border: '1px solid #1e3a5a', borderRadius: 5,
      ...
      <div style={{ fontSize: 10, opacity: 0.4, textAlign: 'center' }}>
        WASD · arrow keys · click arrows above
      </div>
    </div>
  )
}
