  ...
  return (
    <div className="colony">
      ...
      <aside className="hud">
        ...
        {ui.neighborhood.lots.length > 0 && <div className="row"><span>Neighbourhood</span><b style={{ color: '#9fd0a0' }} title="Spec 075 — a prebuilt street of buildable lots. Assign a citizen to a lot, build their minecraft-style voxel home (walls, roof, door, bed, table), or demolish it. Raze-and-evict also destroys the citizen and tears down their Hermes agent.">{ui.neighborhood.built} built · {ui.neighborhood.free} free</b></div>}
        {ui.neighborhood.lots.length > 0 && <div className="row" style={{ flexDirection: 'column', alignItems: 'stretch', gap: 3 }}>{ui.neighborhood.lots.slice(0, 8).map((l) => {
          ...
          return <div key={l.id} style={{ display: 'flex', gap: 5, alignItems: 'center', fontSize: 11 }}>
            <span style={{ flex: 1, color: l.built ? '#cdbf9e' : l.owner ? '#c9a23a' : '#7a8a7a' }}>{l.id.replace('lot_', 'Lot ')}{l.owner ? ` · ${l.owner.split(' ')[0]}` : ' · free'}{l.built ? ' 🏠' : ''}</span>
            {!l.ownerId && firstFree && <button style={{ padding: '0 6px', fontSize: 10 }} onClick={() => runtime.assignLot(firstFree.id, l.id)} title={`Give this lot to ${firstFree.displayName}`}>Assign</button>}
            {l.ownerId && !l.built && <button style={{ padding: '0 6px', fontSize: 10 }} onClick={() => runtime.buildHouse(l.id)} title="Raise a voxel home (costs materials and a free hand)">Build</button>}
