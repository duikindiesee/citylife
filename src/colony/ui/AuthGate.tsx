...
export function AuthGate({ children }: { children: ReactNode }) {
  ...
  // Local-testing-only auth bypass. The colony mounts WITHOUT login ONLY when ALL of these hold: a DEV
  // build, a LOCAL host (localhost / 127.0.0.1 / LAN), and an explicit developer opt-in (VITE_LOCAL_TEST
  // in .env.local or ?skipauth=1). This keeps the skip strictly on the developer's own machine, off the
  // cluster — by the operator's ask. The exact same predicate governs every other DEV/E2E gate (e.g. the
  // arcade Gamehouse) via the shared `isLocalDevAuthBypass`, so no gate can drift toward failing open.
  if (isLocalDevAuthBypass()) return <>{children}</>;
