...
export function R3FDarkCity({ sim }: { sim: ColonySim }) {
  ...
  const mats = useMemo<{
    ...
    const found: { rim: THREE.MeshBasicMaterial | null; halo: THREE.MeshBasicMaterial | null } = {
      rim: null,
      halo: null,
    };
    group.traverse((o) => {
      const m = o as THREE.Mesh;
      if (o.name === 'darkCity-rim') found.rim = m.material as THREE.MeshBasicMaterial;
      if (o.name === 'darkCity-rim-halo') found.halo = m.material as THREE.MeshBasicMaterial;
    });
    return found;
  }, [group]);
  ...
  useFrame(() => {
    ...
    const time = builderActive ? 12 : sim.state.clock.hour + sim.state.clock.minute / 60;
    let dayFactor = 0;
    ...
    if (mats.halo) mats.halo.opacity = HALO_BASE_OPACITY * (0.08 + 0.92 * night);
  });
