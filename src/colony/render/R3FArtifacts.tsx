...
export function R3FArtifacts({ sim, terrainLevel }: R3FArtifactsProps) {
  const assets = useMemo<Record<ArtifactKind, ArtifactAsset>>(
    () => buildArtifactAssets(),
    [],
  );
  const meshes = useRef<Partial<Record<ArtifactKind, THREE.InstancedMesh>>>({});
  const placed = useRef<Record<ArtifactKind, number>>(
  ...
  useEffect(() => {
    const { renderable } = summarizeRenderableArtifacts(
      sim.state.artifacts,
      ARTIFACT_CATALOG_SIZE,
    );
    ...
    const isRoad = (x: number, y: number) =>
      sim.state.roadSet?.has?.(`${x},${y}`) ?? false;
    ...
    <group name="artifacts">
      {ARTIFACT_KINDS.map((kind) => (
        <instancedMesh
          key={kind}
          ref={(el) => {
            if (el) meshes.current[kind] = el;
          }}
          name={`artifact-${kind}`}
          args={[
            assets[kind].geometry,
            assets[kind].material,
            ARTIFACT_CATALOG_SIZE,
          ]}
          castShadow
          frustumCulled={false}
