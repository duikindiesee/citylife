...
export interface BusDepotLayerOptions {
  ...
  wx: (x: number) => number;
  wz: (y: number) => number;
  /** World Y of the slab TOP — the flat plane the buses park on (max ground over the pad). */
  padTopY: number;
}

export function buildBusDepotLayer(opts: BusDepotLayerOptions): THREE.Group {
  const { site, layout, wx, wz, padTopY } = opts;
  ...
  // Apron slab: the whole AABB, top surface at padTopY. Cells span centre±2 m, hence the +CELL.
  const cx = wx(site.x + (site.w - 1) / 2);
  const cz = wz(site.y + (site.h - 1) / 2);
  const apron = new THREE.Mesh(
    new THREE.BoxGeometry(site.w * CELL + 0.4, 0.5, site.h * CELL + 0.4),
    apronMat,
  );
  apron.name = "Depot_Apron";
  apron.position.set(cx, padTopY - 0.25, cz);
  apron.receiveShadow = true;
  g.add(apron);

  // Ten painted bays — a lighter tile per bay; the parked buses standing on them read occupancy.
