...
export function getZoningLayout(
  ...
  const translate = (u: number, d: number) => {
    switch (orientation) {
      case "n":
        return { x: cx + u, y: cy + d };
      case "s":
        return { x: cx + u, y: cy - d };
      case "w":
        return { x: cx + d, y: cy + u };
      case "e":
        return { x: cx - d, y: cy + u };
    }
  };
...
export function R3FRoadBuilder({ sim, runtime }: R3FRoadBuilderProps) {
  ...
  const applyLandscapeEdit = useRoadNetwork(
    (state) => state.applyLandscapeEdit,
  );
  ...
  const sameSessionPlacements = useRoadNetwork(
    (state) => state.sameSessionPlacements,
  );
  const activeRoadType = useRoadNetwork((state) => state.activeRoadType);

  const [startCell, setStartCell] = useState<{ x: number; y: number } | null>(
    null,
  );
      ...
      {builderMode === "bulldoze" &&
        ...
        (() => {
          ...
          const lot = lots.find((l: any) => {
            ...
            return (
              hoverCell.x >= l.x - offX &&
              hoverCell.x <= l.x + offY &&
              hoverCell.y >= l.y - offX &&
              hoverCell.y <= l.y + offY
            );
          });
          ...
          if (tiles[key]) {
            ...
            return (
              <mesh
                position={[
                  (hoverCell.x - terrainSize / 2) * 4,
                  wY + 0.35,
                  (hoverCell.y - terrainSize / 2) * 4,
                ]}
              >
                <boxGeometry args={[4.1, 0.7, 4.1]} />
                <meshStandardMaterial
