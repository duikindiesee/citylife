...
function TurntableCar({ vehicle }: { vehicle: ShowroomVehicle }) {
  ...
  return (
    <group ref={group} name="showroomTurntable">
      ...
      >
        <ringGeometry args={[PLINTH_RADIUS * 0.93, PLINTH_RADIUS * 0.97, 64]} />
  ...
  return (
    <group name="showroomStudio">
      ...
      >
        ...
        <meshStandardMaterial color={0x1a1e22} roughness={0.35} />
      </mesh>
      ...
      {/* FILL — cool blue-hour sky, ~9000 K. Never above 0.6 or the warm/cool split collapses. */}
      <directionalLight
        color={0xc8dcff}
      ...
      {/* KICKER — violet, opposite side and low. This is what makes the rear quarter read as
          three-dimensional rather than as a flat shape against the dark. */}
      <directionalLight
        color={0x8a6bff}
  ...
  return (
    ...
    >
      {/* Replaces the old flat 0xced4d8 clear colour. See showroomEnvironment for why a clear colour
          could never work here: it is not geometry, is not lit, and reflects nothing. */}
      <ShowroomEnvironment />
      <StudioRoom />
