...
export class CitizenGatewayAdapter implements BotAdapter {
  ...
  constructor(
    ...
    private readonly fetchImpl: FetchLike = ((u, i) => fetch(u, i) as unknown as Promise<FetchLikeResponse>),
  ) {}
  ...
  async generate(systemPrompt: string, history: ChatMessage[], speakingAs: Speaker): Promise<string> {
    ...
    try {
      ...
      throw new Error(`citizen pod unreachable (${(e as Error)?.message ?? 'network error'})`)
    }
    if (res.status === 403 || res.status === 401) {
      throw new Error(`citizen pod gateway ${res.status} (browser blocked from the DMZ; falling back)`)
    }
    if (!res.ok) {
...
export function hasReachablePod(botGatewayUrl: string | undefined): boolean {
  return typeof botGatewayUrl === 'string' && /^https?:\/\//.test(botGatewayUrl)
}
