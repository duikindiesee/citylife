...
function safeEntry(lotId: string, e: unknown): StoredBlueprint | null {
  ...
  if (!lotId || !isPublicSafe(script) || !validateBlueprint(script).ok)
    return null;
...
export async function saveBlueprintBackend(
  lotId: string,
  citizenId: string,
  script: string,
): Promise<BackendSaveResult> {
  if (!safeEntry(lotId, { citizenId, script }))
    return { ok: false, error: "invalid blueprint" };
