...
function normalise(s: string): string {
  ...
  const folded = s
    .toLowerCase()
    .replace(/[@48315!0$57+9]/g, (c) => leet[c] ?? c);
...
export function validatePG(text: string | undefined | null): PGResult {
  ...
  if (raw.length > 600)
    return {
      ok: false,
      reason: "That is too long — please keep it short and friendly.",
    };
  const norm = normalise(raw);
  const despaced = norm.replace(/\s+/g, "");
  for (const sub of BANNED_SUBSTRINGS) {
    if (despaced.includes(sub))
      return {
        ok: false,
        reason: "Please keep it friendly and family-appropriate.",
      };
  }
...
export function validatePGName(firstName: string, lastName: string): PGResult {
  ...
  if (
    !/^[A-Za-z][A-Za-z'.-]{0,30}$/.test(firstName.trim()) ||
    !/^[A-Za-z][A-Za-z'.-]{0,30}$/.test(lastName.trim())
  ) {
    return {
      ok: false,
      reason: "A name should be a single word with letters only.",
    };
  }
  return { ok: true };
}
