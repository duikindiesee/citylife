...
export async function spawnCitizenSubUser(input: SpawnCitizenInput): Promise<SpawnResult> {
  ...
  try {
    ...
    const data = (await resp.json()) as { userId?: number; profileId?: number; status?: string }
    return { ok: true, citizenUserId: data.userId, profileId: data.profileId, status: data.status }
  } catch (e) {
