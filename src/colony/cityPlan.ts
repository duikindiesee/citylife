...
export function cellZone(
  landing: { x: number; y: number },
  x: number,
  y: number,
): Zone | null {
  const dx = x - landing.x,
    dy = y - landing.y;
...
export function plotFootprint(
  ...
  const rowsOk = (nhx: number, nhy: number): boolean => {
    for (let yy = cy - nhy; yy <= cy + nhy; yy++)
      for (let xx = cx - nhx; xx <= cx + nhx; xx++)
        if (!padCellOk(t, xx, yy)) return false;
    return true;
  };
...
const DESCRIPTIONS: Record<Vibe, string> = {
  beach:
    "a sandy plot on the crystal shore — sea breeze, gulls, salt in the air",
...
function vibeScore(
  ...
  switch (vibe) {
    case "beach":
      return biome === Biome.Beach && dWater <= 2
        ? 50 - distFromLanding * 0.3
        : 0;
    case "hillside":
      return biome === Biome.Highland || biome === Biome.Mountain
        ? elev * 60 - distFromLanding * 0.2
        : 0;
    case "riverside":
