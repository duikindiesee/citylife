...
function developBlock(state: ColonyState, bx: number, by: number): number {
  ...
  const straight = (ax: number, ay: number, bx2: number, by2: number) => {
    if (ax === bx2)
      for (let y = Math.min(ay, by2); y <= Math.max(ay, by2); y++) lay(ax, y);
    else
      for (let x = Math.min(ax, bx2); x <= Math.max(ax, bx2); x++) lay(x, ay);
  };
...
export function reserveParcelLand(
  state: ColonyState,
...
export function mergeAvenue(
  ...
  for (const c of cells) {
    ...
    const existing = state.roadKind.has(k)
      ? state.roads.find((r) => r.x === c.x && r.y === c.y)
      : undefined;
...
export function roadUpkeepPerDay(state: ColonyState): number {
  ...
  for (const r of state.roads)
    total += COLONY.economy.roadUpkeepByKind[r.kind ?? "street"];
  return total;
}
...
function designHabitat(state: ColonyState, rng: RNG): Artifact {
  return {
    ...
    materialsCost: COLONY.build.matHabitat,
    crew: COLONY.build.crewHabitat,
    materialsGen: 0,
  };
}
function designWorkplace(state: ColonyState, rng: RNG): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: ind ? "industrial" : "commercial",
    color: ind ? INDUSTRIAL_COLOR : COMMERCIAL_COLOR,
    height: ind ? rng.range(0.9, 1.5) : rng.range(1.0, 1.8),
    residents: 0,
    jobs: ind ? COLONY.build.jobsPerIndustrial : COLONY.build.jobsPerCommercial,
    powerLoad: ind ? COLONY.build.industrialLoad : COLONY.build.commercialLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: ind ? COLONY.build.industrialCost : COLONY.build.commercialCost,
    materialsCost: ind
      ? COLONY.build.matIndustrial
      : COLONY.build.matCommercial,
    crew: COLONY.build.crewWork,
    materialsGen: 0,
  };
}
function designSolarFarm(state: ColonyState): Artifact {
  return {
    ...
    materialsCost: COLONY.build.matSolar,
    crew: COLONY.build.crewSolar,
    materialsGen: 0,
  };
}
function designMine(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.mineWorkers,
    powerLoad: 0.3,
    powerGen: 0,
    buildTimeMin: COLONY.build.mineBuildHours * 60,
    cost: COLONY.build.mineCost,
    materialsCost: COLONY.build.matMine,
    crew: COLONY.build.crewMine,
    materialsGen: COLONY.build.mineOutputPerDay,
  };
}
function designWorkshop(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.0,
    residents: 0,
    jobs: COLONY.build.workshopWorkers,
    powerLoad: 0.6,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.workshopCost,
    materialsCost: COLONY.build.matWorkshop,
    crew: COLONY.build.crewWorkshop,
    materialsGen: 0,
  };
}
function designWaterHub(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.8,
    residents: 0,
    jobs: COLONY.build.waterHubWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.waterHubCost,
    materialsCost: COLONY.build.matWaterHub,
    crew: COLONY.build.crewWaterHub,
    materialsGen: 0,
    componentsCost: COLONY.build.compWaterHub,
  };
}
function designGreenhouse(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.6,
    residents: 0,
    jobs: COLONY.build.greenhouseWorkers,
    powerLoad: 0.5,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.greenhouseCost,
    materialsCost: COLONY.build.matGreenhouse,
    crew: COLONY.build.crewGreenhouse,
    materialsGen: 0,
    componentsCost: COLONY.build.compGreenhouse,
  };
}
function designDepot(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.8,
    residents: 0,
    jobs: COLONY.build.depotWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.depotCost,
    materialsCost: COLONY.build.matDepot,
    crew: COLONY.build.crewDepot,
    materialsGen: 0,
    componentsCost: COLONY.build.compDepot,
  };
}
function designClinic(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.clinicWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.clinicCost,
    materialsCost: COLONY.build.matClinic,
    crew: COLONY.build.crewClinic,
    materialsGen: 0,
    componentsCost: COLONY.build.compClinic,
  };
}
function designTheatre(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.1,
    residents: 0,
    jobs: COLONY.build.theatreWorkers,
    powerLoad: 0.6,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.theatreCost,
    materialsCost: COLONY.build.matTheatre,
    crew: COLONY.build.crewTheatre,
    materialsGen: 0,
    componentsCost: COLONY.build.compTheatre,
  };
}
function designSurvey(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.2,
    residents: 0,
    jobs: COLONY.build.surveyWorkers,
    powerLoad: 0.5,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.surveyCost,
    materialsCost: COLONY.build.matSurvey,
    crew: COLONY.build.crewSurvey,
    materialsGen: 0,
    componentsCost: COLONY.build.compSurvey,
  };
}
function designExchange(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.0,
    residents: 0,
    jobs: COLONY.build.exchangeWorkers,
    powerLoad: 0.5,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.exchangeCost,
    materialsCost: COLONY.build.matExchange,
    crew: COLONY.build.crewExchange,
    materialsGen: 0,
    componentsCost: COLONY.build.compExchange,
  };
}
function designFoundry(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.1,
    residents: 0,
    jobs: COLONY.build.foundryWorkers,
    powerLoad: 0.7,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.foundryCost,
    materialsCost: COLONY.build.matFoundry,
    crew: COLONY.build.crewFoundry,
    materialsGen: 0,
    componentsCost: COLONY.build.compFoundry,
  };
}
function designMast(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.6,
    residents: 0,
    jobs: COLONY.build.mastWorkers,
    powerLoad: 0.5,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.mastCost,
    materialsCost: COLONY.build.matMast,
    crew: COLONY.build.crewMast,
    materialsGen: 0,
    componentsCost: COLONY.build.compMast,
  };
}
function designBattery(state: ColonyState): Artifact {
  ...
  return {
    ...
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.batteryCost,
    materialsCost: COLONY.build.matBattery,
    crew: COLONY.build.crewBattery,
    materialsGen: 0,
    componentsCost: COLONY.build.compBattery,
    reelsCost: COLONY.build.reelBattery,
  };
}
function designScrubber(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.45,
    residents: 0,
    jobs: COLONY.build.scrubberWorkers,
    powerLoad: 0.3,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.scrubberCost,
    materialsCost: COLONY.build.matScrubber,
    crew: COLONY.build.crewScrubber,
    materialsGen: 0,
    componentsCost: COLONY.build.compScrubber,
  };
}
function designAcademy(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.0,
    residents: 0,
    jobs: COLONY.build.academyWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.academyCost,
    materialsCost: COLONY.build.matAcademy,
    crew: COLONY.build.crewAcademy,
    materialsGen: 0,
    componentsCost: COLONY.build.compAcademy,
  };
}
function designTransit(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.8,
    residents: 0,
    jobs: COLONY.build.transitWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.transitCost,
    materialsCost: COLONY.build.matTransit,
    crew: COLONY.build.crewTransit,
    materialsGen: 0,
    componentsCost: COLONY.build.compTransit,
  };
}
function designMaintShed(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "maintshed",
    color: MAINTSHED_COLOR,
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.maintShedWorkers,
    powerLoad: 0.3,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.maintShedCost,
    materialsCost: COLONY.build.matMaintShed,
    crew: COLONY.build.crewMaintShed,
    materialsGen: 0,
    componentsCost: COLONY.build.compMaintShed,
  };
}
function designStorehouse(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "storehouse",
    color: STOREHOUSE_COLOR,
    height: 0.6,
    residents: 0,
    jobs: COLONY.build.storehouseWorkers,
    powerLoad: 0.3,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.storehouseCost,
    materialsCost: COLONY.build.matStorehouse,
    crew: COLONY.build.crewStorehouse,
    materialsGen: 0,
    componentsCost: COLONY.build.compStorehouse,
  };
}
function designBellhouse(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "bellhouse",
    color: BELLHOUSE_COLOR,
    height: 1.1,
    residents: 0,
    jobs: COLONY.build.bellhouseWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.bellhouseCost,
    materialsCost: COLONY.build.matBellhouse,
    crew: COLONY.build.crewBellhouse,
    materialsGen: 0,
    componentsCost: COLONY.build.compBellhouse,
    reelsCost: COLONY.build.reelBellhouse,
  };
}
function designLevy(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.2,
    residents: 0,
    jobs: COLONY.build.levyWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.levyOfficeCost,
    materialsCost: COLONY.build.matLevy,
    crew: COLONY.build.crewLevy,
    materialsGen: 0,
    componentsCost: COLONY.build.compLevy,
    reelsCost: COLONY.build.reelLevy,
  };
}
function designFeverWatch(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "feverwatch",
    color: FEVERWATCH_COLOR,
    height: 1.0,
    residents: 0,
    jobs: COLONY.build.feverWatchWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.feverWatchCost,
    materialsCost: COLONY.build.matFeverWatch,
    crew: COLONY.build.crewFeverWatch,
    materialsGen: 0,
    componentsCost: COLONY.build.compFeverWatch,
  };
}
function designMarket(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.9,
    residents: 0,
    jobs: COLONY.build.marketWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.marketCost,
    materialsCost: COLONY.build.matMarket,
    crew: COLONY.build.crewMarket,
    materialsGen: 0,
    componentsCost: COLONY.build.compMarket,
  };
}
function designWard(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.0,
    residents: 0,
    jobs: COLONY.build.wardWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.wardCost,
    materialsCost: COLONY.build.matWard,
    crew: COLONY.build.crewWard,
    materialsGen: 0,
    componentsCost: COLONY.build.compWard,
  };
}
function designPayOffice(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "payoffice",
    color: PAYOFFICE_COLOR,
    height: 1.2,
    residents: 0,
    jobs: COLONY.build.payOfficeWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.payOfficeCost,
    materialsCost: COLONY.build.matPayOffice,
    crew: COLONY.build.crewPayOffice,
    materialsGen: 0,
    componentsCost: COLONY.build.compPayOffice,
    reelsCost: COLONY.build.reelPayOffice,
  };
}
function designFeast(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.feastWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.feastDeckCost,
    materialsCost: COLONY.build.matFeast,
    crew: COLONY.build.crewFeast,
    materialsGen: 0,
    componentsCost: COLONY.build.compFeast,
    reelsCost: COLONY.build.reelFeast,
  };
}
function designSkimmer(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.6,
    residents: 0,
    jobs: COLONY.build.skimmerWorkers,
    powerLoad: 0.3,
    powerGen: 0,
    buildTimeMin: COLONY.build.mineBuildHours * 60,
    cost: COLONY.build.skimmerCost,
    materialsCost: COLONY.build.matSkimmer,
    crew: COLONY.build.crewSkimmer,
    materialsGen: 0,
    fibreGen: COLONY.build.fibreGenPerDay,
  };
}
function designWeavery(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.0,
    residents: 0,
    jobs: COLONY.build.weaveryWorkers,
    powerLoad: 0.5,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.weaveryCost,
    materialsCost: COLONY.build.matWeavery,
    crew: COLONY.build.crewWeavery,
    materialsGen: 0,
    componentsCost: COLONY.build.compWeavery,
  };
}
function designLiaison(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.4,
    residents: 0,
    jobs: COLONY.build.liaisonWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.liaisonCost,
    materialsCost: COLONY.build.matLiaison,
    crew: COLONY.build.crewLiaison,
    materialsGen: 0,
    componentsCost: COLONY.build.compLiaison,
    reelsCost: COLONY.build.reelLiaison,
  };
}
function designStormwatch(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "stormwatch",
    color: STORMWATCH_COLOR,
    height: 1.1,
    residents: 0,
    jobs: COLONY.build.stormwatchWorkers,
    powerLoad: 0.3,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.stormwatchCost,
    materialsCost: COLONY.build.matStormwatch,
    crew: COLONY.build.crewStormwatch,
    materialsGen: 0,
    componentsCost: COLONY.build.compStormwatch,
  };
}
function designHall(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.6,
    residents: 0,
    jobs: COLONY.build.hallWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.hallCost,
    materialsCost: COLONY.build.matHall,
    crew: COLONY.build.crewHall,
    materialsGen: 0,
    componentsCost: COLONY.build.compHall,
  };
}
function designImportOffice(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.3,
    residents: 0,
    jobs: COLONY.build.importOfficeWorkers,
    powerLoad: 0.4,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.importOfficeCost,
    materialsCost: COLONY.build.matImportOffice,
    crew: COLONY.build.crewImportOffice,
    materialsGen: 0,
    componentsCost: COLONY.build.compImportOffice,
  };
}
function designShrine(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.2,
    residents: 0,
    jobs: COLONY.build.shrineWorkers,
    powerLoad: 0.2,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.shrineCost,
    materialsCost: COLONY.build.matShrine,
    crew: COLONY.build.crewShrine,
    materialsGen: 0,
    componentsCost: COLONY.build.compShrine,
  };
}
function designComptroller(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "comptroller",
    color: COMPTROLLER_COLOR,
    height: 1.4,
    residents: 0,
    jobs: COLONY.build.comptrollerWorkers,
    powerLoad: 0.3,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.comptrollerCost,
    materialsCost: COLONY.build.matComptroller,
    crew: COLONY.build.crewComptroller,
    materialsGen: 0,
    componentsCost: COLONY.build.compComptroller,
  };
}
function designRoster(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.3,
    residents: 0,
    jobs: COLONY.build.rosterWorkers,
    powerLoad: 0.3,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.rosterCost,
    materialsCost: COLONY.build.matRoster,
    crew: COLONY.build.crewRoster,
    materialsGen: 0,
    componentsCost: COLONY.build.compRoster,
  };
}
function designSchool(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.1,
    residents: 0,
    jobs: COLONY.build.schoolWorkers,
    powerLoad: 0.2,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.schoolCost,
    materialsCost: COLONY.build.matSchool,
    crew: COLONY.build.crewSchool,
    materialsGen: 0,
  };
}
function designCensus(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.5,
    residents: 0,
    jobs: COLONY.build.censusWorkers,
    powerLoad: 0.3,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.censusCost,
    materialsCost: COLONY.build.matCensus,
    crew: COLONY.build.crewCensus,
    materialsGen: 0,
    componentsCost: COLONY.build.compCensus,
  };
}
function designFolio(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.2,
    residents: 0,
    jobs: COLONY.build.folioWorkers,
    powerLoad: 0.6,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.folioCost,
    materialsCost: COLONY.build.matFolio,
    crew: COLONY.build.crewFolio,
    materialsGen: 0,
    componentsCost: COLONY.build.compFolio,
  };
}
function designTurbine(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.8,
    residents: 0,
    jobs: COLONY.build.turbineWorkers,
    powerLoad: 0,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.turbineCost,
    materialsCost: COLONY.build.matTurbine,
    crew: COLONY.build.crewTurbine,
    materialsGen: 0,
    componentsCost: COLONY.build.compTurbine,
  };
}
function designCistern(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.9,
    residents: 0,
    jobs: COLONY.build.cisternWorkers,
    powerLoad: COLONY.build.cisternPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.cisternCost,
    materialsCost: COLONY.build.matCistern,
    crew: COLONY.build.crewCistern,
    materialsGen: 0,
    componentsCost: COLONY.build.compCistern,
  };
}
function designToolCrib(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "toolcrib",
    color: TOOLCRIB_COLOR,
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.toolCribWorkers,
    powerLoad: COLONY.build.toolCribPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.toolCribCost,
    materialsCost: COLONY.build.matToolCrib,
    crew: COLONY.build.crewToolCrib,
    materialsGen: 0,
    componentsCost: COLONY.build.compToolCrib,
  };
}
function designSeedLoft(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "seedloft",
    color: SEEDLOFT_COLOR,
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.seedLoftWorkers,
    powerLoad: COLONY.build.seedLoftPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.seedLoftCost,
    materialsCost: COLONY.build.matSeedLoft,
    crew: COLONY.build.crewSeedLoft,
    materialsGen: 0,
    componentsCost: COLONY.build.compSeedLoft,
  };
}
function designSurveyCamp(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "surveycamp",
    color: SURVEYCAMP_COLOR,
    height: 0.6,
    residents: 0,
    jobs: COLONY.build.surveyCampWorkers,
    powerLoad: COLONY.build.surveyCampPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.surveyCampCost,
    materialsCost: COLONY.build.matSurveyCamp,
    crew: COLONY.build.crewSurveyCamp,
    materialsGen: 0,
    componentsCost: COLONY.build.compSurveyCamp,
  };
}
function designCalendarOffice(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.8,
    residents: 0,
    jobs: COLONY.build.calendarWorkers,
    powerLoad: COLONY.build.calendarPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.calendarCost,
    materialsCost: COLONY.build.matCalendar,
    crew: COLONY.build.crewCalendar,
    materialsGen: 0,
    componentsCost: COLONY.build.compCalendar,
    reelsCost: COLONY.build.reelCalendar,
  };
}
function designHallOfNames(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "hallofnames",
    color: HALLOFNAMES_COLOR,
    height: 1.0,
    residents: 0,
    jobs: COLONY.build.hallOfNamesWorkers,
    powerLoad: COLONY.build.hallOfNamesPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.hallOfNamesCost,
    materialsCost: COLONY.build.matHallOfNames,
    crew: COLONY.build.crewHallOfNames,
    materialsGen: 0,
    componentsCost: COLONY.build.compHallOfNames,
  };
}
function designNetDock(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.6,
    residents: 0,
    jobs: COLONY.build.netDockWorkers,
    powerLoad: COLONY.build.netDockPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.netDockCost,
    materialsCost: COLONY.build.matNetDock,
    crew: COLONY.build.crewNetDock,
    materialsGen: 0,
    componentsCost: COLONY.build.compNetDock,
    rimfishGen: COLONY.build.rimfishPerDay,
  };
}
function designCellar(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.5,
    residents: 0,
    jobs: COLONY.build.cellarWorkers,
    powerLoad: COLONY.build.cellarPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.cellarCost,
    materialsCost: COLONY.build.matCellar,
    crew: COLONY.build.crewCellar,
    materialsGen: 0,
    componentsCost: COLONY.build.compCellar,
    toolsCost: COLONY.build.toolCellar,
    duskcapGen: COLONY.build.duskcapPerDay,
  };
}
function designBathhouse(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "bathhouse",
    color: BATHHOUSE_COLOR,
    height: 0.8,
    residents: 0,
    jobs: COLONY.build.bathWorkers,
    powerLoad: COLONY.build.bathPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.bathCost,
    materialsCost: COLONY.build.matBath,
    crew: COLONY.build.crewBath,
    materialsGen: 0,
    componentsCost: COLONY.build.compBath,
    toolsCost: COLONY.build.toolBath,
  };
}
function designLibrary(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.0,
    residents: 0,
    jobs: COLONY.build.libraryWorkers,
    powerLoad: COLONY.build.libraryPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.libraryCost,
    materialsCost: COLONY.build.matLibrary,
    crew: COLONY.build.crewLibrary,
    materialsGen: 0,
    componentsCost: COLONY.build.compLibrary,
    toolsCost: COLONY.build.toolLibrary,
  };
}
function designGallery(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.1,
    residents: 0,
    jobs: COLONY.build.galleryWorkers,
    powerLoad: COLONY.build.galleryPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.galleryCost,
    materialsCost: COLONY.build.matGallery,
    crew: COLONY.build.crewGallery,
    materialsGen: 0,
    componentsCost: COLONY.build.compGallery,
    toolsCost: COLONY.build.toolGallery,
  };
}
function designPorter(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.8,
    residents: 0,
    jobs: COLONY.build.porterWorkers,
    powerLoad: 0,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.porterCost,
    materialsCost: COLONY.build.matPorter,
    crew: COLONY.build.crewPorter,
    materialsGen: 0,
    componentsCost: COLONY.build.compPorter,
    toolsCost: COLONY.build.toolPorter,
    reelsCost: COLONY.build.reelPorter,
  };
}
function designAvatar(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 1.5,
    residents: 0,
    jobs: COLONY.build.avatarWorkers,
    powerLoad: COLONY.build.avatarPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.avatarCost,
    materialsCost: COLONY.build.matAvatar,
    crew: COLONY.build.crewAvatar,
    materialsGen: 0,
    componentsCost: COLONY.build.compAvatar,
    toolsCost: COLONY.build.toolAvatar,
    reelsCost: COLONY.build.reelAvatar,
  };
}
function designSanitationPost(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "sanitation",
    color: SANITATION_COLOR,
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.sanitationWorkers,
    powerLoad: COLONY.build.sanitationPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.sanitationCost,
    materialsCost: COLONY.build.matSanitation,
    crew: COLONY.build.crewSanitation,
    materialsGen: 0,
    componentsCost: COLONY.build.compSanitation,
  };
}
function designWatchNook(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "watchnook",
    color: WATCHNOOK_COLOR,
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.watchNookWorkers,
    powerLoad: COLONY.build.watchNookPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.watchNookCost,
    materialsCost: COLONY.build.matWatchNook,
    crew: COLONY.build.crewWatchNook,
    materialsGen: 0,
    componentsCost: COLONY.build.compWatchNook,
  };
}
function designVarietyCounter(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "rationvar",
    color: RATIONVAR_COLOR,
    height: 0.6,
    residents: 0,
    jobs: COLONY.build.varietyCounterWorkers,
    powerLoad: COLONY.build.varietyCounterPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.varietyCounterCost,
    materialsCost: COLONY.build.matVarietyCounter,
    crew: COLONY.build.crewVarietyCounter,
    materialsGen: 0,
    componentsCost: COLONY.build.compVarietyCounter,
    toolsCost: COLONY.build.toolVarietyCounter,
  };
}
function designDryRack(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.dryRackWorkers,
    powerLoad: COLONY.build.dryRackPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.dryRackCost,
    materialsCost: COLONY.build.matDryRack,
    crew: COLONY.build.crewDryRack,
    materialsGen: 0,
    componentsCost: COLONY.build.compDryRack,
    toolsCost: COLONY.build.toolDryRack,
    linenCost: COLONY.build.linenDryRack,
  };
}
function designRegistry(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.9,
    residents: 0,
    jobs: COLONY.build.registryWorkers,
    powerLoad: 0,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.registryCost,
    materialsCost: COLONY.build.matRegistry,
    crew: COLONY.build.crewRegistry,
    materialsGen: 0,
    componentsCost: COLONY.build.compRegistry,
    toolsCost: COLONY.build.toolRegistry,
    folioCost: COLONY.build.folioRegistry,
  };
}
function designPlanter(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.35,
    residents: 0,
    jobs: COLONY.build.planterWorkers,
    powerLoad: 0,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.planterCost,
    materialsCost: COLONY.build.matPlanter,
    crew: COLONY.build.crewPlanter,
    materialsGen: 0,
    componentsCost: COLONY.build.compPlanter,
    toolsCost: COLONY.build.toolPlanter,
  };
}
function designStall(state: ColonyState): Artifact {
  ...
  return {
    ...
    height: 0.5,
    residents: 0,
    jobs: COLONY.build.stallWorkers,
    powerLoad: 0,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.stallCost,
    materialsCost: COLONY.build.matStall,
    crew: COLONY.build.crewStall,
    materialsGen: 0,
    componentsCost: COLONY.build.compStall,
    toolsCost: COLONY.build.toolStall,
    linenCost: COLONY.build.linenStall,
  };
}
function designFireWatch(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "firewatch",
    color: FIREWATCH_COLOR,
    height: 0.9,
    residents: 0,
    jobs: COLONY.build.fireWatchWorkers,
    powerLoad: COLONY.build.fireWatchPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.fireWatchCost,
    materialsCost: COLONY.build.matFireWatch,
    crew: COLONY.build.crewFireWatch,
    materialsGen: 0,
    componentsCost: COLONY.build.compFireWatch,
    toolsCost: COLONY.build.toolFireWatch,
    reelsCost: COLONY.build.reelFireWatch,
    linenCost: COLONY.build.linenFireWatch,
  };
}
function designReclaimer(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "reclaimer",
    color: RECLAIMER_COLOR,
    height: 0.7,
    residents: 0,
    jobs: COLONY.build.reclaimerWorkers,
    powerLoad: COLONY.build.reclaimerPowerLoad,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.reclaimerCost,
    materialsCost: COLONY.build.matReclaimer,
    crew: COLONY.build.crewReclaimer,
    materialsGen: 0,
    componentsCost: COLONY.build.compReclaimer,
    toolsCost: COLONY.build.toolReclaimer,
    reelsCost: COLONY.build.reelReclaimer,
  };
}
function designFestBoard(state: ColonyState): Artifact {
  ...
  return {
    id: state.buildIds++,
    kind: "festboard",
    color: FESTBOARD_COLOR,
    height: 0.8,
    residents: 0,
    jobs: COLONY.build.festBoardWorkers,
    powerLoad: 0,
    powerGen: 0,
    buildTimeMin: COLONY.build.workplaceBuildHours * 60,
    cost: COLONY.build.festBoardCost,
    materialsCost: COLONY.build.matFestBoard,
    crew: COLONY.build.crewFestBoard,
    materialsGen: 0,
    componentsCost: COLONY.build.compFestBoard,
    toolsCost: COLONY.build.toolFestBoard,
    linenCost: COLONY.build.linenFestBoard,
    folioCost: COLONY.build.folioFestBoard,
  };
}
...
export function turbinePower(state: ColonyState): number {
  ...
  if (n === 0) return 0;
  const staffing =
    state.totalJobs > 0 ? Math.min(1, state.colonists / state.totalJobs) : 0;
...
export function waterTankCap(state: ColonyState): number {
  return (
    state.buildings.filter((b) => b.artifact.kind === "cistern").length *
    COLONY.build.cisternTankCap
  );
}
...
export function waterSupplyFactor(state: ColonyState): number {
  ...
  return (
    COLONY.build.waterSupplyFloor + (1 - COLONY.build.waterSupplyFloor) * t
  );
}

/** Spec 046 — Water readout for the HUD: the tank, its capacity, the cistern count, and whether it is running dry. */
export function waterStatus(state: ColonyState): {
  stored: number;
  cap: number;
  cisterns: number;
  dry: boolean;
  ...
  return {
    stored: Math.round(state.water ?? 0),
    cap: waterTankCap(state),
    cisterns,
    dry: cisterns > 0 && (state.water ?? 0) < COLONY.build.waterComfortBuffer,
  };
}
...
function waterStep(state: ColonyState, dtMin: number): void {
  ...
  const staffing =
    state.totalJobs > 0 ? Math.min(1, state.colonists / state.totalJobs) : 0;
  const cisterns = state.buildings.filter(
    (b) => b.artifact.kind === "cistern",
  ).length;
  const fill =
    cisterns *
    COLONY.build.cisternFillPerDay *
    staffing *
    powerFactor(state) *
    frac; // a brownout slows the heavy condensers
  const homes = state.buildings.filter(
    (b) => b.artifact.kind === "habitat",
  ).length;
  const draw = homes * COLONY.build.waterDrawPerHomePerDay * frac;
  state.water = Math.max(
    0,
    Math.min(waterTankCap(state), (state.water ?? 0) + fill - draw),
  );
  ...
  if (homes > 0 && (state.water ?? 0) < COLONY.build.waterComfortBuffer) {
    ...
    state.outbreak = Math.min(
      1,
      (state.outbreak ?? 0) + COLONY.build.dryTankFeverPerDay * lost * frac,
    );
    state.unrest = Math.min(
      COLONY.build.unrestMax,
      (state.unrest ?? 0) + COLONY.build.dryTankUnrestPerDay * lost * frac,
    );
  }
}
...
function reclaimWaterPerDay(state: ColonyState): number {
  ...
  const filterScale =
    (state.linen ?? 0) > 0 ? 1 : COLONY.build.reclaimNoFilterRate;
  ...
  const treated =
    Math.min(greywater, plants * COLONY.build.reclaimGreywaterCapPerDay) *
    staffing *
    powerScale *
    filterScale;
...
function reclaimStep(state: ColonyState, dtMin: number): void {
  ...
  const returned = Math.min(
    reclaimWaterPerDay(state) * frac,
    Math.max(0, waterTankCap(state) - (state.water ?? 0)),
  );
...
export function reclaimStatus(state: ColonyState): {
  plants: number;
  perDay: number;
  active: boolean;
} {
...
function tooledWorkplaceCount(state: ColonyState): number {
  ...
  for (const b of state.buildings)
    if (TOOLED_KINDS.includes(b.artifact.kind) && !b.incident) n++;
  return n;
}
...
export function toolStatus(state: ColonyState): {
  stored: number;
  cap: number;
  cribs: number;
  short: boolean;
  ...
  return {
    stored: Math.round(state.tools ?? 0),
    cap: toolStockCap(state),
    cribs,
    short: cribs > 0 && (state.tools ?? 0) < COLONY.build.toolComfortBuffer,
  };
}
...
function toolStep(state: ColonyState, dtMin: number): void {
  ...
  const frac = dtMin / (24 * 60);
  const cribs = state.buildings.filter(
    (b) => b.artifact.kind === "toolcrib",
  ).length;
  const staffing =
    state.totalJobs > 0 ? Math.min(1, state.colonists / state.totalJobs) : 0;
  ...
  const wantComp =
    cribs *
    COLONY.build.toolCribComponentsPerDay *
    staffing *
    powerFactor(state) *
    frac; // a brownout slows the bench
  const drawComp = Math.min(
    state.components,
    wantComp,
    headroom / COLONY.build.toolKitsPerComponent,
  );
  if (drawComp > 0) {
  ...
  const draw =
    tooledWorkplaceCount(state) *
    COLONY.build.toolUsePerWorkplacePerDay *
    staffing *
    frac;
  tools -= draw;
...
export function seedStatus(state: ColonyState): {
  stored: number;
  cap: number;
  lofts: number;
  short: boolean;
  ...
  return {
    stored: Math.round(state.seed ?? 0),
    cap: seedStockCap(state),
    lofts,
    short: lofts > 0 && (state.seed ?? 0) < COLONY.build.seedComfortBuffer,
  };
}
...
function seedStep(state: ColonyState, dtMin: number): void {
  ...
  const frac = dtMin / (24 * 60);
  const lofts = state.buildings.filter(
    (b) => b.artifact.kind === "seedloft",
  ).length;
  const staffing =
    state.totalJobs > 0 ? Math.min(1, state.colonists / state.totalJobs) : 0;
  ...
  const wantBatches =
    lofts *
    COLONY.build.seedLoftBatchesPerDay *
    staffing *
    powerFactor(state) *
    frac; // a brownout slows the racks
  ...
  const byWater = cisterns
    ? (state.water ?? 0) / COLONY.build.seedLoftWaterPerBatch
    : Infinity;
  const byHeadroom = headroom / COLONY.build.seedPerBatch;
  const batches = Math.max(
    0,
    Math.min(wantBatches, byFood, byWater, byHeadroom),
  );
  if (batches > 0) {
    state.food = Math.max(
      0,
      state.food - batches * COLONY.build.seedLoftFoodPerBatch,
    );
    if (cisterns)
      state.water = Math.max(
        0,
        (state.water ?? 0) - batches * COLONY.build.seedLoftWaterPerBatch,
      );
  ...
  const farms = state.buildings.filter(
    (b) => b.artifact.kind === "greenhouse" && !b.incident,
  ).length;
...
export function wateredFraction(state: ColonyState): number {
  ...
  for (const h of habs)
    if (
      hubs.some(
        (w) => Math.hypot(w.x - h.x, w.y - h.y) <= COLONY.build.waterHubRadius,
      )
    )
      served++;
...
function nearWater(state: ColonyState, x: number, y: number): boolean {
  return state.buildings.some(
    (b) =>
      b.artifact.kind === "water" &&
      Math.hypot(b.x - x, b.y - y) <= COLONY.build.waterHubRadius,
  );
}
...
export function provisionedFraction(state: ColonyState): number {
  ...
  for (const h of habs)
    if (
      depots.some(
        (d) =>
          Math.hypot(d.x - h.x, d.y - h.y) <= COLONY.build.rationDepotRadius,
      )
    )
      inRange++;
...
export function healthFraction(state: ColonyState): number {
  ...
  for (const h of habs)
    if (
      clinics.some(
        (c) => Math.hypot(c.x - h.x, c.y - h.y) <= COLONY.build.clinicRadius,
      )
    )
      served++;
...
export function homeCultured(
  state: ColonyState,
  home: ColonyBuilding,
): boolean {
  if (nearBuildingKind(state, home, "theatre", COLONY.build.theatreRadius))
    return true;
  return (
    libraryActive(state) &&
    nearBuildingKind(state, home, "library", COLONY.build.libraryRadius)
  );
}
...
export function cultureFuelFactor(state: ColonyState): number {
  ...
  return (theatres && state.reels > 0) || libraryActive(state)
    ? 1
    : COLONY.build.cultureStarvedFactor;
}
...
export function solaceCoverage(state: ColonyState): number {
  ...
  if ((state.totalJobs > 0 ? state.colonists / state.totalJobs : 0) <= 0)
    return 0; // unstaffed → nobody keeps the shrine, no Solace
  const shrines = state.buildings.filter((b) => b.artifact.kind === "shrine");
  let served = 0;
  for (const h of habs)
    if (
      shrines.some(
        (s) => Math.hypot(s.x - h.x, s.y - h.y) <= COLONY.build.shrineRadius,
      )
    )
      served++;
...
export function solaceStatus(state: ColonyState): {
  coverage: number;
  shrines: number;
} {
  return {
    coverage: solaceCoverage(state),
    shrines: countKind(state, "shrine"),
  };
}
...
export function educationFraction(state: ColonyState): number {
  ...
  if ((state.totalJobs > 0 ? state.colonists / state.totalJobs : 0) <= 0)
    return 0; // unstaffed → the room sits dark, teaches no one
  const schools = state.buildings.filter((b) => b.artifact.kind === "school");
  let served = 0;
  for (const h of habs)
    if (
      schools.some(
        (s) => Math.hypot(s.x - h.x, s.y - h.y) <= COLONY.build.schoolRadius,
      )
    )
      served++;
  return served / habs.length;
...
export function educationStatus(state: ColonyState): {
  coverage: number;
  schools: number;
} {
  return {
    coverage: educationFraction(state),
    schools: countKind(state, "school"),
  };
}
...
export function prosperityScore(state: ColonyState): number {
  ...
  const solvency =
    state.treasury >= 0 ? 1 : Math.max(0, 1 - debt / COLONY.build.debtCeiling); // spec 039
  const w = COLONY.build;
  const s =
    live * w.prospLiveabilityWeight +
    tierShare * w.prospTierWeight +
    employment * w.prospEmploymentWeight +
    standing * w.prospStandingWeight +
    solvency * w.prospSolvencyWeight;
...
export function prosperityRank(state: ColonyState): number {
  const base = Math.max(
    0,
    Math.min(4, Math.floor(prosperityScore(state) / 0.2)),
  );
...
function prosperityDesirabilityFactor(state: ColonyState): number {
  ...
  return (
    1 +
    COLONY.build.prosperityImmigrationBonus *
      Math.max(0, prosperityScore(state) - COLONY.build.prosperityBonusFloor)
  );
}

/** Spec 040 — Prosperity readout for the HUD: whether the Hall reads, the score (0..100), the rank + its name, and the milestone. */
export function prosperityStatus(state: ColonyState): {
  active: boolean;
  score: number;
  rank: number;
  rankName: string;
  ...
  return {
    active,
    score: Math.round(prosperityScore(state) * 100),
    rank,
    rankName: PROSPERITY_RANKS[rank],
    recognised: active && rank >= 4,
  };
}
...
function registryStep(state: ColonyState, dtMin: number): void {
  ...
  state.unempHighDays =
    u > COLONY.build.registryHighPct ? (state.unempHighDays ?? 0) + frac : 0;
  state.unempSevereDays =
    u > COLONY.build.registrySeverePct
      ? (state.unempSevereDays ?? 0) + frac
      : 0;
  state.unempClearDays =
    u < COLONY.build.registryClearPct ? (state.unempClearDays ?? 0) + frac : 0;
  let p = state.registryPenalty ?? 0;
  if ((state.unempClearDays ?? 0) >= COLONY.build.registryClearDays)
    p = 0; // hysteresis — sustained full employment lifts the drag
  else if ((state.unempSevereDays ?? 0) >= COLONY.build.registrySevereDays)
    p = Math.max(p, 2);
  else if ((state.unempHighDays ?? 0) >= COLONY.build.registryHighDays)
    p = Math.max(p, 1);
...
export function labourStatus(state: ColonyState): {
  active: boolean;
  unemployment: number;
  covered: number;
  penalty: number;
  ...
  const covered =
    state.colonists > 0
      ? Math.min(
          1,
          (COLONY.build.registryCapacity * registries * staffing) /
            state.colonists,
        )
      : 0;
  return {
    active,
    unemployment: unemploymentRate(state),
    covered,
    penalty: state.registryPenalty ?? 0,
    dragging: (state.registryPenalty ?? 0) > 0,
  };
}

/** Spec 011 — is a building of `kind` within `radius` of this home? (per-home service coverage). */
function nearBuildingKind(
  state: ColonyState,
  home: ColonyBuilding,
  kind: BuildKind,
  radius: number,
  ...
  return state.buildings.some(
    (b) =>
      ...
      Math.hypot(b.x - home.x, b.y - home.y) <= radius,
  );
}
...
export function polluted(state: ColonyState, home: ColonyBuilding): boolean {
  const nearIndustry = state.buildings.some(
    (b) =>
      (b.artifact.kind === "mine" || b.artifact.kind === "foundry") &&
      Math.hypot(b.x - home.x, b.y - home.y) <= COLONY.build.smogRadius,
  );
  if (!nearIndustry) return false;
  return !state.buildings.some(
    (b) =>
      b.artifact.kind === "scrubber" &&
      Math.hypot(b.x - home.x, b.y - home.y) <= COLONY.build.scrubberRadius,
  );
}
...
export function homeLiveability(
  state: ColonyState,
  home: ColonyBuilding,
): number {
  if (home.artifact.kind !== "habitat") return 0;
  const watered = nearBuildingKind(
    state,
    home,
    "water",
    COLONY.build.waterHubRadius,
  )
    ? 1
    : 0;
  const provisioned =
    ...
    nearBuildingKind(state, home, "depot", COLONY.build.rationDepotRadius)
      ? 1
      : 0;
  const healthy = nearBuildingKind(
    state,
    home,
    "clinic",
    COLONY.build.clinicRadius,
  )
    ? 1
    : 0;
  ...
  if (polluted(state, home))
    score = Math.max(0, score - COLONY.build.pollutionPenalty); // spec 019 — smog drags it down
...
export function planterBlooming(b: ColonyBuilding): boolean {
  return (
    b.artifact.kind === "planter" &&
    (b.tend ?? 0) >= COLONY.build.planterBloomDays
  );
}
...
function planterDesirabilityPoints(
  state: ColonyState,
  home: ColonyBuilding,
): number {
  let pts = 0;
  for (const b of state.buildings) {
    ...
    if (d <= COLONY.build.planterNearRadius)
      pts += COLONY.build.planterNearBonus;
    else if (d <= COLONY.build.planterFarRadius)
      pts += COLONY.build.planterFarBonus;
  }
...
export function planterLiveabilityBoost(
  state: ColonyState,
  home: ColonyBuilding,
  serviceFraction: number,
): number {
  return (
    planterDesirabilityPoints(state, home) *
    COLONY.build.planterLiveabilityPerPoint *
    serviceFraction
  );
}
...
function planterStep(state: ColonyState, dtMin: number): void {
  ...
  const season =
    countKind(state, "calendar") > 0
      ? seasonOf(calendarStatus(state).month).name
      : ""; // no calendar → no seasons
  const perDay =
    season === "Grey" || season === "Frost"
      ? COLONY.build.planterWaterCool
      : COLONY.build.planterWaterWarm;
...
export function planterStatus(state: ColonyState): {
  squares: number;
  blooming: number;
} {
  const planters = state.buildings.filter((b) => b.artifact.kind === "planter");
  return {
    squares: planters.length,
    blooming: planters.filter(planterBlooming).length,
  };
}
...
export function colonyHeadlines(state: ColonyState): string[] {
  ...
  const tiers = housingTierCounts(state);
  if (tiers[2] > 0)
    h.push("A district reaches Tier 3 — a home that wants for nothing.");
  if (countKind(state, "theatre") > 0 && state.reels <= 0)
    h.push(
      "The reels run dry; the Holo-Theatres dim until the foundry catches up.",
    );
  else if (state.reels > 0)
    h.push("The foundry's reels gleam — luxury bound for the Skybridge.");
  h.push(
    `Population ${Math.round(state.colonists)}, and the border stays busy.`,
  );
  // Spec 041 — when the colony is shedding people, the Courier names it plainly.
  {
    const d = departureStatus(state);
    if (d.atRisk)
      h.push(
        `Households are leaving the failing decks — ${d.cause} drives them to the moorings.`,
      );
  }
  h.push(`${state.buildings.length} structures stand on the island tonight.`);
  if (state.treasury > 0)
    h.push(
      `The Exchange is paying: the treasury holds $${Math.round(state.treasury).toLocaleString()}.`,
    );
  // the council's voices — each citizen and what they raised
  if (countKind(state, "water") > 0)
    h.push("Mara Venn's Water Hubs keep the far homes flowing.");
  if (countKind(state, "depot") > 0)
    h.push("Ration depots carry food to every door — Mara Venn's doing.");
  if (countKind(state, "clinic") > 0)
    h.push("The clinics keep the work crews on their feet.");
  if (countKind(state, "exchange") > 0)
    h.push(
      "Bram Teel's Skybridge Exchange ships the colony's surplus to the dark.",
    );
  if (countKind(state, "foundry") > 0)
    h.push("Niko Vance's foundry weaves components into luxury reels.");
  if (countKind(state, "survey") > 0)
    h.push("The Civic Pulse is read — the colony can see where it thrives.");
  // Spec 040 — the Census Hall reads the colony's prosperity; the top rank is a milestone the Courier proclaims.
  if (censusActive(state)) {
    const p = prosperityStatus(state);
    h.push(
      p.recognised
        ? "Landing One is named a Recognised Sky-Colony — Prosperity at its height."
        : `The Census Hall reads the colony ${p.rankName} (Prosperity ${p.score}).`,
    );
  }
  ...
  if (foundersHallActive(state)) {
    h.push(
      "The Founders’ Hall seats the Living Roster — the people who built Landing One take their posts.",
    );
    const f =
      FOUNDERS[
        (Math.round(state.colonists) + state.buildings.length) % FOUNDERS.length
      ]; // a different founder each reading, deterministic
    h.push(
      `${f.name} keeps the post of ${f.role} — the colony remembers who carries its burdens.`,
    );
  }
  return h;
}
...
export function liveabilityTint(score: number): number {
  ...
  const ar = 0xe6,
    ag = 0x8a,
    ab = 0x3a; // amber — starved
  const cr = 0x3a,
    cg = 0xd1,
    cb = 0xc8; // cyan — thriving
...
export function inBrownout(state: ColonyState): boolean {
  const pct =
    state.power.batteryCapWh > 0
      ? state.power.batteryWh / state.power.batteryCapWh
      : 0;
  return (
    state.power.loadW > peakSupply(state) &&
    pct < COLONY.build.brownoutBatteryThreshold
  );
}
...
function chooseArtifact(state: ColonyState, rng: RNG): Artifact {
  ...
  if (state.materials < COLONY.build.materialsLowThreshold)
    return designMine(state);
  ...
  if (
    countKind(state, "mine") > 0 &&
    state.materials > COLONY.build.materialsSurplus &&
    countKind(state, "workshop") < countKind(state, "mine") * 2
  )
    return designWorkshop(state);
  // Spec 005 — service the homes: thirsty habitats + components on hand → raise a Water Hub (~1 per 4 homes).
  if (
    countKind(state, "habitat") > 0 &&
    wateredFraction(state) < 0.9 &&
    state.components >= COLONY.build.compWaterHub &&
    countKind(state, "water") < Math.ceil(countKind(state, "habitat") / 4)
  )
    return designWaterHub(state);
  // Spec 007 — feed the colony: a hungry stockpile + components on hand → raise a Skyfarm Greenhouse (~1 per 12 colonists).
  if (
    state.colonists > 4 &&
    state.food < state.colonists * COLONY.build.foodPerColonistPerDay &&
    state.components >= COLONY.build.compGreenhouse &&
    countKind(state, "greenhouse") < Math.ceil(state.colonists / 12)
  )
    return designGreenhouse(state);
  // Spec 008 — get the food to the homes: built homes + food on hand + components → raise a Ration Depot.
  if (
    countKind(state, "habitat") > 0 &&
    state.food > 0 &&
    provisionedFraction(state) < 0.9 &&
    state.components >= COLONY.build.compDepot &&
    countKind(state, "depot") <
      Math.ceil(countKind(state, "habitat") / COLONY.build.rationDepotHomes)
  )
    return designDepot(state);
  ...
  if (
    countKind(state, "habitat") > 0 &&
    housewaresFraction(state) < 0.9 &&
    state.components >= COLONY.build.compMarket &&
    countKind(state, "market") <
      Math.ceil(countKind(state, "habitat") / COLONY.build.marketHomes)
  )
    return designMarket(state);
  // Spec 009 — keep the workers well: homes exist + low health coverage + components → raise a First Aid Clinic.
  if (
    countKind(state, "habitat") > 0 &&
    healthFraction(state) < 0.9 &&
    state.components >= COLONY.build.compClinic &&
    countKind(state, "clinic") < Math.ceil(countKind(state, "habitat") / 6)
  )
    return designClinic(state);
  // Spec 010 — culture for a thriving colony: homes exist + low culture coverage + components → raise a Holo-Theatre.
  if (
    countKind(state, "habitat") > 0 &&
    cultureFraction(state) < 0.9 &&
    state.components >= COLONY.build.compTheatre &&
    countKind(state, "theatre") < Math.ceil(countKind(state, "habitat") / 8)
  )
    return designTheatre(state);
  // Spec 037 — comfort the homes: an established colony with components on hand raises a Mooring Shrine to carry Solace.
  if (
    state.colonists > 8 &&
    countKind(state, "habitat") > 0 &&
    solaceCoverage(state) < 0.9 &&
    state.components >= COLONY.build.compShrine &&
    countKind(state, "shrine") <
      Math.ceil(countKind(state, "habitat") / COLONY.build.shrineHomes)
  )
    return designShrine(state);
  // Spec 042 — teach the homes: an established colony with materials on hand raises a Little Schoolroom for Education.
  if (
    state.colonists > 8 &&
    countKind(state, "habitat") > 0 &&
    educationFraction(state) < 0.9 &&
    state.materials >= COLONY.build.matSchool &&
    countKind(state, "school") <
      Math.ceil(countKind(state, "habitat") / COLONY.build.schoolHomes)
  )
    return designSchool(state);
  // Spec 019 — clear the air: smoggy homes + components on hand → plant an Air Scrubber Garden.
  if (
    pollutedFraction(state) > 0.1 &&
    state.components >= COLONY.build.compScrubber &&
    countKind(state, "scrubber") < Math.ceil(countKind(state, "habitat") / 6)
  )
    return designScrubber(state);
  // Spec 011 — once the colony is established, raise a Civic Pulse Survey Office to unlock the liveability map.
  if (
    state.colonists > 8 &&
    countKind(state, "survey") < 1 &&
    state.components >= COLONY.build.compSurvey
  )
    return designSurvey(state);
  // Spec 012 — when components pile up past the trade reserve, raise a Skybridge Exchange to sell the surplus.
  if (
    state.colonists > 8 &&
    countKind(state, "exchange") < 1 &&
    state.components > COLONY.build.tradeComponentReserve &&
    state.components >= COLONY.build.compExchange
  )
    return designExchange(state);
  // Spec 013 — with components plentiful and an Exchange to sell through, raise a Reel Foundry to refine luxury reels.
  if (
    state.colonists > 10 &&
    countKind(state, "foundry") < 1 &&
    state.components > 40 &&
    state.components >= COLONY.build.compFoundry
  )
    return designFoundry(state);
  // Spec 031 — open the second line only once the core economy is comfortable (materials in surplus) and fibre is short.
  if (
    state.colonists > 12 &&
    state.materials > COLONY.build.materialsSurplus &&
    (state.fibre ?? 0) < COLONY.build.fibreLowThreshold &&
    countKind(state, "skimmer") < COLONY.build.maxSkimmer
  )
    return designSkimmer(state);
  // Spec 031 — weave the surplus: with fibre plentiful, raise a Weavery (up to one per dock) to make linen.
  if (
    countKind(state, "skimmer") > 0 &&
    (state.fibre ?? 0) > COLONY.build.fibreSurplus &&
    countKind(state, "weavery") < countKind(state, "skimmer")
  )
    return designWeavery(state);
  // Spec 044 — bind the two chains: with reels + linen plentiful and an Exchange to sell through, raise a Folio House.
  if (
    state.colonists > 12 &&
    countKind(state, "folio") < 1 &&
    countKind(state, "exchange") > 0 &&
    state.reels > 40 &&
    (state.linen ?? 0) > 30 &&
    state.components >= COLONY.build.compFolio
  )
    return designFolio(state);
  // Spec 020 — train the advanced trades: workshops/foundries up but skilled workers short → raise a Skillhouse Academy.
  if (
    countKind(state, "workshop") + countKind(state, "foundry") > 0 &&
    state.skilled <
      (countKind(state, "workshop") + countKind(state, "foundry")) *
        COLONY.build.skilledPerAdvanced &&
    state.components >= COLONY.build.compAcademy &&
    countKind(state, "academy") < 2
  )
    return designAcademy(state);
  // Spec 016 — once the colony is a real town, raise a Broadcast Mast so the Kookerverse Courier can speak.
  if (
    state.colonists > 12 &&
    countKind(state, "mast") < 1 &&
    state.components >= COLONY.build.compMast
  )
    return designMast(state);
  ...
  if (
    state.colonists > 10 &&
    countKind(state, "levy") < 1 &&
    state.components >= COLONY.build.compLevy &&
    state.reels >= COLONY.build.reelLevy
  )
    return designLevy(state);
  ...
  if (
    state.colonists > 10 &&
    countKind(state, "payoffice") < 1 &&
    state.components >= COLONY.build.compPayOffice &&
    state.reels >= COLONY.build.reelPayOffice
  )
    return designPayOffice(state);
  // Spec 030 — a place to gather: a real town with reels on hand raises a Feast Deck so the council can fund a Civic Feast.
  if (
    state.colonists > 12 &&
    countKind(state, "feast") < 1 &&
    state.components >= COLONY.build.compFeast &&
    state.reels >= COLONY.build.reelFeast
  )
    return designFeast(state);
  // Spec 032 — answer the wider world: an established colony with reels on hand raises a Kookerverse Liaison Office.
  if (
    state.colonists > 12 &&
    countKind(state, "liaison") < 1 &&
    state.components >= COLONY.build.compLiaison &&
    state.reels >= COLONY.build.reelLiaison
  )
    return designLiaison(state);
  // Spec 034 — respect the sky: an established colony raises a Stormwatch Shelter to brace for the Cloudsea Fronts.
  if (
    state.colonists > COLONY.build.frontMinColonists &&
    countKind(state, "stormwatch") < COLONY.build.maxStormwatch &&
    state.components >= COLONY.build.compStormwatch
  )
    return designStormwatch(state);
  // Spec 035 — honour the builders: a mature colony with components on hand raises a Founders' Hall to seat the Living Roster.
  if (
    state.colonists > 14 &&
    countKind(state, "hall") < 1 &&
    state.components >= COLONY.build.compHall
  )
    return designHall(state);
  // Spec 040 — take the colony's measure: a mature colony with components on hand raises a Census Hall to read its Prosperity.
  if (
    state.colonists > 14 &&
    countKind(state, "census") < 1 &&
    state.components >= COLONY.build.compCensus
  )
    return designCensus(state);
  // Spec 046 — make water real: a large, well-powered colony raises a Mist Condenser Cistern (one per ~60 homes) to fill the tanks.
  if (
    state.colonists > 16 &&
    !inBrownout(state) &&
    countKind(state, "habitat") > 0 &&
    countKind(state, "cistern") <
      Math.max(1, Math.ceil(countKind(state, "habitat") / 60)) &&
    state.components >= COLONY.build.compCistern
  )
    return designCistern(state);
  ...
  if (
    state.colonists > 16 &&
    !inBrownout(state) &&
    countKind(state, "cistern") > 0 &&
    countKind(state, "registry") > 0 &&
    countKind(state, "reclaimer") <
      Math.max(1, Math.ceil(state.colonists / 40)) &&
    state.components >= COLONY.build.compReclaimer &&
    state.materials >= COLONY.build.matReclaimer &&
    (state.tools ?? 0) >= COLONY.build.toolReclaimer &&
    state.reels >= COLONY.build.reelReclaimer
  )
    return designReclaimer(state);
  ...
  if (
    ...
    !inBrownout(state) &&
    countKind(state, "mine") > 0 &&
    countKind(state, "workshop") > 0 &&
    state.components >=
      COLONY.build.compToolCrib + COLONY.build.toolCribSpareComponents &&
    countKind(state, "toolcrib") <
      Math.max(1, Math.ceil(tooledWorkplaceCount(state) / 6))
  )
    return designToolCrib(state);
  ...
  if (
    state.colonists > 12 &&
    !inBrownout(state) &&
    countKind(state, "greenhouse") > 0 &&
    state.food > COLONY.build.seedLoftFoodSurplus &&
    state.components >=
      COLONY.build.compSeedLoft + COLONY.build.seedLoftSpareComponents &&
    countKind(state, "seedloft") <
      Math.max(1, Math.ceil(countKind(state, "greenhouse") / 6))
  )
    return designSeedLoft(state);
  ...
  if (
    state.colonists > 14 &&
    countKind(state, "surveycamp") < 1 &&
    (state.claims ?? 0) < COLONY.build.maxClaims &&
    nextBlock(state) === null &&
    state.components >= COLONY.build.compSurveyCamp &&
    state.materials >= COLONY.build.matSurveyCamp + COLONY.build.matPerClaim
  )
    return designSurveyCamp(state);
  ...
  if (
    state.colonists > 18 &&
    countKind(state, "calendar") < 1 &&
    state.components >= COLONY.build.compCalendar &&
    state.reels >= COLONY.build.reelCalendar &&
    state.materials >= COLONY.build.matCalendar
  )
    return designCalendarOffice(state);
  ...
  if (
    state.colonists > 20 &&
    countKind(state, "calendar") > 0 &&
    countKind(state, "hallofnames") < 1 &&
    state.components >= COLONY.build.compHallOfNames &&
    state.materials >= COLONY.build.matHallOfNames
  )
    return designHallOfNames(state);
  ...
  if (
    ...
    !inBrownout(state) &&
    countKind(state, "greenhouse") > 0 &&
    countKind(state, "netdock") <
      Math.max(1, Math.ceil(countKind(state, "greenhouse") / 3)) &&
    state.components >= COLONY.build.compNetDock &&
    state.materials >= COLONY.build.matNetDock + COLONY.build.rimfishSurplus
  )
    return designNetDock(state);
  ...
  if (
    state.colonists > 16 &&
    countKind(state, "netdock") > 0 &&
    countKind(state, "cistern") > 0 &&
    countKind(state, "cellar") < Math.max(1, Math.ceil(state.colonists / 40)) &&
    state.components >= COLONY.build.compCellar &&
    state.materials >= COLONY.build.matCellar &&
    (state.tools ?? 0) >= COLONY.build.toolCellar
  )
    return designCellar(state);
  ...
  if (
    ...
    countKind(state, "greenhouse") > 0 &&
    countKind(state, "netdock") > 0 &&
    (state.tools ?? 0) >= COLONY.build.toolVarietyCounter &&
    countKind(state, "rationvar") <
      Math.max(
        1,
        Math.ceil(state.colonists / COLONY.build.varietyCounterCapacity),
      ) &&
    state.components >= COLONY.build.compVarietyCounter &&
    state.materials >= COLONY.build.matVarietyCounter
  )
    return designVarietyCounter(state);
  ...
  if (
    ...
    countKind(state, "netdock") > 0 &&
    (state.rimfish ?? 0) >
      COLONY.build.dryRackRimfishReserve + COLONY.build.dryRackRimfishPerDay &&
    (state.linen ?? 0) >= COLONY.build.linenDryRack &&
    (state.tools ?? 0) >= COLONY.build.toolDryRack &&
    state.components >= COLONY.build.compDryRack &&
    state.materials >= COLONY.build.matDryRack &&
    countKind(state, "dryrack") <
      Math.max(1, Math.ceil(countKind(state, "netdock") / 2))
  )
    return designDryRack(state);
  ...
  if (
    state.colonists > 14 &&
    (state.waste ?? 0) > COLONY.build.wasteHarmlessBelow * 0.7 &&
    countKind(state, "sanitation") <
      Math.max(
        1,
        Math.ceil(countKind(state, "habitat") / COLONY.build.wasteOccupancyRef),
      ) &&
    state.components >= COLONY.build.compSanitation &&
    state.materials >= COLONY.build.matSanitation
  )
    return designSanitationPost(state);
  ...
  if (
    state.colonists >= COLONY.build.theftPopFloor &&
    state.treasury > COLONY.build.theftTreasuryFloor * 1.2 &&
    countKind(state, "watchnook") < 2 &&
    state.components >= COLONY.build.compWatchNook &&
    state.materials >= COLONY.build.matWatchNook
  )
    return designWatchNook(state);
  // Spec 036 — once trade is established (an Exchange stands) and the bank is flush, raise an Import Office to buy shortages.
  if (
    state.colonists > 12 &&
    countKind(state, "import") < 1 &&
    countKind(state, "exchange") > 0 &&
    state.components >= COLONY.build.compImportOffice &&
    state.treasury > COLONY.build.importOfficeCost
  )
    return designImportOffice(state);
  ...
  if (
    state.colonists > 12 &&
    countKind(state, "payoffice") > 0 &&
    ((state.linen ?? 0) > COLONY.build.stallReserve + 10 ||
      (state.folios ?? 0) > COLONY.build.stallReserve + 10) &&
    countKind(state, "stall") <
      Math.max(1, Math.ceil(state.colonists / COLONY.build.stallServedCap)) &&
    state.components >= COLONY.build.compStall &&
    state.materials >= COLONY.build.matStall &&
    (state.tools ?? 0) >= COLONY.build.toolStall &&
    (state.linen ?? 0) >= COLONY.build.linenStall
  )
    return designStall(state);
  ...
  if (
    state.colonists > 18 &&
    countKind(state, "gallery") < 1 &&
    colonyLiveability(state) > 0.45 &&
    state.components >= COLONY.build.compGallery &&
    state.materials >= COLONY.build.matGallery &&
    (state.tools ?? 0) >= COLONY.build.toolGallery &&
    state.treasury > COLONY.build.galleryCost
  )
    return designGallery(state);
  ...
  if (
    ...
    countKind(state, "porter") < Math.max(1, Math.ceil(state.colonists / 40)) &&
    state.components >= COLONY.build.compPorter &&
    state.materials >= COLONY.build.matPorter &&
    (state.tools ?? 0) >= COLONY.build.toolPorter &&
    (state.reels ?? 0) >= COLONY.build.reelPorter
  )
    return designPorter(state);
  ...
  if (
    state.colonists > 20 &&
    countKind(state, "avatar") < 1 &&
    countKind(state, "hall") > 0 &&
    state.components >= COLONY.build.compAvatar &&
    state.materials >= COLONY.build.matAvatar &&
    (state.tools ?? 0) >= COLONY.build.toolAvatar &&
    (state.reels ?? 0) >= COLONY.build.reelAvatar &&
    state.treasury > COLONY.build.avatarCost
  )
    return designAvatar(state);
  ...
  if (
    state.colonists > 16 &&
    countKind(state, "festboard") < 1 &&
    countKind(state, "calendar") > 0 &&
    countKind(state, "registry") > 0 &&
    countKind(state, "stall") > 0 &&
    countKind(state, "rationvar") > 0 &&
    state.components >= COLONY.build.compFestBoard &&
    state.materials >= COLONY.build.matFestBoard &&
    (state.tools ?? 0) >= COLONY.build.toolFestBoard &&
    (state.linen ?? 0) >= COLONY.build.linenFestBoard &&
    (state.folios ?? 0) >= COLONY.build.folioFestBoard
  )
    return designFestBoard(state);
  // Spec 039 — a mature colony raises a Comptroller's Office so the treasury can ride a hard stretch on managed debt.
  if (
    state.colonists > 14 &&
    countKind(state, "comptroller") < 1 &&
    state.components >= COLONY.build.compComptroller &&
    state.treasury > COLONY.build.comptrollerCost
  )
    return designComptroller(state);
  // Spec 038 — a mature colony raises a Roster Office so the council can prioritise scarce labour by sector.
  if (
    state.colonists > 14 &&
    countKind(state, "roster") < 1 &&
    state.components >= COLONY.build.compRoster
  )
    return designRoster(state);
  ...
  if (
    state.colonists > 14 &&
    countKind(state, "census") > 0 &&
    countKind(state, "payoffice") > 0 &&
    countKind(state, "registry") <
      Math.max(1, Math.ceil(state.colonists / COLONY.build.registryCapacity)) &&
    state.components >= COLONY.build.compRegistry &&
    state.materials >= COLONY.build.matRegistry &&
    (state.tools ?? 0) >= COLONY.build.toolRegistry &&
    (state.folios ?? 0) >= COLONY.build.folioRegistry
  )
    return designRegistry(state);
  ...
  if (
    ...
    countKind(state, "habitat") > 0 &&
    countKind(state, "cistern") > 0 &&
    countKind(state, "planter") <
      Math.max(1, Math.ceil(countKind(state, "habitat") / 4)) &&
    state.components >= COLONY.build.compPlanter &&
    state.materials >= COLONY.build.matPlanter &&
    (state.tools ?? 0) >= COLONY.build.toolPlanter
  )
    return designPlanter(state);
  ...
  if (
    ...
    countKind(state, "cistern") > 0 &&
    (state.water ?? 0) > 0 &&
    countKind(state, "bathhouse") <
      Math.max(1, Math.ceil(state.colonists / COLONY.build.bathServes)) &&
    state.components >= COLONY.build.compBath &&
    state.materials >= COLONY.build.matBath &&
    (state.tools ?? 0) >= COLONY.build.toolBath
  )
    return designBathhouse(state);
  ...
  if (
    state.colonists > 16 &&
    countKind(state, "folio") > 0 &&
    (state.folios ?? 0) > COLONY.build.libraryFoliosPerDay &&
    cultureFraction(state) < 0.9 &&
    countKind(state, "library") < Math.ceil(countKind(state, "habitat") / 8) &&
    state.components >= COLONY.build.compLibrary &&
    state.materials >= COLONY.build.matLibrary &&
    (state.tools ?? 0) >= COLONY.build.toolLibrary
  )
    return designLibrary(state);
  // Spec 026 — answer an outbreak: when fever climbs past the build line and no post contains it → raise a Fever Watch Post.
  if (
    state.outbreak > COLONY.build.feverBuildThreshold &&
    state.components >= COLONY.build.compFeverWatch &&
    countKind(state, "feverwatch") < COLONY.build.maxFeverWatch
  )
    return designFeverWatch(state);
  // Spec 028 — keep the peace: when unrest climbs past the build line and no post patrols → raise a Ward Post.
  if (
    state.unrest > COLONY.build.unrestBuildThreshold &&
    state.components >= COLONY.build.compWard &&
    countKind(state, "ward") < COLONY.build.maxWard
  )
    return designWard(state);
  ...
  if (
    maintenanceUncovered(state) &&
    state.components >= COLONY.build.compMaintShed &&
    countKind(state, "maintshed") <
      Math.ceil(
        state.buildings.filter(isWorking).length / COLONY.build.maintShedCovers,
      )
  )
    return designMaintShed(state);
  // Spec 023 — make room before the surplus spills: a stockpile near its cap + components on hand → raise a Storehouse Platform.
  if (
    storageNearCap(state) &&
    state.components >= COLONY.build.compStorehouse &&
    countKind(state, "storehouse") < COLONY.build.maxStorehouses
  )
    return designStorehouse(state);
  // Spec 024 — answer the bell: a building on fire/at-risk with too little response cover → raise an Emergency Bellhouse.
  if (
    colonyAtRisk(state) &&
    state.components >= COLONY.build.compBellhouse &&
    state.reels >= COLONY.build.reelBellhouse &&
    countKind(state, "bellhouse") < COLONY.build.maxBellhouses
  )
    return designBellhouse(state);
  ...
  if (
    state.colonists > 14 &&
    countKind(state, "cistern") > 0 &&
    countKind(state, "firewatch") <
      Math.max(1, Math.ceil(state.buildings.length / 12)) &&
    state.components >= COLONY.build.compFireWatch &&
    state.materials >= COLONY.build.matFireWatch &&
    (state.tools ?? 0) >= COLONY.build.toolFireWatch &&
    state.reels >= COLONY.build.reelFireWatch &&
    (state.linen ?? 0) >= COLONY.build.linenFireWatch
  )
    return designFireWatch(state);
  ...
  if (
    inBrownout(state) &&
    state.reels >= COLONY.build.reelBattery &&
    state.components >= COLONY.build.compBattery &&
    countKind(state, "battery") < countKind(state, "solar") + 1
  )
    return designBattery(state);
  // Spec 021 — keep the workers moving: when commute demand outruns capacity, raise a Skybridge Transit Depot.
  if (
    Math.min(state.colonists, state.totalJobs) >
      COLONY.build.transitBaseCapacity +
        countKind(state, "transit") * COLONY.build.transitPerDepot &&
    state.components >= COLONY.build.compTransit &&
    countKind(state, "transit") < 5
  )
    return designTransit(state);
  ...
  if (
    state.power.loadW >
      (peakSupply(state) + queuedGen) * COLONY.build.powerHeadroom &&
    state.colonists > 12 &&
    state.components >= COLONY.build.compTurbine &&
    countKind(state, "turbine") < COLONY.build.maxTurbines
  )
    return designTurbine(state);
  if (
    state.power.loadW >
    (peakSupply(state) + queuedGen) * COLONY.build.powerHeadroom
  )
  ...
  if (
    housingCapacity(state) === 0 &&
    state.colonists > COLONY.seed.colonists &&
    !state.jobs.some((j) => j.artifact.kind === "habitat")
  ) {
    return designHabitat(state, rng);
  }
  if (
    state.colonists - (state.totalJobs + pendingJobs) >
    COLONY.build.jobDeficitThreshold
  )
    return designWorkplace(state, rng);
  return designHabitat(state, rng);
}
...
export function autoGrow(state: ColonyState, rng: RNG): boolean {
  if (state.buildings.length + state.jobs.length >= COLONY.build.maxBuildings)
    return false;
  ...
  if (free < COLONY.build.crewMine || state.materials < COLONY.build.matMine)
    return false;
  ...
  state.jobs.push({
    ...
    artifact,
    progress: 0,
    path: roadPath(state, c.x, c.y, lot.x, lot.y),
  });
  return true;
}
...
function sectorDemand(state: ColonyState, sector: Sector): number {
  let d = 0;
  for (const b of state.buildings)
    if (SECTOR_OF[b.artifact.kind] === sector) d += b.artifact.jobs;
  return d;
}
...
export function sectorStaffing(state: ColonyState, sector: Sector): number {
  ...
  if ((state.rosterMode ?? "balanced") === "balanced" || !rosterActive(state))
    return uniform;
  if (state.colonists >= D) return 1; // no shortage: everyone is placed, priority is moot
  const order =
    state.rosterMode === "essentials" ? ESSENTIALS_ORDER : INDUSTRY_ORDER;
...
export function rosterStatus(state: ColonyState): {
  active: boolean;
  mode: "essentials" | "balanced" | "industry";
} {
...
function produceMaterials(state: ColonyState, dtMin: number): void {
  ...
  for (const b of state.buildings) {
    ...
    if (b.vein !== undefined && staffing > 0)
      b.vein = Math.max(0, b.vein - staffing * frac); // spec 052 — a staffed, producing mine spends its vein; an idle one holds
  }
  if (gen <= 0) return;
  state.materials +=
    gen *
    ...
    feverFactor(state) *
    orderFactor(state) *
    toolSupplyFactor(state) *
    frac; // spec 009/017/021/026/028/047 — sick, brownout, congested, fevered, restless or tool-starved mines dig less
}
...
export function veinStatus(state: ColonyState): {
  mines: number;
  poorest: number;
} {
  let mines = 0;
  let poorest = 1;
  for (const b of state.buildings)
    if (b.artifact.kind === "mine") {
      mines++;
      const f = veinFactor(b);
      if (f < poorest) poorest = f;
    }
...
function skillFactor(state: ColonyState): number {
  ...
  return (
    0.6 +
    0.4 * Math.min(1, Math.min(state.colonists, state.skilled) / advancedNeed)
  );
}
...
export function commute(state: ColonyState): {
  demand: number;
  capacity: number;
  congested: boolean;
} {
...
function maintFactor(b: ColonyBuilding): number {
  ...
  return Math.max(
    COLONY.build.maintFloor,
    1 - t * (1 - COLONY.build.maintFloor),
  );
}
...
function maintenanceStep(state: ColonyState, dtMin: number): void {
  ...
  const staffed =
    sheds.length > 0 &&
    (state.totalJobs > 0 ? state.colonists / state.totalJobs : 0) > 0;
  for (const b of state.buildings) {
    ...
    if (
      staffed &&
      sheds.some(
        (s) => Math.hypot(s.x - b.x, s.y - b.y) <= COLONY.build.maintRadius,
      )
    ) {
...
export function maintenanceStatus(state: ColonyState): {
  worstWear: number;
  needing: number;
  sheds: number;
} {
...
function maintenanceUncovered(state: ColonyState): boolean {
  ...
  return state.buildings.some(
    (b) =>
      isWorking(b) &&
      (b.wear ?? 0) > COLONY.build.wearBuildThreshold &&
      !sheds.some(
        (s) => Math.hypot(s.x - b.x, s.y - b.y) <= COLONY.build.maintRadius,
      ),
  );
...
export function storageCaps(state: ColonyState): {
  materials: number;
  components: number;
  food: number;
  reels: number;
  ...
  return {
    materials:
      COLONY.build.storeBaseMaterials + n * COLONY.build.storePerMaterials,
    components:
      COLONY.build.storeBaseComponents + n * COLONY.build.storePerComponents,
    ...
    driedFish:
      COLONY.build.storeBaseDriedFish + n * COLONY.build.storePerDriedFish, // spec 061
...
export function storageStatus(state: ColonyState): {
  fill: number;
  full: boolean;
  tightest: string;
} {
...
function storageNearCap(state: ColonyState): boolean {
  ...
  return (
    state.materials > cap.materials * th ||
    state.components > cap.components * th ||
    state.food > cap.food * th ||
    state.reels > cap.reels * th
  );
}
...
function buildingHazard(state: ColonyState, b: ColonyBuilding): number {
  ...
  return (
    COLONY.build.hazardBasePerDay *
    ((wear - COLONY.build.hazardWearThreshold) /
      (1 - COLONY.build.hazardWearThreshold))
  );
}
...
function incidentResource(
  kind: BuildKind,
): "materials" | "components" | "food" | "reels" {
  ...
  if (active.length > 0) {
    ...
    active.forEach((b, i) => {
      ...
      if (i < capacity) {
        ...
        b.wear = Math.min(
          1,
          (b.wear ?? 0) + COLONY.build.incidentResolveWearBump,
        );
      } else {
        ...
        if (res === "materials")
          state.materials = Math.max(
            0,
            state.materials - state.materials * loss,
          );
        else if (res === "components")
          state.components = Math.max(
            0,
            state.components - state.components * loss,
          );
        else if (res === "food")
          state.food = Math.max(0, state.food - state.food * loss);
...
export function incidentStatus(state: ColonyState): {
  active: number;
  capacity: number;
} {
  let active = 0;
...
function colonyAtRisk(state: ColonyState): boolean {
  return state.buildings.some(
    (b) => b.incident || (isWorking(b) && buildingHazard(state, b) > 0),
  );
}
...
function inFireDistrict(state: ColonyState, b: ColonyBuilding): boolean {
  return state.buildings.some(
    (p) =>
      p.artifact.kind === "firewatch" &&
      Math.hypot(p.x - b.x, p.y - b.y) <= COLONY.build.fireWatchRadius,
  );
}
...
function fireAdjacentCount(state: ColonyState, b: ColonyBuilding): number {
  ...
  for (const o of state.buildings)
    if (
      o !== b &&
      Math.hypot(o.x - b.x, o.y - b.y) <= COLONY.build.fireAdjacency
    )
      n++;
  return n;
}
...
function fireRiskPoints(state: ColonyState, b: ColonyBuilding): number {
  let pts = 0;
  if ((b.wear ?? 0) > COLONY.build.wearBuildThreshold)
    pts += COLONY.build.fireWornPoints; // spec 022
  if (inBrownout(state)) pts += COLONY.build.fireBrownoutPoints; // spec 017
  let warm = false;
  if (countKind(state, "calendar") > 0) {
    const n = seasonOf(calendarStatus(state).month).name;
    warm = n === "Bloom" || n === "Highsun";
  } // spec 054
  if (warm) pts += COLONY.build.fireWarmPoints;
  if (fireHazardousKind(b.artifact.kind))
    pts += COLONY.build.fireHazardKindPoints;
  if (storageNearCap(state)) pts += COLONY.build.fireFullStorePoints; // spec 023
  if (fireAdjacentCount(state, b) >= COLONY.build.fireCrowdedNeighbours)
    pts += COLONY.build.fireCrowdedPoints;
  return pts;
}
...
function mostFlammableNeighbour(
  state: ColonyState,
  b: ColonyBuilding,
): ColonyBuilding | null {
...
function fireStep(state: ColonyState, dtMin: number): void {
  ...
  state.water = Math.max(
    0,
    (state.water ?? 0) - COLONY.build.fireWatchWaterPerDay * posts * frac,
  ); // the Posts draw barrel water
  ...
  for (const b of state.buildings) {
    ...
    const suppress =
      protectedColony && inFireDistrict(state, b)
        ? COLONY.build.fireSuppressStrength * safety
        : 0;
    b.fire = b.fire + dtMin * (1 - suppress);
    if (b.fire <= 0) {
      b.fire = undefined;
      b.fireSpread = undefined;
      continue;
    } // put out
  ...
  for (const b of state.buildings) {
    if (b.fire) continue;
    if (!inFireDistrict(state, b)) {
      b.fireRisk = 0;
      continue;
    }
    const accrue = fireRiskPoints(state, b) * COLONY.build.fireRiskPerPoint;
    const drain = protectedColony
      ? COLONY.build.fireWatchDrainPerDay * safety
      : 0;
    b.fireRisk = Math.max(
      0,
      Math.min(
        COLONY.build.fireIgniteThreshold,
        (b.fireRisk ?? 0) + (accrue - drain) * frac,
      ),
    );
    if ((b.fireRisk ?? 0) > worstRisk) {
      worstRisk = b.fireRisk ?? 0;
      worst = b;
    }
  }
  if (
    worst &&
    worstRisk >= COLONY.build.fireIgniteThreshold &&
    (state.fireCooldown ?? 0) <= 0
  ) {
...
export function fireStatus(state: ColonyState): {
  posts: number;
  active: number;
  risk: number;
  watered: boolean;
  ...
  return {
    posts,
    active,
    risk: Math.round((maxRisk / COLONY.build.fireIgniteThreshold) * 100),
    watered: (state.water ?? 0) > 0,
  };
}
...
export function levyStatus(state: ColonyState): {
  active: boolean;
  rate: "low" | "normal" | "high";
} {
...
function feverPressure(state: ColonyState): number {
  const habs = state.buildings.filter(
    (b) => b.artifact.kind === "habitat",
  ).length;
  if (habs === 0) return 0;
  ...
  const crowding =
    occupancy <= th ? 0 : Math.min(1, (occupancy - th) / (1 - th)); // only a packed colony feeds the fever
...
function feverStep(state: ColonyState, dtMin: number): void {
  ...
  if (feverWatchActive(state)) {
    ...
    o +=
      COLONY.build.feverSpreadPerDay *
      feverPressure(state) *
      (1 - COLONY.build.bathHygieneRelief * hygieneLevel(state)) *
      frac; // spreads while the colony stays sick; spec 069 — a clean colony (hygiene) slows how fast it takes hold (multiplier is 1 with no Bathhouse)
...
function feverFactor(state: ColonyState): number {
  ...
  if (o <= 0) return 1;
  const severity =
    COLONY.build.feverPenalty *
    (1 - COLONY.build.feverClinicRelief * healthFraction(state));
...
export function feverStatus(state: ColonyState): {
  outbreak: number;
  contained: boolean;
} {
...
export function hygieneLevel(state: ColonyState): number {
  ...
  const coverage = Math.min(
    1,
    (baths * COLONY.build.bathServes) / Math.max(1, state.colonists),
  );
...
export function bathStep(state: ColonyState, dtMin: number): void {
  ...
  state.water = Math.max(
    0,
    (state.water ?? 0) - COLONY.build.bathWaterPerDay * baths * frac,
  ); // the bathhouse draw on the tanks
}
...
export function bathhouseStatus(state: ColonyState): {
  hygiene: number;
  baths: number;
  drawBonus: number;
  climbBonus: number;
  ...
  return {
    hygiene: hygieneLevel(state),
    baths: countKind(state, "bathhouse"),
    drawBonus: hygieneDesirabilityFactor(state) - 1,
    climbBonus: 1 - hygieneEvolutionFactor(state),
  };
}
...
export function libraryStep(state: ColonyState, dtMin: number): void {
  ...
  const frac = dtMin / (24 * 60);
  state.folios = Math.max(
    0,
    (state.folios ?? 0) - COLONY.build.libraryFoliosPerDay * libs * frac,
  ); // the day's lending, drawn from the stores
}
...
export function libraryStatus(state: ColonyState): {
  libraries: number;
  lending: boolean;
  foliosPerDay: number;
} {
  ...
  return {
    libraries,
    lending: libraryActive(state),
    foliosPerDay:
      libraries > 0 ? COLONY.build.libraryFoliosPerDay * libraries : 0,
  };
}
...
function unrestPressure(state: ColonyState): number {
  ...
  const employed = Math.min(state.colonists, state.totalJobs);
  const jobless =
    state.colonists > 0 ? (state.colonists - employed) / state.colonists : 0;
  const unemployment = Math.max(
    0,
    (jobless - COLONY.build.unrestJoblessThreshold) /
      (1 - COLONY.build.unrestJoblessThreshold),
  );
  if (unemployment <= 0) return 0;
  const hardship = Math.max(
    levyActive(state) && state.levyRate === "high" ? 1 : 0,
    inBrownout(state) ? 1 : 0,
    lowWageBites(state) ? 1 : 0,
  ); // spec 029 — a low wage is a squeeze too
...
function unrestStep(state: ColonyState, dtMin: number): void {
  ...
  if (
    liaisonActive(state) &&
    (state.standing ?? 0.5) < COLONY.build.lowStandingThreshold
  )
...
function orderFactor(state: ColonyState): number {
  ...
  if (u <= 0) return 1;
  return Math.max(
    COLONY.build.unrestProductionFloor,
    1 - u * COLONY.build.unrestProductionPenalty,
  );
}

/** Spec 028 — order readout for the HUD: the unrest level (0..1) and whether a Ward Post is keeping the peace. */
export function unrestStatus(state: ColonyState): {
  unrest: number;
  warded: boolean;
} {
...
export function wageStatus(state: ColonyState): {
  active: boolean;
  rate: "low" | "standard" | "generous";
  payroll: number;
} {
  return {
    active: payOfficeActive(state),
    rate: state.wageRate,
    payroll: Math.round(payrollPerDay(state)),
  };
}
...
function feastStep(state: ColonyState, dtMin: number): void {
  ...
  if (
    ...
    state.treasury >
      COLONY.build.feastTreasuryCost * COLONY.build.feastAutoTreasuryMargin &&
    state.food >= COLONY.build.feastFoodCost &&
    state.components >= COLONY.build.feastWaresCost
  ) {
    callFeast(state);
...
export function feastStatus(state: ColonyState): {
  active: boolean;
  daysLeft: number;
  canCall: boolean;
} {
  return {
    active: feasting(state),
    daysLeft: Math.ceil((state.feastTimer ?? 0) / (24 * 60)),
    canCall: canCallFeast(state),
  };
}
...
function spendGood(
  ...
  if (good === "components")
    state.components = Math.max(0, state.components - amount);
  else if (good === "linen")
    state.linen = Math.max(0, (state.linen ?? 0) - amount);
...
function makeRequest(state: ColonyState): {
  good: RequestGood;
  amount: number;
  deadline: number;
} {
  ...
  return {
    good: best,
    amount: requestQuota(best),
    deadline: COLONY.build.requestDeadlineDays * 24 * 60,
  };
}
...
export function fulfillRequest(state: ColonyState): boolean {
  ...
  state.standing = Math.min(
    1,
    (state.standing ?? 0.5) +
      COLONY.build.standingReward *
        (spireComplete(state) ? COLONY.build.spireStandingMult : 1) *
        (foundersHallActive(state) ? COLONY.build.foundersStandingMult : 1),
  ); // spec 033/035 — the Beacon makes the world notice; the seated founders are a face to it
  state.request = null;
...
function requestStep(state: ColonyState, dtMin: number): void {
  ...
  if (!liaisonActive(state)) {
    ...
    if (s > 0.5)
      state.standing = Math.max(
        0.5,
        s - COLONY.build.standingDriftPerDay * frac,
      );
    else if (s < 0.5)
      state.standing = Math.min(
        0.5,
  ...
  if (state.request) {
    ...
    if (state.request.deadline <= 0 && !fulfillRequest(state)) {
      state.standing = Math.max(
        0,
        (state.standing ?? 0.5) - COLONY.build.standingPenalty,
      ); // missed → standing falls
      state.request = null;
      state.requestCooldown = COLONY.build.requestIntervalDays * 24 * 60;
    }
    return;
...
function standingDesirabilityFactor(state: ColonyState): number {
  ...
  return (
    COLONY.build.standingDesireLow +
    (COLONY.build.standingDesireHigh - COLONY.build.standingDesireLow) * s
  );
}

/** Spec 032 — Kookerverse readout for the HUD: the channel, the standing, any open request, and whether it's affordable. */
export function liaisonStatus(state: ColonyState): {
  active: boolean;
  standing: number;
  ...
  return {
    ...
    request: req
      ? {
          good: req.good,
          amount: req.amount,
          daysLeft: Math.ceil(req.deadline / (24 * 60)),
        }
      : null,
...
function spireStageBundle(stage: number): {
  treasury: number;
  materials: number;
  components: number;
  reels: number;
...
function spireAfford(state: ColonyState, margin: number): boolean {
  ...
  if (stage >= COLONY.build.spireStageCount || state.spireBuilding)
    return false;
...
function spireStep(state: ColonyState, dtMin: number): void {
  if (state.spireBuilding) {
    state.spireProgress =
      (state.spireProgress ?? 0) +
      dtMin / (COLONY.build.spireStageBuildHours * 60);
  ...
  if (
    ...
    state.treasury >=
      spireStageBundle(state.spireStage ?? 0).treasury +
        COLONY.build.spireTreasuryMargin &&
...
export function spireStatus(state: ColonyState): {
  stage: number;
  total: number;
  progress: number;
  building: boolean;
  ...
  return {
    stage: state.spireStage ?? 0,
    total: COLONY.build.spireStageCount,
    progress: state.spireProgress ?? 0,
    building: state.spireBuilding ?? false,
    complete: spireComplete(state),
  };
}
...
export function foundersRoster(
  state: ColonyState,
...
export function foundersStatus(state: ColonyState): {
  active: boolean;
  seated: number;
  ...
  return {
    active,
    seated: active ? FOUNDERS.length : 0,
    notable: active ? FOUNDERS[7] : null,
  }; // Tessa Quill, who raised the Hall
}
...
function frontWarningWindow(state: ColonyState): number {
  return (
    COLONY.build.frontWarningMin *
    (spireComplete(state) ? COLONY.build.spireBeaconWarnMult : 1)
  );
}
...
function frontStep(state: ColonyState, dtMin: number): void {
  ...
  state.materials = Math.max(
    0,
    state.materials - state.materials * COLONY.build.frontGoodsLoss * severity,
  ); // stored goods spoil
  state.components = Math.max(
    0,
    state.components -
      state.components * COLONY.build.frontGoodsLoss * severity,
  );
  for (const b of state.buildings) {
    if (!isWorking(b)) continue;
    b.wear = Math.min(
      1,
      (b.wear ?? 0) + COLONY.build.frontWearDamage * severity,
    ); // exposed buildings batter — the Maintenance Sheds (022) then matter
  }
...
export function frontStatus(state: ColonyState): {
  timerDays: number;
  incoming: boolean;
  braced: boolean;
  watching: boolean;
  ...
  return {
    timerDays: Math.max(0, Math.ceil(t / (24 * 60))),
    incoming,
    braced: incoming && watching,
    watching,
    established,
  };
}
...
function academyStep(state: ColonyState, dtMin: number): void {
  ...
  state.skilled = Math.min(
    state.colonists,
    state.skilled +
      academies *
        COLONY.build.academyTrainPerDay *
        staffing *
        (1 + COLONY.build.educationAcademyBonus * educationFraction(state)) *
        (dtMin / (24 * 60)),
  ); // spec 042 — a schooled populace learns the advanced trades faster
}
...
function produceComponents(state: ColonyState, dtMin: number): void {
  const eff =
    ...
    feverFactor(state) *
    orderFactor(state) *
    toolSupplyFactor(state); // spec 009/017/020/021/026/028/038/047 — sick, brownout, unskilled, congested, fevered, restless, sector-deprioritised or tool-starved workshops refine less
  if (eff <= 0) return;
  const day = 24 * 60;
  for (const b of state.buildings) {
    ...
    state.components +=
      COLONY.build.workshopComponentsOut *
      eff *
      mf *
      (dtMin / day) *
      (consume / need);
...
function produceReels(state: ColonyState, dtMin: number): void {
  const eff =
    ...
    transitFactor(state) *
    feverFactor(state) *
    orderFactor(state); // spec 009/017/020/021/026/028/038 — sick, brownout, unskilled, congested, fevered, restless or sector-deprioritised foundries weave less
  if (eff <= 0) return;
  const day = 24 * 60;
  for (const b of state.buildings) {
...
function produceFibre(state: ColonyState, dtMin: number): void {
  let gen = 0;
  for (const b of state.buildings)
    if (b.artifact.kind === "skimmer" && !b.incident)
      gen += (b.artifact.fibreGen ?? 0) * maintFactor(b);
  if (gen <= 0) return;
...
function produceRimfish(state: ColonyState, dtMin: number): void {
  let gen = 0;
  for (const b of state.buildings)
    if (b.artifact.kind === "netdock" && !b.incident)
      gen += (b.artifact.rimfishGen ?? 0) * maintFactor(b);
  if (gen <= 0) return;
...
function produceDuskcap(state: ColonyState, dtMin: number): void {
  ...
  let gen = 0;
  for (const b of state.buildings)
    if (b.artifact.kind === "cellar" && !b.incident)
      gen += (b.artifact.duskcapGen ?? 0) * maintFactor(b);
  if (gen <= 0) return;
  const frac = dtMin / (24 * 60);
  ...
  state.water = Math.max(
    0,
    (state.water ?? 0) - COLONY.build.cellarWaterPerDay * cellars * frac,
  ); // the damp-line draw
}

/** Spec 068 — Duskcap readout for the HUD: the stock and the Cellar count. */
export function duskcapStatus(state: ColonyState): {
  stock: number;
  cellars: number;
} {
  return {
    stock: Math.round(state.duskcap ?? 0),
    cellars: countKind(state, "cellar"),
  };
}

/** Spec 056 — Rimfish readout for the HUD: the stock, the dock count, and whether the colony is offering a varied table. */
export function rimfishStatus(state: ColonyState): {
  stock: number;
  docks: number;
  varied: boolean;
} {
  return {
    stock: Math.round(state.rimfish ?? 0),
    docks: countKind(state, "netdock"),
    varied: (state.rimfish ?? 0) > 0,
  };
}
...
function produceDriedFish(state: ColonyState, dtMin: number): void {
  const eff =
    ...
    transitFactor(state) *
    feverFactor(state) *
    orderFactor(state); // spec 038 — sick, brownout, congested, fevered, restless or sector-deprioritised racks dry less
  if (eff <= 0) return;
  const day = 24 * 60;
  const cap = storageCaps(state).driedFish;
  const ratio =
    COLONY.build.dryRackOutputPerDay / COLONY.build.dryRackRimfishPerDay; // dried produced per fresh consumed (a trimming loss)
  for (const b of state.buildings) {
    ...
    const surplus = Math.max(
      0,
      (state.rimfish ?? 0) - COLONY.build.dryRackRimfishReserve,
    ); // only ever dry the catch above the working reserve
...
export function driedFishStatus(state: ColonyState): {
  stock: number;
  cap: number;
  racks: number;
} {
  return {
    stock: Math.round(state.driedFish ?? 0),
    cap: storageCaps(state).driedFish,
    racks: countKind(state, "dryrack"),
  };
}
...
export function wasteDesirabilityFactor(state: ColonyState): number {
  const over = Math.max(
    0,
    (state.waste ?? 0) - COLONY.build.wasteHarmlessBelow,
  );
...
export function wasteStatus(state: ColonyState): {
  level: number;
  posts: number;
  harmful: boolean;
  fevered: boolean;
  ...
  return {
    level: Math.round(w * 100),
    posts: countKind(state, "sanitation"),
    harmful: w >= COLONY.build.wasteHarmlessBelow,
    fevered: w >= COLONY.build.wasteFeverThreshold,
  };
}
...
function theftActive(state: ColonyState): boolean {
  if (
    (state.treasury ?? 0) <= COLONY.build.theftTreasuryFloor ||
    state.colonists < COLONY.build.theftPopFloor
  )
    return false; // poor or small → nothing worth taking
  if (
    inBrownout(state) ||
    frontStatus(state).incoming ||
    (state.food ?? 0) <= 0
  )
    return false; // a storm/shortage — thieves lie low
  return (
    1 - COLONY.build.watchSuppressionPerPost * staffedWatchNooks(state) > 0
  );
}

/** Spec 059 — Security readout for the HUD: whether theft is active, the small daily loss, and the Watch Nook count. */
export function securityStatus(state: ColonyState): {
  active: boolean;
  lossPerDay: number;
  nooks: number;
  guarded: boolean;
  ...
  const watchFactor = Math.max(
    0,
    1 - COLONY.build.watchSuppressionPerPost * staffedWatchNooks(state),
  );
  const lossPerDay = active
    ? Math.min(
        (state.treasury ?? 0) * COLONY.build.theftRatePerDay,
        COLONY.build.theftCapPerDay,
      ) * watchFactor
    : 0;
  return {
    active,
    lossPerDay: Math.round(lossPerDay * 100) / 100,
    nooks,
    guarded: staffedWatchNooks(state) > 0,
  };
}
...
export function wasteStep(state: ColonyState, dtMin: number): void {
  ...
  if (homes > 0) {
    ...
    const tierF =
      1 +
      Math.max(0, habitatMeanTier(state) - 1) * COLONY.build.wasteTierWeight;
    const seasonF =
      countKind(state, "calendar") > 0
        ? calendarStatus(state).month <= 6
          ? COLONY.build.wasteWarmSeason
          : COLONY.build.wasteColdSeason
        : 1; // warm months (1-6) ripen filth faster
  ...
  const staffing =
    state.totalJobs > 0 ? Math.min(1, state.colonists / state.totalJobs) : 0;
  waste -=
    (posts * COLONY.build.wasteClearPerPostPerDay * staffing +
      COLONY.build.wasteNaturalDecay) *
    frac;
  ...
  if ((state.waste ?? 0) >= COLONY.build.wasteFeverThreshold && homes > 0) {
    state.outbreak = Math.min(
      1,
      (state.outbreak ?? 0) +
        COLONY.build.wasteFeverPerDay *
          (state.waste - COLONY.build.wasteFeverThreshold) *
          frac,
    );
  }
}
...
function produceLinen(state: ColonyState, dtMin: number): void {
  const eff =
    ...
    transitFactor(state) *
    feverFactor(state) *
    orderFactor(state); // spec 038 — sick, brownout, congested, fevered, restless or sector-deprioritised weaveries weave less
  if (eff <= 0) return;
  const day = 24 * 60;
  for (const b of state.buildings) {
...
function produceFolios(state: ColonyState, dtMin: number): void {
  const eff =
    ...
    transitFactor(state) *
    feverFactor(state) *
    orderFactor(state); // sick, brownout, congested, fevered, restless or sector-deprioritised binderies bind less
  if (eff <= 0) return;
  const day = 24 * 60;
...
export function housingCapacity(state: ColonyState): number {
  ...
  for (const b of state.buildings)
    if (b.artifact.kind === "habitat")
      cap += b.artifact.residents + housingTierBonus(b.tier);
  return cap;
}
...
export function housingTierCounts(
  state: ColonyState,
): [number, number, number] {
  const c: [number, number, number] = [0, 0, 0];
  for (const b of state.buildings)
    if (b.artifact.kind === "habitat")
      c[Math.max(1, Math.min(3, b.tier ?? 1)) - 1]++;
  return c;
}
...
function habitatMeanTier(state: ColonyState): number {
  ...
  for (const b of state.buildings)
    if (b.artifact.kind === "habitat") {
      n++;
      sum += Math.max(1, Math.min(3, b.tier ?? 1));
    }
  return n > 0 ? sum / n : 1;
}
...
export function housewaresSupplied(
  state: ColonyState,
  home: ColonyBuilding,
): boolean {
...
export function luxurySupplied(
  state: ColonyState,
  home: ColonyBuilding,
): boolean {
...
export function housewaresFraction(state: ColonyState): number {
  ...
  for (const h of habs)
    if (
      markets.some(
        (m) => Math.hypot(m.x - h.x, m.y - h.y) <= COLONY.build.marketRadius,
      )
    )
      inRange++;
...
function housingStep(state: ColonyState, dtMin: number): void {
  ...
  if (
    state.housingTimer <
    COLONY.build.housingUpgradeIntervalHours *
      60 *
      varietyEvolutionFactor(state) *
      hygieneEvolutionFactor(state)
  )
    return;
  ...
  for (const b of state.buildings) {
    ...
    const served =
      fullyServed(state, b) &&
      luxurySupplied(state, b) &&
      (state.linen ?? 0) > 0; // spec 015/027/031 — the whole stack, luxury wares, AND linen for the top tier
    // Climb: T1→T2 on water, T2→T3 on the full service stack + luxury wares — each step spends spare components.
    const canClimb =
      b.tier < 3 &&
      (b.tier === 1 ? watered : served) &&
    ...
    if (canClimb) {
      ...
      if (
        b.dryMin >= COLONY.build.housingDevolveGraceHours * 60 &&
        b.tier > 1
      ) {
        b.tier--;
        b.dryMin = 0;
...
export function colonyDistress(state: ColonyState): number {
  ...
  if (arrearsStrain(state))
    d = Math.max(d, COLONY.build.departureArrearsDistress); // missed wages bite on their own
  return Math.min(1, d);
}
...
function departureStep(state: ColonyState, dtMin: number): void {
  ...
  if (
    ...
    state.departurePressure = Math.max(
      0,
      p - COLONY.build.departureDrainPerDay * frac,
    ); // nothing to lose → bleed off
    return;
  }
  if (distress > 0)
    p += COLONY.build.departureRisePerDay * distress * frac; // failed homes fray, slowly
  ...
  if (p >= 1) {
    ...
    const leave = Math.min(
      COLONY.build.departureHouseholdSize,
      state.colonists - COLONY.seed.colonists,
    );
    state.colonists = Math.max(COLONY.seed.colonists, state.colonists - leave);
    state.standing = Math.max(
      0,
      (state.standing ?? 0.5) - COLONY.build.exodusStandingHit,
    ); // the wider world notices an exodus
...
export function departureStatus(state: ColonyState): {
  pressure: number;
  atRisk: boolean;
  cause: string;
} {
...
export function immigration(state: ColonyState, dtMin: number): void {
  ...
  if (powerDead) {
    state.colonists = Math.max(
      COLONY.seed.colonists,
      state.colonists - COLONY.build.emigrationPerDay * perDay,
    );
    return;
  }
  const cap = housingCapacity(state);
  ...
  const reach =
    countKind(state, "depot") > 0
      ? provisionedFraction(state)
      : state.food > 0
        ? 0.5
        : 0;
  ...
  const cultureFactor =
    1 +
    COLONY.build.cultureDesirabilityBonus *
      cultureFraction(state) *
      cultureFuelFactor(state);
  const desirability =
    Math.max(0.25, wateredFraction(state)) *
    fedFactor *
    tierFactor *
    cultureFactor *
    levyDesirabilityFactor(state) *
    (1 - (state.outbreak ?? 0) * COLONY.build.feverEmigrationWeight) *
    (1 + COLONY.build.waresDesirabilityBonus * housewaresFraction(state)) *
    (1 - (state.unrest ?? 0) * COLONY.build.unrestDesirabilityWeight) *
    wageDesirabilityFactor(state) *
    (feasting(state) ? 1 + COLONY.build.feastDesirabilityBonus : 1) *
    standingDesirabilityFactor(state) *
    (spireComplete(state) ? COLONY.build.spireImmigrationBonus : 1) *
    (foundersHallActive(state) ? COLONY.build.foundersDesirabilityBonus : 1) *
    (1 + COLONY.build.solaceDesirabilityBonus * solaceCoverage(state)) *
    (arrearsStrain(state) ? COLONY.build.arrearsStrainDesirabilityFactor : 1) *
    (1 + COLONY.build.educationDesirabilityBonus * educationFraction(state)) *
    prosperityDesirabilityFactor(state) *
    ((state.rimfish ?? 0) > 0 ? 1 + COLONY.build.rimfishDesirabilityBonus : 1) *
    wasteDesirabilityFactor(state) *
    varietyDesirabilityFactor(state) *
    planterDesirabilityFactor(state) *
    hygieneDesirabilityFactor(state); // spec 025/026/027/028/029/030/032/033/035/037/039/042/040/056/058/060/063/070 — levy, outbreak, stocked homes, unrest, wages, a feast, Kookerverse standing, the Spire, named founders, a consoled colony, a colony deep in arrears, a schooled colony, a high-Prosperity colony, a varied table (rimfish), a colony that minds its drains, a colony whose Variety Ration Counter keeps a Varied Diet, and a clean colony (hygiene) all pull on who comes and stays
  if (state.colonists < cap)
    state.colonists = Math.min(
      cap,
      state.colonists +
        COLONY.build.immigrationPerDay *
          desirability *
          confidenceImmigrationFactor(state) *
          perDay,
    ); // spec 049 — arrivals also follow the colony's Settler Confidence (a healthy colony sits at the plateau, so this is 1; deep distress slows it, terrible distress halts it)
}
...
export function settlerConfidence(state: ColonyState): number {
  ...
  const wageGap = ws.active
    ...
    : 0;
  const cheer =
    (state.festivalCheer ?? 0) > 0 ? (state.festivalCheerBonus ?? 0) / 100 : 0; // spec 067 — Lantern Cheer lifts confidence for its window (0 with no festival)
  const c =
    1 -
...
export function confidenceStatus(state: ColonyState): {
  confidence: number;
  factor: number;
  slowed: boolean;
  halted: boolean;
  ...
  return {
    confidence: settlerConfidence(state),
    factor,
    slowed: factor < 1,
    halted: factor <= 0,
  };
...
function birthStability(state: ColonyState): number {
  ...
  const calm = Math.max(
    0,
    1 - (state.unrest ?? 0) / COLONY.build.birthCalmUnrest,
  );
...
function birthableHomes(state: ColonyState): number {
  ...
  for (const b of state.buildings)
    if (
      b.artifact.kind === "habitat" &&
      (b.tier ?? 1) >= COLONY.build.birthMinTier
    )
      n++;
  return n;
}
...
export function birthStatus(state: ColonyState): {
  children: number;
  homes: number;
  growing: boolean;
} {
...
function birthStep(state: ColonyState, dtMin: number): void {
  ...
  if (stability >= COLONY.build.birthNeglectStability) {
    ...
    if (homes > 0 && children < cap)
      children = Math.min(
        cap,
        children +
          homes * COLONY.build.birthRatePerHomePerDay * stability * frac,
      );
    ...
    if (room > 0 && children > 0) {
      const mature = Math.min(
        children,
        children * COLONY.build.childMatureFraction * frac,
        room,
      );
      children -= mature;
    ...
    children = Math.max(
      0,
      children - children * COLONY.build.childNeglectDrainPerDay * frac,
    );
  }
...
export function effectiveBuildRadius(state: ColonyState): number {
  return (
    COLONY.build.maxBlockRadius +
    Math.min(state.claims ?? 0, COLONY.build.maxClaims)
  );
}

/** Spec 051 — Footprint readout for the HUD: the effective build radius, claims made / allowed, and the current survey progress. */
export function footprintStatus(state: ColonyState): {
  radius: number;
  claims: number;
  maxClaims: number;
  progress: number;
  ...
  return {
    radius: effectiveBuildRadius(state),
    claims,
    maxClaims: COLONY.build.maxClaims,
    progress: state.claimProgress ?? 0,
    camp: countKind(state, "surveycamp") > 0,
    atEdge: claims >= COLONY.build.maxClaims,
  };
}
...
function claimStep(state: ColonyState, dtMin: number): void {
  const camps = state.buildings.filter(
    (b) => b.artifact.kind === "surveycamp",
  ).length;
  if (camps === 0) return; // inert — no camp, no claims
  if ((state.claims ?? 0) >= COLONY.build.maxClaims) {
    state.claimProgress = 0;
    return;
  } // frontier maxed — nothing left to claim
  const staffing =
    state.totalJobs > 0 ? Math.min(1, state.colonists / state.totalJobs) : 0;
  ...
  let progress =
    (state.claimProgress ?? 0) +
    camps *
      (1 / COLONY.build.claimWorkDays) *
      staffing *
      powerFactor(state) *
      frac; // a brownout slows the survey
  if (progress >= 1) {
...
export function calendarStatus(state: ColonyState): {
  year: number;
  month: number;
  ...
  const month =
    Math.floor((day % COLONY.build.daysPerYear) / COLONY.build.daysPerMonth) +
    1; // 1..12
  return {
    year,
    month,
    monthsToFounders: 13 - month,
    office: countKind(state, "calendar") > 0,
  };
}
...
export function calendarStep(state: ColonyState): void {
  const year = Math.floor(
    Math.max(0, state.clock?.day ?? 0) / COLONY.build.daysPerYear,
  );
  if (year <= (state.lastFoundersYear ?? 0)) return; // no new year has turned
  if (calendarStaffed(state))
    state.unrest = Math.max(
      0,
      (state.unrest ?? 0) - COLONY.build.foundersDayUnrestRelief,
    ); // a free, annual morale lift
...
function throwSupper(state: ColonyState): void {
  ...
  const served = Math.max(
    0,
    Math.min(tables, bySupply, Math.floor(tables * staffing)),
  );
  ...
  state.food = Math.max(
    0,
    state.food - served * COLONY.build.festGreensPerTable,
  );
  ...
  fishNeed -= freshUse;
  if (fishNeed > 0)
    state.driedFish = Math.max(0, (state.driedFish ?? 0) - fishNeed);
  state.linen = Math.max(
    0,
    (state.linen ?? 0) - served * COLONY.build.festLinenPerTable,
  );
  state.materials = Math.max(
    0,
    state.materials - served * COLONY.build.festMaterialsPerTable,
  );
  ...
  if (coverage >= COLONY.build.festFullCoverage) {
    ...
    state.unrest = Math.max(
      0,
      (state.unrest ?? 0) - COLONY.build.festUnrestRelief,
    ); // a full supper calms the colony
    state.standing = Math.min(
      1,
      (state.standing ?? 0.5) + COLONY.build.festStandingGain,
    ); // ...and the wider world notices
  } else {
...
function festivalStep(state: ColonyState, dtMin: number): void {
  if ((state.festivalCheer ?? 0) > 0)
    state.festivalCheer = Math.max(0, (state.festivalCheer ?? 0) - dtMin); // the cheer fades over its window
  ...
  if (
    cal.year > (state.lastFestivalYear ?? 0) &&
    cal.month >= COLONY.build.festivalMonthStart &&
    cal.month <= COLONY.build.festivalMonthEnd
  ) {
...
export function festivalStatus(state: ColonyState): {
  board: boolean;
  cheerDays: number;
  bonus: number;
  active: boolean;
  ...
  return {
    board,
    cheerDays: Math.ceil(cheer / (24 * 60)),
    bonus: cheer > 0 ? (state.festivalCheerBonus ?? 0) : 0,
    active: cheer > 0,
  };
}
...
export function seasonOf(month: number): { name: string; multiplier: number } {
  ...
  if (month <= 6)
    return { name: "Highsun", multiplier: COLONY.build.highsunYield };
...
export function seasonStatus(state: ColonyState): {
  name: string;
  modifier: number;
  ...
  return {
    name: s.name,
    modifier: Math.round((s.multiplier - 1) * 100),
    solarModifier: Math.round((solarSeasonOf(month) - 1) * 100),
    active: countKind(state, "calendar") > 0,
  };
}
  ...
  const fed =
    countKind(state, "depot") > 0
      ...
      : (state.food ?? 0) > 0
        ...
        : 0;
  const care =
    (healthFraction(state) +
      wateredFraction(state) +
      fed +
      (1 - Math.min(1, Math.max(0, state.unrest ?? 0)))) /
    4;
...
export function ledgerStatus(state: ColonyState): {
  ageYears: number;
  onset: number;
  turning: boolean;
  lastPassings: number;
  ...
  return {
    ageYears,
    onset: COLONY.build.naturalSpanYears,
    turning: ageYears >= COLONY.build.naturalSpanYears,
    lastPassings: Math.round(state.lastPassings ?? 0),
    hall: countKind(state, "hallofnames") > 0,
  };
}
...
export function ledgerStep(state: ColonyState): void {
  ...
  const ramp =
    1 +
    Math.max(0, ageYears - COLONY.build.naturalSpanYears) *
      COLONY.build.naturalPassRampPerYear; // an older colony has more elders
  let passings =
    state.colonists * COLONY.build.naturalPassRate * ramp * careFactor(state);
  ...
  passings = Math.min(
    passings,
    Math.max(0, state.colonists - COLONY.seed.colonists),
  ); // the founding crew always remains
  passings = Math.max(0, passings);
  if (passings <= 0) return;
  state.colonists -= passings;
  state.lastPassings = passings;
  const hallStaffed =
    countKind(state, "hallofnames") > 0 &&
    (state.totalJobs > 0 ? state.colonists / state.totalJobs : 0) > 0;
  if (hallStaffed)
    state.unrest = Math.max(
      0,
      (state.unrest ?? 0) - COLONY.build.remembranceRelief,
    ); // remembrance eases grief
}
...
function serviceUpkeep(state: ColonyState, dtMin: number): void {
  ...
  for (const b of state.buildings) {
    if (b.artifact.kind === "water")
      upkeep += COLONY.build.waterHubMaintCompPerDay;
    else if (b.artifact.kind === "depot")
      upkeep += COLONY.build.depotMaintCompPerDay;
    else if (b.artifact.kind === "clinic") {
      upkeep += COLONY.build.clinicMaintCompPerDay;
      linenUpkeep +=
        COLONY.build.clinicLinenPerDay *
        (1 + (state.outbreak ?? 0) * (COLONY.build.clinicLinenFeverMult - 1)); // spec 031 — more dressings during an outbreak
    } else if (b.artifact.kind === "theatre") {
      upkeep += COLONY.build.theatreMaintCompPerDay;
      reelUpkeep += COLONY.build.theatreReelsPerDay;
    } else if (b.artifact.kind === "survey")
      matUpkeep += COLONY.build.surveyMaintMatPerDay;
    else if (b.artifact.kind === "battery")
      upkeep += COLONY.build.batteryMaintCompPerDay;
    else if (b.artifact.kind === "scrubber")
      upkeep += COLONY.build.scrubberMaintCompPerDay;
    else if (b.artifact.kind === "maintshed")
      upkeep += COLONY.build.maintShedMaintCompPerDay; // spec 022 — spare parts
    else if (b.artifact.kind === "storehouse")
      upkeep += COLONY.build.storehouseMaintCompPerDay; // spec 023 — logistics
    else if (b.artifact.kind === "bellhouse")
      upkeep += COLONY.build.bellhouseMaintCompPerDay; // spec 024 — foam/alarm upkeep
    else if (b.artifact.kind === "levy")
      upkeep += COLONY.build.levyMaintCompPerDay; // spec 025 — ledger supply
    else if (b.artifact.kind === "feverwatch")
      upkeep += COLONY.build.feverWatchMaintCompPerDay; // spec 026 — medical supply
    ...
    } else if (b.artifact.kind === "ward")
      upkeep += COLONY.build.wardMaintCompPerDay; // spec 028 — patrol supply
    else if (b.artifact.kind === "payoffice")
      upkeep += COLONY.build.payOfficeMaintCompPerDay; // spec 029 — ledger supply
    else if (b.artifact.kind === "feast")
      upkeep += COLONY.build.feastDeckMaintCompPerDay; // spec 030 — deck upkeep
    else if (b.artifact.kind === "liaison")
      upkeep += COLONY.build.liaisonMaintCompPerDay; // spec 032 — dispatch supply
    else if (b.artifact.kind === "stormwatch")
      upkeep += COLONY.build.stormwatchMaintCompPerDay; // spec 034 — watch supply
    else if (b.artifact.kind === "hall")
      upkeep += COLONY.build.hallMaintCompPerDay; // spec 035 — archive + roster upkeep
    else if (b.artifact.kind === "census")
      upkeep += COLONY.build.censusMaintCompPerDay; // spec 040 — survey + ledger supply
    else if (b.artifact.kind === "import")
      upkeep += COLONY.build.importOfficeMaintCompPerDay; // spec 036 — office + dispatch supply
    else if (b.artifact.kind === "shrine")
      linenUpkeep += COLONY.build.shrineLinenPerDay; // spec 037 — prayer flags + memorial wraps (linen)
    else if (b.artifact.kind === "comptroller")
      upkeep += COLONY.build.comptrollerMaintCompPerDay; // spec 039 — ledger + audit supply
    else if (b.artifact.kind === "roster")
      upkeep += COLONY.build.rosterMaintCompPerDay; // spec 038 — roster + dispatch supply
  }
  if (upkeep > 0)
    state.components = Math.max(
      0,
      state.components - upkeep * (dtMin / (24 * 60)),
    );
  if (matUpkeep > 0)
    state.materials = Math.max(
      0,
      state.materials - matUpkeep * (dtMin / (24 * 60)),
    );
  if (reelUpkeep > 0)
    state.reels = Math.max(0, state.reels - reelUpkeep * (dtMin / (24 * 60)));
  if (linenUpkeep > 0)
    state.linen = Math.max(
      0,
      (state.linen ?? 0) - linenUpkeep * (dtMin / (24 * 60)),
    ); // spec 031
}
...
function foodStep(state: ColonyState, dtMin: number): void {
  ...
  const staffing =
    ...
    orderFactor(state) *
    toolSupplyFactor(state) *
    seedSupplyFactor(state) *
    seasonFactor(state); // spec 009/017/021/026/028/038/047/048/054 — sick, brownout, congested, fevered, restless, sector-deprioritised, tool-starved, seed-starved or out-of-season greenhouses grow less (seasons inert without a Calendar Office)
  let grown = 0;
  for (const b of state.buildings) {
    if (b.artifact.kind !== "greenhouse" || b.incident) continue; // spec 024 — a blighted greenhouse grows nothing
    const boost = nearWater(state, b.x, b.y)
      ? COLONY.build.greenhouseWaterBoost
      : 1;
    grown +=
      COLONY.build.greenhouseFoodPerDay * boost * staffing * maintFactor(b); // spec 022 — a worn greenhouse grows less
  }
  ...
  const consumption =
    (state.colonists +
      (state.children ?? 0) * COLONY.build.childDependentLoad) *
    COLONY.build.foodPerColonistPerDay *
    (dtMin / day);
  ...
  if (freshFish > 0)
    state.rimfish = Math.max(0, (state.rimfish ?? 0) - freshFish);
  const driedFishMeals = Math.min(state.driedFish ?? 0, fishCap - freshFish);
  if (driedFishMeals > 0)
    state.driedFish = Math.max(0, (state.driedFish ?? 0) - driedFishMeals);
  const duskMeals = Math.min(
    state.duskcap ?? 0,
    fishCap - freshFish - driedFishMeals,
  ); // spec 068 — the cellar's duskcap fills the protein course after the fish, sparing skygrain
  if (duskMeals > 0)
    state.duskcap = Math.max(0, (state.duskcap ?? 0) - duskMeals);
...
function dietTrack(
  ...
  state.dietShort =
    (state.dietShort ?? 0) * decay +
    Math.max(0, demand - grainMeals - fishMeals);
}
...
export function varietyCovered(state: ColonyState): number {
  ...
  return Math.min(
    1,
    (COLONY.build.varietyCounterCapacity * counters) / state.colonists,
  );
}
...
function dietQualifies(state: ColonyState): boolean {
  ...
  if (
    share < COLONY.build.varietyMinShare ||
    share > COLONY.build.varietyMaxShare
  )
    return false; // one food is only a token
  if ((state.dietShort ?? 0) > total * COLONY.build.dietShortTolerance)
    return false; // the larder ran short in the window
  return true;
}
...
function dietStep(state: ColonyState, dtMin: number): void {
  if (dietQualifies(state)) state.dietStanding = 1;
    ...
    state.dietStanding = Math.max(
      0,
      (state.dietStanding ?? 0) -
        dtMin / (COLONY.build.varietyHoldDays * 24 * 60),
    );
}
...
export function varietyDesirabilityFactor(state: ColonyState): number {
  return (
    1 +
    COLONY.build.varietyDesirabilityBonus *
      varietyCovered(state) *
      (state.dietStanding ?? 0)
  );
}
...
export function dietVarietyStatus(state: ColonyState): {
  counters: number;
  covered: number;
  served: number;
  standing: number;
...
export type ImportGood =
  | "materials"
  | "components"
  | "food"
  | "linen"
  | "reels";
export const IMPORT_GOODS: ImportGood[] = [
  "materials",
  "components",
  "food",
  "linen",
...
function importStep(state: ColonyState, dtMin: number): void {
  ...
  const headroom = Math.max(
    0,
    importCap(state, good) - importStock(state, good),
  ); // a full store stops the delivery
...
export function importStatus(state: ColonyState): {
  active: boolean;
  ...
  const perDay =
    active && good ? offices * COLONY.build.importPerDay * staffing : 0;
...
export function arrearsStrain(state: ColonyState): boolean {
  return (
    comptrollerExists(state) &&
    colonyDebt(state) >
      COLONY.build.debtCeiling * COLONY.build.arrearsStrainFraction
  );
}
...
export function arrearsStatus(state: ColonyState): {
  office: boolean;
  debt: number;
  ceiling: number;
  strain: boolean;
  ...
  return {
    ...
    ceiling: COLONY.build.debtCeiling,
    strain: arrearsStrain(state),
    unmanaged: office && debt > 0 && !comptrollerActive(state),
  };
}
...
function tradeStep(state: ColonyState, dtMin: number): void {
  ...
  if (staffing <= 0) return;
  const frac = dtMin / (24 * 60);
  const compSell = Math.min(
    Math.max(0, state.components - COLONY.build.tradeComponentReserve),
    exchanges * COLONY.build.tradeComponentCapPerDay * staffing * frac,
  );
  if (compSell > 0) {
    state.components -= compSell;
    state.treasury += compSell * COLONY.build.tradeComponentPrice;
  }
  const foodSell = Math.min(
    Math.max(0, state.food - COLONY.build.tradeFoodReserve),
    exchanges * COLONY.build.tradeFoodCapPerDay * staffing * frac,
  );
  if (foodSell > 0) {
    state.food -= foodSell;
  ...
  const reelSell = Math.min(
    Math.max(0, state.reels - COLONY.build.reelReserve),
    exchanges * COLONY.build.reelCapPerDay * staffing * frac,
  );
  if (reelSell > 0) {
  ...
  const folioSell = Math.min(
    Math.max(0, (state.folios ?? 0) - COLONY.build.folioReserve),
    exchanges * COLONY.build.folioCapPerDay * staffing * frac,
  );
  if (folioSell > 0) {
...
export function tradeExportRate(state: ColonyState): number {
  ...
  const compSell = Math.min(
    Math.max(0, state.components - COLONY.build.tradeComponentReserve),
    exchanges * COLONY.build.tradeComponentCapPerDay * staffing,
  );
  const foodSell = Math.min(
    Math.max(0, state.food - COLONY.build.tradeFoodReserve),
    exchanges * COLONY.build.tradeFoodCapPerDay * staffing,
  );
  const reelSell = Math.min(
    Math.max(0, state.reels - COLONY.build.reelReserve),
    exchanges * COLONY.build.reelCapPerDay * staffing,
  );
  const folioSell = Math.min(
    Math.max(0, (state.folios ?? 0) - COLONY.build.folioReserve),
    exchanges * COLONY.build.folioCapPerDay * staffing,
  ); // spec 044
  return (
    compSell * COLONY.build.tradeComponentPrice +
    foodSell * COLONY.build.tradeFoodPrice +
    reelSell * COLONY.build.reelPrice +
    folioSell * COLONY.build.folioPrice
  );
}
...
function stallStep(state: ColonyState, dtMin: number): void {
  ...
  const served = Math.min(
    COLONY.build.stallServedCap * stalls * staffing,
    state.colonists,
  );
  const salesPerDay =
    Math.floor(served / COLONY.build.stallServedPerSale) * wage; // one sale per 20 served; halved in arrears
  if (salesPerDay <= 0) return;
  let remaining = salesPerDay * (dtMin / (24 * 60));
  const reserve = COLONY.build.stallReserve;
  const order: ("linen" | "folios")[] =
    Math.max(0, (state.linen ?? 0) - reserve) >=
    Math.max(0, (state.folios ?? 0) - reserve)
      ? ["linen", "folios"]
      : ["folios", "linen"];
  let sold = 0;
...
export function stallStatus(state: ColonyState): {
  stalls: number;
  open: boolean;
  coinPerDay: number;
} {
  ...
  const sales = Math.min(
    Math.floor(served / COLONY.build.stallServedPerSale) * wage,
    surplus,
  );
  return {
    stalls,
    open: staffing > 0 && wage > 0 && sales > 0,
    coinPerDay: Math.round(sales * COLONY.build.stallCoinPerSale),
  };
}
...
export function galleryStep(state: ColonyState, dtMin: number): void {
  ...
  if (staffing <= 0) return;
  const coinPerDay =
    COLONY.build.galleryVisitorCoin *
    galleryAppeal(state) *
    staffing *
    galleries;
...
export function galleryStatus(state: ColonyState): {
  galleries: number;
  open: boolean;
  coinPerDay: number;
} {
  ...
  return {
    galleries,
    open: staffing > 0 && coin > 0,
    coinPerDay: Math.round(coin),
  };
}
...
export function porterStatus(state: ColonyState): {
  sheds: number;
  working: boolean;
  porters: number;
} {
  ...
  return {
    sheds,
    working,
    porters: working ? sheds * COLONY.build.portersPerShed : 0,
  };
}
...
export function avatarStatus(state: ColonyState): {
  foundries: number;
  staffed: boolean;
  capacity: number;
} {
  ...
  return {
    foundries,
    staffed,
    capacity: staffed ? foundries * COLONY.build.avatarMaxCitizens : 0,
  };
}
...
export function stepBuild(state: ColonyState, rng: RNG, dtMin: number): void {
  ...
  state.renewalThisYear =
    (state.renewalThisYear ?? 0) +
    Math.max(0, state.colonists - popAtStepStart); // spec 055 — accumulate the year's net renewal (arrivals + births) before the Ledger reads it
  ...
  for (let i = state.jobs.length - 1; i >= 0; i--) {
    ...
    if (j.progress >= 1) {
      const nb: ColonyBuilding = {
        id: j.id,
        x: j.x,
        y: j.y,
        artifact: j.artifact,
      };
      if (j.artifact.kind === "habitat") {
        nb.tier = 1;
        nb.dryMin = 0;
      } // spec 006 — homes start at tier 1
      ...
      if (a.kind === "solar") state.powerGen += a.powerGen;
        ...
        state.power.batteryCapWh += COLONY.build.batteryShedCapWh; // spec 018 — a bigger tank
      else {
        ...
        if (a.kind === "industrial")
          state.pollution += COLONY.build.pollutionPerIndustrial;
      }
      if (a.kind === "cistern")
        state.water = Math.min(
          waterTankCap(state),
          (state.water ?? 0) +
            COLONY.build.cisternTankCap * COLONY.build.cisternStartCharge,
        ); // spec 046 — a freshly built cistern starts its tank charged (no construction-day water crash)
      if (a.kind === "toolcrib")
        state.tools = Math.min(
          toolStockCap(state),
          (state.tools ?? 0) +
            COLONY.build.toolStockCap * COLONY.build.toolStartCharge,
        ); // spec 047 — a freshly built crib starts its rack charged (no construction-day output crash)
      if (a.kind === "seedloft")
        state.seed = Math.min(
          seedStockCap(state),
          (state.seed ?? 0) +
            COLONY.build.seedStockCap * COLONY.build.seedStartCharge,
        ); // spec 048 — a freshly built loft starts its bin charged (no construction-day harvest crash)
  ...
  if (state.clock.day > state.lastIncomeDay) {
    ...
    const pollutionPenalty = Math.min(
      0.3,
      state.pollution / COLONY.economy.pollutionPenaltyScale,
    );
    const income =
      state.colonists *
      COLONY.economy.incomePerColonistPerDay *
      (0.6 + 0.4 * rate) *
      (1 - pollutionPenalty) *
      levyIncomeFactor(state) *
      (1 - (state.unrest ?? 0) * COLONY.build.unrestIncomeRefusal); // spec 025/028 — the levy rate scales the take; unrest refuses a slice of it
    const upkeep =
      state.buildings.length * COLONY.economy.buildingUpkeepPerDay +
      roadUpkeepPerDay(state); // spec 084 S3 — per-kind road bill
    state.treasury += (income - upkeep - payrollPerDay(state)) * days; // spec 029 — the colony pays its workers a daily wage
    if (state.treasury < 0) {
      // spec 039 — interest accrues on the deficit each payday, doubled while the debt desk is unstaffed (unmanaged arrears)
      const rate =
        COLONY.build.debtInterestPerPayday *
        (comptrollerActive(state) ? 1 : COLONY.build.debtUnmanagedMult);
  ...
  if (
    state.clock.totalMinutes - state.lastGrowMin >=
    COLONY.build.growIntervalHours * 60
  ) {
    state.lastGrowMin = state.clock.totalMinutes;
    autoGrow(state, rng);
  }
