...
export async function registerSettler(name: string): Promise<KookerCard> {
  ...
  const body = JSON.stringify({ name, username, email, locale: 'en', apps: ['citylife'] })

  let lastErr = 'unknown error'
  for (let attempt = 1; attempt <= 4; attempt++) {
    try {
      const res = await fetch('/kooker/api/users', { method: 'POST', headers: { 'content-type': 'application/json' }, body })
      if (res.ok) {
  ...
  throw new Error(`Kooker gateway unavailable (${lastErr}) — it flaps under the cluster's RAM pressure. Try again in a moment.`)
}
