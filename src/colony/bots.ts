...
export const GREETING =
  "Welcome to the border of Landing One. Please state your name and tell us about your family.";
...
export function composeLifeHistory(h: Household): string {
  ...
  const family = h.members
    .map((m) => `${m.name} (${m.age}, ${m.occupation})`)
    .join(", ");
  return [
...
export async function generateSafePersonality(
  ...
  if (!input.ok)
    return {
      ok: false,
      reason: input.reason ?? "Please keep it friendly and family-appropriate.",
    };
  if (!magicPrompt || !magicPrompt.trim())
    return {
      ok: false,
      reason: "Tell us a little about who your colonist is.",
    };
  if (!adapter.generatePersonality)
    return {
      ok: false,
      reason: "Personality generation is unavailable right now.",
    };
  let out: string;
  try {
    ...
    return {
      ok: false,
      reason: "Could not reach the storyteller — please try again.",
    };
  }
  ...
  if (!safe.ok)
    return {
      ...
      reason:
        safe.reason ??
        "The generated personality was not family-appropriate — try a different idea.",
    };
...
export class MockBotAdapter implements BotAdapter {
  ...
  async generate(
    ...
    const lastOther = [...history]
      .reverse()
      .find((m) => m.speaker !== speakingAs && m.speaker !== "narrator");
    ...
    if (speakingAs === "patrol") {
      ...
      const hint = (() => {
        if (/(sea|beach|shore|coast)/.test(lastText))
          return plots.find((n) => /(beach|shell|crystal)/i.test(n));
        if (/(hill|mountain|view|ridge|height)/.test(lastText))
          return plots.find((n) => /(hill|ochre|ridge)/i.test(n));
        if (/(river|water|brook|creek)/.test(lastText))
          return plots.find((n) => /(river|brook|creek)/i.test(n));
        if (/(forest|wood|tree|quiet)/.test(lastText))
          return plots.find((n) => /(forest|grove|wood)/i.test(n));
        return undefined;
      })();
    ...
    // newcomer mock
    const first = (
      systemPrompt.match(/You are ([^,]+),/)?.[1] ?? "a settler"
    ).split(" ")[0];
    if (lastText.includes("name") || lastText.includes("who"))
      return `I'm ${first}. We've travelled a long way, hoping for a fresh start by the sea.`;
    if (lastText.includes("why") || lastText.includes("come"))
      return `Honestly, we needed a fresh start. The crossing was hard, but we're ready to build a real life here.`;
    if (lastText.includes("work") || lastText.includes("skill"))
      return `We're hard workers — whatever the colony needs, we'll learn it fast.`;
  ...
  async generatePersonality(magicPrompt: string): Promise<string> {
    const seed =
      magicPrompt
        .replace(/[^a-zA-Z ]/g, "")
        .trim()
        .slice(0, 60) || "a hopeful new arrival";
...
export async function resolveBotAdapter(): Promise<BotAdapter> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (cfg.botBackend && cfg.botBackend !== "kooker")
...
export class BotService {
  ...
  async narrateView(
    ...
    const timeOfDay = view.clock.isDay
      ? `day ${view.clock.day}, ${view.clock.hour}:${String(view.clock.minute).padStart(2, "0")}`
      : `night (day ${view.clock.day})`;
    ...
    if (view.mood.unrest > 0.4)
      moodBits.push("there is unease among the people");
    ...
    const systemPrompt = [
      ...
      nearBuildings
        ? `Nearby: ${nearBuildings}.`
        : "The area around you is sparse.",
    ...
    try {
      const history: ChatMessage[] = [
        { speaker: "patrol", text: "(describe)", ts: Date.now() },
      ];
  ...
  setCityPlan(plan: CityPlan): void {
    this.plan = plan;
    if (this.patrolBot)
      this.patrolBot.systemPrompt = composeCityBrainPrompt(plan);
  }
  ...
  private ensurePatrol(): Bot {
    if (this.patrolBot && this.patrolBot.status !== "deleted") {
      if (this.plan)
        this.patrolBot.systemPrompt = composeCityBrainPrompt(this.plan);
  ...
  async create(h: Household): Promise<Bot> {
    ...
    try {
      const reply = await this.adapter.generate(
        bot.systemPrompt,
        bot.messages,
        "newcomer",
      );
  ...
  private async patrolAllocates(newcomerBot: Bot, h: Household): Promise<void> {
    ...
    try {
      ...
      const newcomerLast = [...newcomerBot.messages]
        .reverse()
        .find((m) => m.speaker === "newcomer");
      ...
      const patrolView: ChatMessage[] = [
        { speaker: "newcomer", text: introText, ts: Date.now() },
      ];
      const reply = await this.adapter.generate(
        patrol.systemPrompt,
        patrolView,
  ...
  remove(botId: string): void {
    ...
    if (this.patrolBot && this.patrolBot.id === botId)
      this.patrolBot.status = "deleted";
  }

  forHousehold(householdId: string): Bot | undefined {
    return this.bots.find(
      (b) => b.householdId === householdId && b.status !== "deleted",
    );
  }
}
