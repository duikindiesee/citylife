...
export function blueprintReport(
  ...
  for (const r of p.rooms)
    roomAreas[r.kind] = (roomAreas[r.kind] ?? 0) + r.w * r.d;
  return {
...
function improveOnce(
  ...
  report: BlueprintReport,
  seed: number,
): { next: ParsedBlueprint; mutation: string } {
  if (!report.hasOutdoor) {
    return {
      next: addRoom(p, (seed & 1) === 0 ? "patio" : "pool"),
      mutation: "add-outdoor",
    };
  }
  ...
  if (
    report.storeys < 2 &&
    report.roomAreas["living"] !== undefined &&
    p.w * p.d -
      (report.roomAreas["patio"] ?? 0) -
      (report.roomAreas["pool"] ?? 0) >=
      p.w * p.d * 0.7
  ) {
  ...
  if (living >= 0)
    return { next: resizeRoom(p, living, 1, 0), mutation: "grow-living" };
...
export function selfDesign(
  ...
  return {
    script: current,
    report: blueprintReport(current, zone, seed),
    iterations,
  };
}
