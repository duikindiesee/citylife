...
test("R3F tarentaal: the flock renders from the sim-stepped state", async ({
  page,
}) => {
  ...
  await page.waitForFunction(
    () => {
      ...
      (window as any).__r3fScene?.traverse((o: any) => {
        if (o.name === "tarentaal-adults") found = true;
      });
      return found;
    },
    undefined,
  ...
  console.log(
    `tarentaal adults: ${probe.adults}, chicks: ${probe.chicks}, flock size: ${probe.flock}`,
  );
