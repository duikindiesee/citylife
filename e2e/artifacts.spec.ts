...
test("R3F artifacts: the civic-art catalog renders from sim state", async ({
  page,
}) => {
  ...
  await page.waitForFunction(
    () => {
      ...
      (window as any).__r3fScene?.traverse((o: any) => {
        if (o.name === "artifact-bench") found = true;
      });
      return found;
    },
    undefined,
  ...
  const probe = await page.evaluate((kinds) => {
    ...
    const total = kinds.reduce(
      (s: number, k: string) => s + (counts[k] ?? 0),
      0,
    );
    const roster =
      (window as any).__colony?.sim?.state?.artifacts?.length ?? -1;
    return { counts, total, roster };
  }, KINDS);

  console.log(
    `artifact instance counts: ${JSON.stringify(probe.counts)}, total: ${probe.total}, roster: ${probe.roster}`,
  );
