...
test('R3F port-finish: porters, nameplate layer, snapshot PNG', async ({ page }) => {
  ...
  await page.waitForFunction(() => {
    let f = false;
    (window as any).__r3fScene?.traverse((o: any) => { if (o.name === 'porters') f = true; });
    return f;
  ...
  const probe = await page.evaluate(() => {
    ...
    let piles = -1, cartsMesh = false, plates = false;
    scene.traverse((o: any) => {
      if (o.name === 'porter-piles') piles = o.count;
      if (o.name === 'porter-carts') cartsMesh = true;
      if (o.name === 'rally-nameplates') plates = true;
    });
    const sheds = rt.sim.state.buildings.filter((b: any) => b.artifact.kind === 'porter').length;
    return { piles, cartsMesh, plates, sheds, materials: rt.sim.state.materials, food: rt.sim.state.food };
  });
