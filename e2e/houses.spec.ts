...
test("R3F houses/lots: seated houses, draped overlays, no trees on lots", async ({
  page,
}) => {
  ...
  const probe = await page.evaluate(() => {
    ...
    scene.traverse((o: any) => {
      if (typeof o.name === "string" && o.name.indexOf("zone-ground-") === 0)
        overlays.push(o);
    });
    let drapedOk = true;
    let checked = 0;
    for (const ov of overlays) {
      if (!ov.isInstancedMesh) {
        drapedOk = false;
        break;
      }
      ...
      for (let i = 0; i < Math.min(ov.count, 8); i++) {
        ov.getMatrixAt(i, m);
        const wx = m.elements[12],
          wy = m.elements[13],
          wz = m.elements[14];
        const gx = Math.round(wx / 4 + N / 2),
          gy = Math.round(wz / 4 + N / 2);
        const ground = s.terrain.worldY(gx, gy);
        if (Math.abs(wy - ground) > 0.5) {
          drapedOk = false;
        }
        checked++;
      }
