...
test("R3F avatars: the citizen layer draws roster avatars in the scene", async ({
  page,
}) => {
  ...
  await page.waitForFunction(
    () => {
      ...
      (window as any).__r3fScene?.traverse((o: any) => {
        if (o.name === "avatar-bodies") found = true;
      });
      return found;
    },
    undefined,
  ...
  console.log(
    `avatar capsules drawn: ${counts.meshCount}, crabs: ${counts.crabs} (crab group visible: ${counts.crabVisible}), roster size: ${counts.roster}`,
  );
