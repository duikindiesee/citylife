...
test("R3F pedestrians: the crowd renders and tracks the colony population", async ({
  page,
}) => {
  ...
  await page.waitForFunction(
    () => {
      ...
      (window as any).__r3fScene?.traverse((o: any) => {
        if (o.name === "pedestrian-bodies") found = true;
      });
      return found;
    },
    undefined,
  ...
  console.log(
    `pedestrian bodies: ${probe.bodies}, heads: ${probe.heads}, colonists: ${probe.colonists}`,
  );
  ...
  if (probe.colonists >= 0) {
    expect(probe.bodies).toBe(
      Math.max(0, Math.min(28, Math.round(probe.colonists))),
    );
  }
});
