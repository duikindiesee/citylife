...
| **Player & UI**        | first-person / mobile / camera / controls, in-world characters + avatars, nameplates, the social read, the Kookerbook UI + in-browser navigation | `src/colony/ui/FirstPersonPanel.tsx`, `ColonyApp.tsx`, `colony.css`, the nameplate path in `render/R3FRallyNameplates.tsx`, `src/colony/bot/firstPersonView.ts` | —        |
