...
## Cap-quality pass (operator: "intersections not perfect", "corners inverted 90°")

Two render-only fixes for the ragged coastal/satellite junctions the operator flagged on
main v3. Both touch the ribbon WAYS, not the road cells traffic/bus/rally drive.

1. **Self-intersecting cap polygons — sanitized.** The exact carriageway-union walk is
   right for clean crossings, but a SHALLOW/degenerate junction (two near-parallel arms, an
   oblique cross whose mouth clamps at `MOUTH_MAX`) makes the kerb-corner intersections blow
   up into a self-crossing, near-duplicate-point outline. `pointInPoly` then returns garbage
   (patchy paint suppression → ragged edge lines), `capKerbLines` traces the self-crossing
   (ragged white teeth), and `drapeCap`'s centre-fan inverts triangles (messy cap). Measured
   live: seed 4242 zone Z0 (a ~28° crossing at grid 486,368) had a self-intersecting poly.
   `sanitizeCapPoly` (in `junctionCap.ts`) drops consecutive near-duplicate vertices and, if
   the outline still self-intersects, falls back to the **convex hull** of its points — always
   a simple CCW polygon. Only the broken degenerate cases take the hull; a valid plus-shape
   cross keeps its exact concave outline (its reflex corners never self-cross), so the
   general-case exact-union geometry is untouched. Pinned in `junctionCap.test.ts`.

2. **Spine ribbon ways string-pulled.** The founders' avenue, each hamlet spine and the
   commercial high street were pushed as ribbon ways from their RAW least-cost spines (cell-by-
   cell staircases), while the trunk roads (`layRoad`) string-pull into clean straight runs
   first. So the avenue ribbon wiggled and its edge lines jittered into the ragged mess along
   the founders' avenue (seed 4242 W5: 231 points). `runtime.ts` now runs the same
   `simplifyPath` over these spines before recording them — every ribbon centre-line, trunk or
   spine, is a clean polyline. Render-only; the road cells are untouched.

**Still open (not in this pass):** dense multi-road CONVERGENCES — where several satellite
trunk-links meet the coast/commercial area at oblique angles (seed 4242 grid ~455,386) — remain
messy: overlapping oblique caps, scattered crosswalks, verge furniture crowded onto sand. That
is a road-NETWORK topology issue (the trunk-link mesh creates oblique convergences), not a cap
geometry bug, and wants a planning-level fix (cleaner/orthogonal trunk connections, fewer
redundant links) rather than more render patching.
