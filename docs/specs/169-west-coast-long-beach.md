...
Terrain cost **~150–200 ms per reach** (extrapolated, _not measured_ — measure before committing to synchronous generation).
...
2. Reaches append **south only**, noise sampled in absolute strip coordinates, so reach _k_ is a pure function of `(lbSeed, x, y_global)`. This needs the one real engine change: **`Terrain` must support rectangular `(width, height)`** — mechanical but wide, hence behind the gate.
...
- **Straights** are about _holding_ top speed with the giant on the horizon, not reaching it (acceleration takes <1 s). Signature straight ≥ **120 cells (480 m ≈ 10–14 s flat-out)**.
