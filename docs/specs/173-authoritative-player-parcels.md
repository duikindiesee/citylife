...
Task `e4f805e7-afff-4a93-90d3-bdebb7645f9d` tracks server-backed vehicle offers and remains a dependency of final deployed acceptance. Acquisition-enabled Gearbox now reads `/kooker/api/v1/citylife/players/me/vehicle/offers`, displays its canonical model's whole-KCO price and never falls back to a planned amount. Failed/malformed reads or absent offers withhold Acquire and provide price refresh. Preview-only mode keeps its explicitly planned catalogue labels. Reads are bounded, no-store and discard responses after identity changes; the showroom is keyed by account. Purchase still submits only the vehicle key and the server rechecks price and eligibility. This is price discovery, not a reserved quote. The backend offer endpoint must deploy before this client; publication and real purchase/display evidence remain outstanding.

Visual acceptance initially failed: the complete fixture journey passed its ownership/request/reload assertions, but its home-arrival screenshot placed the view underneath the X19. The car used leveled driveway height while the seated camera incorrectly used a road-only height off-road. Both now use `ownedVehicleSurfaceY`: road ribbon height on roads, interpolated leveled ground on driveways. The local browser verifies the 1.05 m eye offset at spawn and after keyboard driving onto the road, with inspectable screenshots. Measured X19 footprint is 4.08 by 1.825 m, within the 4.10 by 1.84 m driving footprint; no model rescaling was needed.
