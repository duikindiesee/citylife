...
- **Design provenance:** the player goal requires at least two signed-in players to drive their own cars on shared roads, see each other in-world and on the map, and preserve identity/isolation through reconnects. Current source snapshots: CityLife main `083e8ae55df9ee2bd7059249ea5cde8897fbd19d` ([owned-car controller](https://github.com/duikindiesee/citylife/blob/083e8ae55df9ee2bd7059249ea5cde8897fbd19d/src/colony/car/ownedDriving.ts), [presence readout](https://github.com/duikindiesee/citylife/blob/083e8ae55df9ee2bd7059249ea5cde8897fbd19d/src/colony/spatial/presenceReadout.ts)); Kooker Infra main `e6b55ca2b968708f5e0249b40d8e061e4b80532b` ([APISIX routes](https://github.com/duikindiesee/kooker-infra/blob/e6b55ca2b968708f5e0249b40d8e061e4b80532b/manifests/base/apisix-routes/apisix-routes.yaml)); and issue #529. The current CityLife source has local car movement and presence projection, but no gameplay WebSocket transport. The Infra base manifest's `kooker-service-citylife-route` matches `/api/v1/citylife` and `/api/v1/citylife/*`, enables `jwt-auth`, and targets `kooker-service-user`; it contains no `/api/v1/citylife/realtime` route or WebSocket enablement. This is source configuration, not evidence of the rendered or deployed gateway. A dedicated WSS route must take precedence over the broad HTTP route and be checked in both rendered config and live APISIX before release. APISIX documents route-level WebSocket support in its [Ingress Controller CRD](https://apisix.apache.org/docs/ingress-controller/reference/apisix-ingress-controller/api-reference/) and connection limits through [`limit-conn`](https://apisix.apache.org/docs/apisix/plugins/limit-conn/). This spec does not claim a realtime service or WSS route exists or is deployed.
...
- A socket close or heartbeat timeout without a `session_ended` event is an unexpected transport loss, not proof that the room ended. The client enters `reconnecting` and asks the authenticated control-plane ticket endpoint for a fresh ticket. Before issuing it, the control plane checks the current single realtime replica for the same active session and membership. It returns a fresh ticket only while that room is live, HTTP 409 with stable code `SESSION_ENDED` when the room is absent (including after replica restart), and HTTP 503 with stable code `REALTIME_UNAVAILABLE` when liveness cannot be determined. Clients retry only `REALTIME_UNAVAILABLE` with capped backoff; they must not interpret a network outage as an ended session.
- `session_ended` is a best-effort server event, never the only restart signal. On `SESSION_ENDED`, clients close the old socket, discard its ticket and membership, remove every remote vehicle and map marker, and show the session-ended state with a create/join path. Remote snapshots also expire locally 15 seconds after their last accepted server snapshot, using a monotonic receive-time deadline; remove the peer from both world and map at expiry even if no final frame arrives. Until a fresh authenticated snapshot arrives, no cached or synthetic peer may be shown as current.
...
4. Disconnect and reconnect both clients; show one vehicle per account throughout, stale removal after the 30-second slot expiry, and no cross-session visibility. Test replacement sockets and fresh ticket issuance for a still-live room.
5. Restart and abruptly terminate the realtime process while clients are connected, including a case where no final `session_ended` frame can be sent. Prove clients discover `SESSION_ENDED` through the authenticated ticket check, clear remote cars and map markers within the 15-second snapshot expiry, and expose the create/join path without fabricating multiplayer.
6. Load-test 8 participants at the configured 20 Hz/10 Hz rates; require p95 server simulation time below 25 ms and p99 below the 50 ms tick budget, and p95 snapshot age below 300 ms at clients. Retain p50/p95/p99 tick cost, snapshot age, rejected-message counts, CPU/memory, resource requests and the accepted marginal-cost decision.
7. Retain exact deployment/image/config identifiers and inspectable screenshots/video from the actual deployed CityLife interface. Source tests, HTTP status, a local mock, a single browser, or synthetic peer snapshots do not satisfy acceptance.
